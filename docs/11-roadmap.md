...
| **M3** (STARTED 2026-09-15; **exit gate RUN 2026-09-16 and FAILED** - T-206 acceptance_m3 7/9: unknown recall 0.7778 vs 0.80, false-known 0.2222 vs 0.10, ruled over the full grid; blockers T-247 C15 call site, T-248 analog/psk-qam abstention) | Classification | cascaded classical classifier + open-set (C15), fingerprint clustering (C18), ml-runtime (C38) on GPU, on-device fine-tuning from labelled captures | after normalization exists (C13/C14 from M0); ML as a stage, classical first (CLAUDE.md) | S5 |
