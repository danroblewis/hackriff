...
**Status: draft (2026-09-13).** The cards follow the *draft* taxonomy in docs/06, which is awaiting approval in architecture planning. If the taxonomy changes (merges, splits, renames), the affected cards must be updated or regenerated. Example use cases in the cards were chosen mostly before the docs/06 §3 mapping was filled into `use-cases.yaml`. Where a card disagrees with the mapping, the YAML wins; see [taxonomy-feedback.md](taxonomy-feedback.md) §7. Issues found while writing the cards (missing edges, unowned responsibilities, overlaps, factual corrections) are collected in [taxonomy-feedback.md](taxonomy-feedback.md) for the architect's checkpoint. Cards name domain objects provisionally until `docs/07-data-model.md` exists. Once the ADRs land, the architect should update each card's Open questions and link the deciding ADR.
