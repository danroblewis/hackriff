...
## Methods
- **Drivers:** libhackrf directly (API-documented since 2024.02.1; One/Pro/rad1o), cargo feature `hackrf` (T-037a). **librtlsdr directly** for RTL-SDR dongles (NooElec NESDR Nano 3, R820T), cargo feature `rtlsdr` (T-514) — a second native driver rather than SoapySDR, which stays available as the generic path for later devices (docs/03 §1.3 "Hardware abstraction"). Both features are off by default, so the default build links neither library; both drivers pass the same `source::conformance` suite, the RTL against the real dongle as a HIL case.
- **Declaring a lesser front end (T-514):** the RTL descriptor is the worked example of the capabilities descriptor doing its job — 25 MHz–1.75 GHz (not 1 MHz–6 GHz), a discrete rate list capped at 2.4 Msps (not 20), `cu8` not `ci8`, receive-only, **one** combined gain stage of 29 uneven steps 0–49.6 dB, and no RF amp, baseband filter, bias tee, external clock or hardware timestamps. Each absence is declared, never faked: `set_bias_tee` is `Unsupported` and provenance reports `BiasTee::Unknown` — the driver never drives that pin, so it cannot claim `off`. `GainStage` cannot express 29 uneven steps, so the stage is declared continuous and the driver snaps to the **device's own** gain table (read at open) and records the value the device reports back; a request for 30 dB is answered with 29.7 dB in provenance.
