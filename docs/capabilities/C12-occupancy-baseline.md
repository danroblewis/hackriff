...
- **Front-end keying (resolved, T-303):** the baseline key carries the **receive chain** as well as the site and calibration. A noise floor is a property of one chain (antenna, cable, LNA, mixer), so two front ends at one site have genuinely different floors; `CalKey::Uncalibrated` is one value for all of them, so without the chain their floors averaged and novelty fired on the mixture. Keyed on the **device**, not the antenna port: a key is a total equality key and the on-disk path, so an `Option` port would split one device's history the day a switch is fitted. A port change is sequential and already explained as a self-inflicted provenance step; two devices are concurrent, and only the key separates them. **End to end (T-314):** the key alone was not enough — the occupancy close read history unfiltered (`Pyramid::query` = `OriginFilter::ANY`), so a per-chain key sat over a measurement that had already pooled every front end in the pyramid. Every occupancy read (the close, `span_stats`, the provenance-step read behind alarm explanations) now goes through `engine::OriginLevels` under `engine::chain_filter(chain)` — the T-133 filter on the same `device_id` hash the key uses. A level-0 tile holding two chains answers **unobserved** (counted in `OccupancyServiceStats::cells_excluded`), never the mixture and never the majority contributor: T-359's rule one layer down, at the cost of coverage rather than accuracy. The chain itself now comes from the source's own `DeviceInfo`, not `PipelineConfig::device_id` (a `"sigmf-replay"` default outside `hk replay`, naming no device that produced a frame); a source with no identity gives `ChainKey::Unknown` and an unrestricted read, so pooling is declared rather than claimed away. **Ingest too (T-377):** the read-side filter could not reach the *stored* occupancy, which was decided when the frame was folded against a floor the pyramid tracked per frequency block and every front end fed, so a pure tile could carry an occupancy decided against another chain's floor (`local_floors` then called it suspect and the baseline folded nothing — safe, but silent). `Pyramid::floors` is keyed `(source, f_block)` on the same device hash, each frame's threshold is resolved at push against its own origin's floor, and a sealed tile feeds only its own source's track — a tile pooling two front ends feeds none, T-359's rule at ingest. The pipeline's own `NoiseFloorTracker` is **not** split: it is fed from one segment's ring, which carries one source's blocks. Stored cells cannot be recomputed and are left as written; tile format 6 marks which ones carry the guarantee, and `PyramidStats::tiles_pre_origin_floor` counts the older ones. Single-device runs are unchanged.
- **Bias-tee keying (resolved, T-333):** the key also carries the antenna-port **bias-tee state**, which completes the receive chain — the DC powers an external LNA, so the floor moves the instant it arrives (the same fact T-331 keyed the floor tracker on). The comparison across differing states is **declined**, not disclosed-and-pooled, because a disclosed-but-pooled comparison still **masks**: a real 12 dB rise scores 1.000 against its own cohort and 0.000 against the pooled one (T-303's measurement, reproduced in T-333 for a 25 dB bias-tee step). `Unknown` is a third cohort, never `Off`: it pools with other unknowns and with nothing else. **Carrier laid (T-332):** `FrameInput::bias_tee` → `FrontEndState::bias_tee` → `ProvenanceSummary::bias_tee`/`bias_tee_mixed` (tile format 5; v1–v4 read `unknown`), and a switch is now a `bias-tee` provenance step in the report. Unlike the chain this is per-measurement, not per-run — a bias tee is switched during a run. **End to end (T-359):** `VisitSample::bias_tee` comes from `grid.provenance` beside `dominant_gain_key()`, `OccupancyStat::bias_tee` is the state **every** visit of the row's window agreed on (a mixed grid, or visits that disagree, give `unknown` — a majority rule would fold a two-chain measurement into a pure cohort), and `from_occupancy_stat` returns it, so folds carry real values. A row is `unknown` when its source cannot report (replay, synthetic), when it straddles a switch, or when it predates T-359; the field is omitted on the wire while unknown, like `calibration`. The first `off`/`on` fold at a site therefore starts an immature baseline: alarms there are suppressed and a report reads `immature` for the first 24 h of observed time, which is the price of not comparing across front ends.
