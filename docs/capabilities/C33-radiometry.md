...
- **What the 8-bit front end costs, concretely:** every radiometric product this capability produces is bounded by an 8-bit ADC with no preselector and no factory NF, not a lab radiometer. Consequences to state per use case rather than imply away: (1) dynamic range is limited (`marginal-8bit`), so a real transit/absorption/line signal competes with quantization noise at low SNR; (2) with no preselector, in-band IMD and out-of-band overload raise total power and are indistinguishable from sky signal without a band filter; (3) `K(f, G, T)` gain drifts with temperature over the long integrations this capability needs, and that drift, not the target signal, dominates long-run error unless cal is cycled; (4) accessory gain (LNA, LNB) adds its own drift and noise contribution that the front end cannot calibrate out. Every C33 use case therefore ships as a **relative, site-calibrated estimate** (against a quiet-day curve, a sky model fit, or a same-day baseline) rather than an absolute, metrology-grade measurement — record that explicitly rather than let "calibrated" or "measure NF" imply lab precision.
