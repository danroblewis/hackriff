...
**Guard built (T-568, 2026-09-24).** `acceptance_mauto::mauto_negatives` runs every population blind through the mock SDR (N1 `noise_floor_rise` with `step_db=0, n_weak_signals=0`; N2 `tone` at 0.4 s, `nbfm_voice`, `am_voice`; N3 T-623's three structures; N4 the 2026-09-13 433 MHz quiet window; N5 both `mismatched_hypothesis` shapes), asks `POST /api/analyze {emitter_id}` about every emitter the run produced, and holds the answers to the table above: **0 false labels over N1 ∪ N2 ∪ N4 ∪ N5, 0 `solved` on N3 (scored apart)**, N1 0 emitters, a spur never ≥ `demodulated`, N5 judged by `classify_outcome`. CI n is 3 seeds per synthetic sub-population less the sealed fifth (first run: 18 pooled jobs, 0 false labels, rule-of-three bound 0.167); the full n is T-576's. **On this build every answer is `not-searched`**: no production backend evaluates stages over IQ (region jobs end `no_evaluator`), so the must-return column is armed but cannot yet be reached, and the NEG plane of the coverage manifest says so, with `n4-terminator-50ohm` declared SKIPPED against T-375. An over-claiming stub engine turns the suite red in every population (§6.2).
