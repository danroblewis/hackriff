...
- **Inventory lifecycle (T-078):** `lifecycle_state` is `candidate` / `confirmed` / `deleted`, with an append-only history (`emitter_lifecycle`: new and previous state, author `auto` or `user`, actor = rule id such as `hk-pipeline/confirm@1` or the API token fingerprint, reason, `t`).
  ...
  - **`ConfirmPolicy.synthesized`** (*T-860, MAUTO M-9; [ADR-0015](adr/0015-decoder-synthesis-contracts.md) §5.5 as derived by [ADR-0022](adr/0022-false-confirm-budget.md) §6*; actor `hk-pipeline/confirm-synth@2`) is a third route, for a region-analyze job whose rank-1 pipeline **solved on hold-out**: fields `enabled`, `min_analytic_holdout_bits` (24), `hard_check_floor_bits` (16), `min_check_width` (16, measured by T-577 and **re-measured by T-921** on the shipped `CheckTally` count, which keeps it at 16 — ADR-0022 §4.3.2: the count closed the init-cancel and shift artefacts, including the searched half, but a frame holding two different counted bursts, `x^a·A + x^b·B`, is valid by construction and costs 0.31–84 × the budget at w = 8 against 0.0013 × at 16; never below 8), `assumed_decisions_per_week` (20 000, the budget's denominator, monitored — below), `require_null_control_when_searched` (true), `max_suspect_detection_fraction` (0.5), `forbid_overload_in_window` (true). It confirms a candidate only when the profile is not `quick`, the check is ≥ 16 bits wide, `check_bits = width × differences − L_check ≥ 16` over frames valid without FEC correction, analytic hold-out bits (each stage net of its own look-elsewhere) ≥ 24, a **searched** check's null control ran and passed, and the analysed window has ≥ 1 detection of the emitter with ≤ 50 % suspect and the IQ known not overloaded. `min_evidence_bits` and `min_distinct_valid` do not exist (ADR-0022 §6: wrong currency; the frame count is the per-job formula `max(1, ⌈(24 + L_check)/width⌉)`). Synthesized decodes (`decoder_id` `synth:…`) never count toward the identity route above. The lifecycle reason carries the arithmetic. `differences` is ADR-0022 §4.3.1's count, never `distinct_valid`: frames valid without FEC correction that are not degenerate (over the bits the check **covers** — `span.start_bit` to the end of the check field, not the whole frame, *T-928* — short-periodic whole or with up to w register bits trimmed from either end: the init-cancel frame), counted once per zero-trimmed polynomial up to multiples and divisors (shifts, doubled copies and repeats of one burst are one trial), capped at the GF(2) affine rank + 1 per frame length, and read off the same check block as the width; only `check_distinct_valid`, `sync_excess`, `field_fit` and `identity_recurrence` pay toward the analytic bits, and the job-total `coverage.look_elsewhere_bits` is never an input (*T-575*). **Laundering rule** (ADR-0022 §5.1): a template document (`hackriff.template/1`) whose `provenance.kind` is `discovered` carries `provenance.discovery_look_elsewhere_bits` — the `L_check` its discovering search spent, recorded at save-as-template and **never null** (the loader refuses one without it, and refuses it on a `builtin`/`user` template) — and every later use inherits it inside `L_check`, as a searched check; the gate refuses a row whose `L_check` falls short of its inherited charge. **Decision-rate counter** (ADR-0022 §8, *T-575*): every evaluation of the rule is recorded (table `confirm_decision`: `rule`, `t` on the device's wall clock, in the attach transaction; rows older than seven days pruned on insert), and each decision carries `decision_rate: {decisions_7d, assumed_per_week, budget_claim: holds | void}` — `void` strictly above `assumed_decisions_per_week`, with a sentence appended to the reason. The thresholds never move with the count. **Configuration can only tighten the rule** (*T-575 × T-884*): the configured clause is applied through `SynthesizedConfirm::effective`, which clamps any value on the loose side of ADR-0022 §6 to the floor (width 16, 24 analytic bits, 16 check bits, null control required, no overload, ≤ 50 % suspect, ≤ 20 000 assumed decisions a week) and keeps stricter ones; the width floor moves to 8 only through ADR-0022 §4.3.1's re-measurement, never through config.
...
- **One entry per physical emitter (T-082):** a decoder or chain output of an emission the tracker also followed (RDS PI on a WFM track, the blind framer's signature on an FSK sensor track) is linked to that track's entry instead of staying a second one.
  ...
  - **A decode attaches to the region it came from (T-961)** — before a second entry exists, not after. A decoded identity nobody holds, offered with no emitter context (a recipe run on a band or a selection resolves none), resolves to the live, listed, **unidentified** entry whose centre is within the same centre tolerance of the decode's channel and one of whose observations covers the decode's own time; closest centre wins, then the most recently seen. It is restricted to schemes that do not share a channel (today `rds-pi`), and refused for a decode that names no channel width. Nothing matching still creates an entry. This is why 106.1 MHz no longer reads as an `rds-pi:1323` row beside an `unknown` detection row: the CRC-valid decode names, confirms and classifies the detection it was made from, and its family is written at the **decoder** arbitration rank (ADR-0016 §2 rank 1), so it supersedes the classifier's `unknown` rather than tying with it.
...
- **Synthesis (`emitter_synthesis`, *T-546; job rows T-860*, ADR-0015 §5.4):** an append-only history of analyses — `emitter_id`, provenance `synthesized by output analysis`, `engine`, `t`, `verdict`, `stage_reached`, the chosen `pipeline`, per-stage `evidence`, the `trace`, a `resolution` whenever the verdict is below `solved` (never `not-searched`: a row *is* a finished search), the receiver fit, and — on a row a region-analyze job attached — `job`: `job_id`, `profile`, `evidence_bits` (rank key), `prior_bits`, `analytic_holdout_bits` (the confirm key), `template`, the rank-1 `recipe` inline (≤ 64 KiB) with its `recipe_hash` (`sha256:…`), `check`, the hold-out evidence, `trace_summary`, `replay_key`, `null_control`, the `sealed_resolution`, `decodes_stored`/`decodes_valid` and the `confirm` decision. **An analysis never overwrites the emitter's measured values.** `/api/inventory` rows carry the latest row summarised (`synthesis`, `null` = not searched) and whether the identity rests only on synthesized decodes (`identity_synthesized`).
  - **The `Resolution` (*T-567*, [ADR-0021](adr/0021-search-trace-and-negative-result.md) §7A):** what the *absence* of an identification means, and a first-class finding rather than an empty field. `kind` ∈ `unknown` · `structured-unidentified` (framed, check-valid, unidentified — a result, and it may confirm the emitter) · `unsupported-structure` · `not-searched`, with `deepest_verdict`, the closed `reason` (§7A.3), and `summary`. **`not-searched` is not `unknown`** (§7A.4): an emitter with no row reads `not-searched` and so does a **cancelled or failed** job, because an aborted look ruled nothing out; only a *finished* job may write `unknown`. A row is a finished search, so a stored row can never say `not-searched` (validated). An `unsupported-structure` row — and only that kind — carries `suspected: {structure, missing_block}`, the stable block id §9.4's backlog counts; the full sealed object in `job.sealed_resolution` adds who suspected it and its posterior, the decode-side `coverage` map (per-family tried/deferred/unsupported: the decode-side grey), the §8.2 `null_control` record **whether or not it fired**, `ruled_out` (the named templates this look eliminated), and §10's `retry` policy — whether more budget would buy anything, what would have to change first, and the 1 h floor before any automatic re-analysis. It is **sealed by `hk-synth`** before any context lookup can run (§9.3), and `explanations[]` is attached afterwards and modifies nothing in it.
...
- **The silence the gap is compared against is *observed* silence, never the wall clock** (*T-940, staging 2026-09-25: every on-air FM station, the Confirmed 101.3 MHz one included, read `ended`*). An open track's row was refreshed only by the 5 s live offer (and a chain-bound track had no track row at all), so under the 1 s gap `now − t_end` read nearly every station as ended; and time the receiver spent tuned elsewhere counted as quiet. Two measurements now decide it: (1) while the pipeline still follows a row's track, the row carries the tracker's own **observed** silence since `t_end` (`emitter_observation.live_silence_ns`, migration 0021; `0` while a burst is in flight, counted only while the tuned window contained the track), refreshed every writer pass, and nothing after that report is silence — **unreported is not quiet**; (2) once every source is closed, only the part of `[t_end, now]` the receiver was **watching that band** counts (`hk_model::Watched`, the same IQ-ring tune journal the gap is measured from; before the journal's oldest record, elapsed time — unknown is neither quiet nor on the air). Observed silence is compared with the continuous floor (`IdleGap::continuous`), never the `2 × revisit` gap, which already discounts unwatched time — using both would double-discount (a swept burst would stay open ~2·P²/d instead of ~2·P). It is a measurement, not a stored decision: the gap is still a parameter of the reading. Joining is unchanged, so an unobserved stretch is never claimed as air.
...
- **Storage:** `emitter_observation`; ADR-0017's migration 0012 adds `idx_emitter_observation_time (emitter_id, t_start, t_end)` and nothing else, and T-940's migration 0021 adds `live_silence_ns` (the tracker's observed silence on a still-followed track — a measurement, `NULL` once the source closes) — **no `closed_at` column, no persisted decay score** (a mutable score on `emitter` would be a second `count` waiting to happen; if one must persist, it goes in an append-only table like `emitter_lifecycle`).
