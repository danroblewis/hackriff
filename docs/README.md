...
| — | [Tutorial 2: POCSAG from blocks](tutorials/02-pocsag.md) | — | **Engineering (T-095, M1; blocked.)** The POCSAG recipe (`fsk_demod`→`clock_recovery`→`sync_search`→`bch`→`assemble`→`follow_hops`→`fields`) for a multi-channel pager net; code compiles/lints and the recipe/field-map wiring is verified, but blind detection does not yet resolve the synthetic net's simultaneous close-packed channels, so the acceptance test is `#[ignore]`d pending a follow-up (`docs/tasks.yaml` T-095). |
