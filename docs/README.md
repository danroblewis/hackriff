...
| — | [Tutorial 4: ADS-B from blocks](tutorials/04-adsb.md) | — | **Engineering (T-097, M1).** The ADS-B recipe (`ppm_demod` → `crc` → `fields`) stage by stage; finding the 1090 MHz band blind (bursts don't form an inventory emitter, unlike RDS's carrier); blind mock-SDR acceptance and readsb agreement; the T-097 investigation into a content-dependent `ppm_demod` decode-margin gap (not fixed, filed as a follow-up). Live HIL blocked on a 1090 MHz antenna (T-025). |
