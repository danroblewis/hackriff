...
### `GET /ws/changes` — the versioned change feed: the server says what changed (T-1065)
...
Contract: `hk-cli/tests/api_contract.rs::change_feed_and_control_state_validator_answer_as_documented` (a real write over the mock SDR → exactly one `changed` per route it wrote); the feed itself, including 30 s of idleness producing nothing, in `hk-api/tests/changes_feed.rs`.
...
| GET | `/ws/changes` | token (`?token=`) | The **versioned change feed**: `changed {route, version}` instead of a client poll — see [its section](#get-wschanges--the-versioned-change-feed-the-server-says-what-changed-t-1065) (T-1065) |
