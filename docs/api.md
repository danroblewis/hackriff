...
- **A receiver artefact is explained as one — but never over a measurement.** A row the receiver made carries `service: "receiver-artefact"`, label `Receiver artefact`, with an `evidence[]` entry of kind `artefact` naming what said so. Three ways a row earns it: a standing `artifact-of` relation; a majority of its newest linked detections explained by a mechanism **measured against something** (an IQ image, an intermodulation product, a measured spur-mask entry, a retune-settled LO-relative verdict); or a majority explained by a frequency **coincidence** with one of the receiver's own numbers (DC/LO leakage, a reference or clock harmonic, a comb tooth) **and only where the LO moved and the mark followed it** — flagged at every tuning centre the row was measured under, spanning at least 100 kHz. A coincidence alone is never enough, because a dwell centres the radio on what it is listening to: every real signal being demodulated is "at the tuned centre", and a channel plan is a comb. Four further qualifications a client can rely on: `clipped` and `compressed` are **not** counted at all (they say the measurement could not be trusted, not that the receiver invented the signal, so a strong station that clipped the ADC is never relabelled); a row that *does* carry demodulator, decoder or classifier evidence keeps that evidence at rank 1 and gets the artefact only as a ranked alternative below it; the verdict is **derived on every read from the current detections and never stored**, so a line that stops being flagged stops being an artefact; and only a row with no such evidence gets the artefact as its single explanation, because nothing was on the air to allocate.
