  ...
  "entries": [
    {
      ...
      "identity_value": "A1B2",
      "identity_label": "KROQ", "identity_label_share": 0.75, "identity_label_meaning": "vote-share",
      ...
      "resolution": { "kind": "not-searched", "reason": null, "t": null,
                      "profile": null, "withheld": false }   // T-566
    }
  ],
...
**`identity_label` / `identity_label_share` / `identity_label_meaning` (T-967; declared contract T-1017).** The decoded identity in the form a list row wants to show beside the bare `identity_value` (an RDS PI is a 4-hex-digit code, unreadable at a glance). **The label is DECLARED by the decoder, never inferred from a field's name.** A decoder — a built-in chain, a recipe (`decode.identity_label`) or a plugin manifest (`output.identity_label`, `docs/stream-contract.md` §9.1) — states which of its output fields is the identity's human-readable label, which field carries the label's confidence, and **what that confidence means**. A decoder that declares nothing is served `null` for all three, however its fields are spelled: pluggable decoders are consumers, not privileged insiders (CLAUDE.md), and the first T-967 implementation's key-name guessing (`ps`/`callsign`/`station`, `pi_share`/`share`) was an accidental plugin-output contract in which any plugin's `station` field became an identity label. Declarations are stored beside the decodes they describe (`identity_label_decl`), so a database serves the same label after the pipeline that decoded it has gone; a decoder's set is **reconciled** whenever it loads, so a manifest or recipe edited to drop its declaration stops labelling its rows instead of leaving the old declaration in force. `identity_label_meaning` is one of `vote-share`, `crc-valid-rate`, `decoder-score` and is `null` exactly when `identity_label_share` is — a bare 0–1 number beside a name is not self-describing, and the UI words the figure from the declared meaning (`75% of frames` for a vote share) rather than assuming one.

For the built-in RDS chain the declaration is `hk-rds` / `rds-pi` → label `ps`, confidence = the vote share of that label in the `ps_frames` table: the **stable, voted label of the identity's latest declared row in the window — the most frequent label, not the latest fragment**, because a station scrolling song/artist text through PS sends many fragments and one of them is always the newest. The share is deliberately not `pi_share`, which is the PI vote and says nothing about the name. Where two decoders declare labels for one identity the **newest declared row in the window wins**, by capture time, never by declaration order.

**The label is scoped to the view's own moment, as an upper bound and only an upper bound** (T-1017). The inventory is time-scoped to the view (CLAUDE.md), so a pane scrubbed into the past must not be told a name that was **decoded after** the moment it is looking at. That moment is the request's `t1` when it named a window and `at` (the caller's own live edge) when it did not — which is what scopes the unwindowed **Confirmed** list too. There is deliberately **no lower bound**: a decoder's summary row is stamped with the **start** of the session it summarises, and an identity learned earlier stays known while the emission is still on air (one interval `[start, end?]`, ongoing until an end is detected — ADR-0017/0019). A window inside a long RDS session, and any live-following pane whose window has moved past that session's start, therefore still show the station's name; bounding below would have stripped the name off a station that is still transmitting. A request that names neither `t0`/`t1` nor `at` asks about now and gets the latest of all time. All three fields are also `null` when no declared row at or before that moment holds a label, on a withheld-identity row, on a row whose own class hides its metadata, and on a row with no decoded identity, exactly as `estimated_params`/`cluster_id` are (T-159/T-163): the field can never confirm a withheld identity by appearing — including the case of a restricted, metadata-only decode naming the same identity as an unrestricted summary row, which withholds the label with the identity. Cost: one identity-class check and one indexed row per declaration per list entry, with the declarations read **once per answer**. No parsing happens in the UI (CLAUDE.md thin-client rule).
...
**`explanations` require measured support, and rank 1 is never a bare allocation (T-990).** Each entry is `{rank, service, label, score, evidence_confidence, status_evidence_confidence, status, prior_ref, flags[], evidence[]}`, best first. Three rules a client can rely on:

- **A band-plan allocation alone is never the top explanation.** A candidate with no demodulator, decoder or classifier evidence (`evidence_confidence == 0`, flag `allocation-only`) is a statement about the *band*, not about the emission — an empty channel gets the same one. When nothing about a row rests on signal evidence, rank 1 is `service: "unidentified"`, label `Unidentified emission`, flag `no-measured-support`, and the allocations rank under it as what the band plan expects here. This is the finding from an empty airband where 52 noise rows in 8 minutes were each topped by "Aviation voice (VHF AM)".
- **Every candidate discloses what the measurement says about it.** An `evidence[]` entry of kind `occupancy` carries `{bandwidth_hz, duty_cycle?, expected_obw_hz?, support, reason}`, where `support` is `measurement-supports`, `measurement-contradicts`, `measurement-not-discriminating` (the service has no bandwidth signature — amateur, Part 15) or `measurement-unavailable`. An allocation-only candidate whose occupancy contradicts the service is flagged `measurement-contradicts` and discounted: 230 kHz of occupied bandwidth is not a 25 kHz aviation voice channel however clearly the band is allocated to one.
- **A receiver artefact is explained as one — but only where the mechanism was measured.** A row the receiver made carries `service: "receiver-artefact"`, label `Receiver artefact`, with an `evidence[]` entry of kind `artefact` naming what said so, and it is the row's only explanation — no service is offered, because nothing was on the air to allocate. Two ways in: a standing `artifact-of` relation, or a majority of the row's newest linked detections explained by an **established** mechanism (an IQ image, an intermodulation product, a hit in a *measured* spur mask, a retune-settled LO-relative verdict).
- **A bare frequency coincidence is said, not claimed.** DC at the tuned centre, `n × 10 MHz`, `n × fs` and comb teeth are *not* verdicts: 120.000 MHz is a 10 MHz multiple and a valid 25 kHz airband channel, and a dwell centres the radio on what it is listening to, so a real emission being demodulated sits at the tuned centre too. Where a majority of a row's detections carry one, the rank-1 `unidentified` row gains the flag `receiver-coincidence:<mechanism>` and an `evidence[]` entry of kind `artefact` giving the mechanism and the count — and the band-plan allocations stay ranked below it. Deciding such a row belongs to the receiver is the admission-time DC/spur mask's job (T-948); a row that mask lists carries `spur-map`, which is established evidence and does reach the verdict above.
- **Three qualifications on the verdict.** `clipped` and `compressed` are **not** counted at all (they say the measurement could not be trusted, not that the receiver invented the signal, so a strong station that clipped the ADC is never relabelled); a row that carries demodulator, decoder or classifier evidence keeps that evidence at rank 1 and gets the artefact only as a ranked alternative below it; and the verdict is **derived on every read from the current detections and never stored**, so a line that stops being flagged stops being an artefact.
- **The `unidentified` row carries the raster verdict it displaced.** A client reads `explanations[0]` for the off-raster chip and the channel-raster readout, so when `unidentified` takes rank 1 it copies the `off-raster` flag and the `raster` evidence entry of the best-ranked candidate below it. A centre off its channel raster is flagged, never dropped and never snapped.

Neither `unidentified` nor `receiver-artefact` ever sets `known_status`; only a candidate with non-shape evidence does, exactly as before.
...
- **Scope.** With `t0`/`t1`, the window is exactly that range and `open` is derived against `t1` — a caller's `t1` *is* its own live edge — so scrubbing back re-derives the liveness a row had at that time rather than marking every past window `ended`. Without `t0`/`t1` the window is all of time up to now, against the wall clock: Explore's Confirmed list is deliberately not time-filtered and still has to render liveness. Interval closure uses the idle gap **measured** off the band's tune history (T-410: `1 s` for a band the receiver never looked away from, `2 ×` the revisit period for a combed one, the conservative `60 s` only when nothing recorded whether it looked), and **the silence compared against it is observed silence, never the wall clock** (T-940, ADR-0019 §3): for a row whose track the pipeline is still following, the silence the tracker *reported* since the row's measured `t_end` (`0` while the burst is in flight) — time since the last report is time the detector has not yet analysed, and unreported is not quiet; for a closed row, only the part of `[t_end, t1]` the receiver was **watching this band** in the IQ ring's tune journal (before the journal's oldest record, elapsed time). Observed silence is judged against the continuous `1 s` floor, not the revisit-scaled gap: that gap exists to discount unwatched time, which observed silence has already excluded, so using both would double-discount and hold a stopped burst open for minutes under a sweep (≈ two silent visits is the end, as before). So an on-air station reads `live` between sightings, and a station the receiver has retuned away from is unobserved, not ended. An END found this way is still provisional and revocable (§ `revoked_s`).
...
**`relation` and `relations=` (T-219, C40).** One physical signal can produce several overlapping inventory rows, and a receiver can manufacture a row out of thin air. `relation` says why a row **defers to another**, or is `null` (the normal case): `{"kind", "artifact", "source_id", "author", "actor", "t_s", "reason", "score", "detail"}`, where `kind` is `suppressed-by` (the row overlaps a **Confirmed** entry's band by at least 60 % of **both** the narrower and the wider band, with nothing to tell the two apart — so a narrow emission sitting inside a wide one is never hidden by it), `duplicate-of` (the weaker of two overlapping candidates, ranked by a provisional SNR × duty × trust proxy — `score` — until decode evidence in bits exists), or `artifact-of` (a receiver artifact, with `artifact` ∈ `image` / `harmonic` / `intermod` and `detail` carrying the arithmetic: `n`, `a`, `b`, the tuning centre used, `predicted_hz`, `error_hz`, `tolerance_hz`, `suppression_db`, and (T-307) `receive_chain: {device_id, antenna_port}` — the front end the claim rests on, since T-302 gates every image/harmonic/intermod pairing on the chain and an artifact is a property of **one** receive chain, never a universal claim). `retune-sibling-of` (T-598: the **same LO-relative receiver artefact** as the row it names, seen from a different tuning centre, with `detail` carrying `slope` (`lo-locked` / `image`), `slope_factor`, `invariant_hz` — the coordinate `f − slope·f_LO` both sightings sit on — `centres`, `spread_hz` and `los_hz`; unlike every other kind the two bands do **not** overlap, because an LO-relative line lands at a different absolute frequency at each centre, which is exactly why one artefact was showing up as N emitters). `multipath-of` (T-222, C40 content half: the **same emission as the row it names, arriving over a second path** — the same content, delayed and attenuated — with `score` the peak correlation and `detail` carrying `content_kind` (`envelope` / `audio` / `bits` / `frames`), `delay_s` and `delay_resolution_s`, `path_difference_m` and `path_difference_resolution_m` (`c · delay`, the **reflection reading** of the delay: a relay, a translator or a re-broadcast of one programme measures the same, so the delay is the finding and the distance its interpretation), `correlation`, `dominance` (how far the peak stood above the best competing lag), `support` of `support_segments` (how many **independent parts of the shared window** each showed the alignment on their own — a peak says the series match and says nothing about whether the match was earned, and one coincidence correlates 1.00), `self_similarity` (the strongest repeat either series shows at a lag **beyond** the searched range — a series that repeats every P has a lag ambiguous modulo P, and `dominance` is structurally blind to any period longer than 150 ms), `attenuation_db`, `overlap_s`, `separation_hz` and `identity` (`same` / `unknown`). Like `retune-sibling-of` the two bands do **not** overlap — overlapping rows are the T-219 rules' business — and unlike every artifact kind it is **not** gated on a receive chain, because a reflection happens in the air rather than in one front end's mixer. Guards, all a priori: two different decoded identities end it outright; the correlation must reach 0.75 and stand 1.5x above its runner-up; it must be **earned by at least 3 independent parts of the window, and at least half of them** (two identical-model sensors that each key once 50 ms apart correlate a perfect 1.00, and relating them would hide a real emission); **neither series may repeat itself** past the searched lag range (a 0.5 s keying cadence makes the delay ambiguous by whole periods, and the dominance test cannot see it); the delay must exceed twice the measurement's own time resolution (below that a second path cannot be told from a second transmitter) and stay under 150 ms; and the **later** copy must be the weaker one, by 0-40 dB. Anything short of that claims nothing rather than guessing. Rules and thresholds: `hk_model::multipath`; correlation: `hk_dsp::xcorr`.) `reason` is backend-rendered and never names an identity. Rows with a standing relation are **hidden by default** and listed with `relations=all`; the default is `relations=shown`, and an unknown value is `400 invalid`.
...
**`classification` / `latest_classification` (T-211, ADR-0016 §2).** `classification` is the classification that sets `family`: the lowest **arbitration rank** (`arb_rank` 0 user > 1 decoder > 2 lock-verified > 3 classifier > 4 track shape), latest among equals, so `family`, `classification.family` and the `family` filter always agree. `latest_classification` is the most recently appended row **that differs from `classification`** (e.g. a later rank-3 `unknown` under a rank-2 lock-verified label), else `null`. It is the latest row *unlike* the current one, not simply the latest (T-886): on the rank-3 tie the pipeline appends the C15 posterior and then restates the chain's own label after it so the family stays with the chain, and reading only the newest row served `null` for exactly the emitters the classifier had measured. Both have the same shape, or are `null` when the emitter has no classification:
...
**`resolution` (T-566, ADR-0021 §7A.2/§7A.4).** The row's **decode-side resolution**, present on every row: `{"kind", "reason", "t", "profile", "withheld"}`. *Not-yet-analysed and analysed-and-found-nothing are different states, and neither may be rendered as the other* — the decode-side statement of the canvas's grey rule — so an emitter **no analysis has run on** reads `kind: "not-searched"` with `t: null` (a positive state, never a missing field), and a finished search that identified nothing reads its own sealed kind: `unknown` (it looked and found nothing), `structured-unidentified` (framed and check-valid with no identity — a result the inventory renders as one, not as a row hunting for a label) or `unsupported-structure` (the suspicion has no block in this build). `kind: null` means the latest analysis **solved**: no unresolved finding, and `synthesis` carries the pipeline it chose. `reason` is the ADR-0021 §7A.3 enum (`no-signal` · `nothing-scored` · `tied` · `budget-exhausted` · `unsupported-structure`), `t` when that analysis ran and `profile` the depth it ran at (`null` on a row the trunking chain wrote, which had no job). A **withheld-identity** row reads every field `null` with `withheld: true` — the T-159/T-163 rule, and still not `not-searched`, so nothing analysed is rendered as un-looked-at.

**Filtering by it: `?resolution=`.** `not-searched` · `unknown` · `structured-unidentified` · `unsupported-structure`; anything else is `400 invalid` rather than silently ignored. `not-searched` selects an emitter with **no** `emitter_synthesis` row — including one whose last job failed or was cancelled, which attaches nothing, because an aborted look ruled nothing out. A stored row can never itself resolve `not-searched` (`EmitterSynthesis::validate` refuses it: a row *is* a finished search). A solved row matches no value, and a withheld-identity row matches none either — it serves no resolution, exactly as a tag outside the controlled vocabulary never matches a `tag` filter. The filter reads the same stored row the field is served from, so the two can never disagree.
...
`{id}` may be the id of an entity that has since been merged into another (the API resolves to the live emitter). **So the `id` in the answer is the survivor's, not necessarily the one asked for** — on every route here, and on the `entry`/`deleted` row the mutating ones echo back. That is how a caller learns its id was merged away: the answer is `200` with the live row, never a `404` and never a shell under the old id. Because the inventory is self-cleaning and merges near-duplicates continuously (CLAUDE.md, "Detection is fast, continuous and self-cleaning"), an id read from `/api/inventory` can be re-keyed before the next call, so a client holding one **re-reads it from the row the server returns** rather than assuming it is still live (T-972). A merge keeps a **confirmed** entry's id over a candidate's, so promoting is what settles an id. `reason` (optional on the mutating routes) is a free-text string of up to `LIFECYCLE_TEXT_MAX` bytes; promote and delete are audited (`inventory_promote`, `inventory_delete`) with the old/new lifecycle state and the token fingerprint as actor. Errors: `404 not_found` (unknown id, or an entry already deleted), `400 invalid` (unknown body field, bad `reason`, a band breaking the rules below), `503 unavailable` (no inventory store or no audit log).
...
- **`latest`** is the most recently appended row **that differs from `classification`**, else `null` — the same rule as the row's `latest_classification`, including T-886's: a restatement of the current row is skipped, so a classifier posterior recorded beside a chain's label is served here. **This is the normal shape for a demodulated emitter:** ADR-0016 §2 leaves the family to the demodulator chain that locked (rank 2), while the C15 cascade's posterior is recorded at rank 3 beside it, so a client that reads only `classification` would report `null` for an emitter the classifier did measure. Read `latest` first and fall back to `classification`.
  ...
  "decodes": [
    { "decoder": "hk-rds", "recipe_id": null, "frame_model": "rds-pi", "at": 1789300820.5,
      "fields": { "pi": "C0DE", "pty": 10, "tp": false, "ta": false, "ps": "KROQ    ",
                  "pi_votes": 12, "pi_total_votes": 12, "pi_share": 1.0, "pi_provisional": false,
                  "identity_provisional": false, "identity_votes": 12, "identity_votes_needed": 10,
...
One row per `(decoder, frame_model)` pair the emitter's decoded identity has produced, newest first: a plugin decoder (`readsb`, or the built-in `hk-rds`) commits one frame model per row, while a recipe's several `messages` outputs share one `decoder` (`recipe:<id>`, e.g. `rds.recipe.json`'s `group-info`/`station`/`radiotext` outputs) but each names its own `frame_model`, so RDS's PI/group metadata, PS and RadioText each get their own row. `recipe_id` is the id after `recipe:` when `decoder` has that prefix, else `null`. `fields` merges the row's metadata (frame type, addresses, counts — always stored) and content (payload/text, only when the caller's identity access reveals it; content gating is off by default, T-143) into one object, exactly as the decoder/recipe committed them. `crc.valid` is whether the frame's check passed; `source_session` is the producing `Demodulation`'s id, else the replayed `Recording`'s id, else `null` for a live decode with neither. An emitter one content gating withholds answers `{"decodes": []}` — the same lookup never confirms a withheld identity by naming its decodes (T-036). `404 not_found` for an unknown id.

**Provisional identity (T-962).** A decoded identity whose scheme cannot carry it on one frame needs a **vote** before any decoder writes it: an RDS PI becomes an identity only after **10 agreeing CRC-valid groups within 5 s of capture time** (`hk_model::RDS_PI_COMMIT_VOTES`, `RDS_PI_COMMIT_WINDOW_NS`; 11.4 groups/s is under a second of real lock, while a chance lock at one agreeing group per ~15 s never puts 10 in any 5 s however long a pipeline runs — the bar is a rate, measured on the rows' own times, never the wall clock; once reached it stays reached), the same bar for the built-in `hk-rds` decoder and for every recipe `messages` output naming `rds-pi`. Below it the row is still written and served, but **with no identity** (no emitter is created, keyed or confirmed by it) and these fields: `identity_provisional: true`, `identity_votes` (agreeing CRC-valid frames so far — groups for `hk-rds`; for a recipe output, its frames, each at least one group), `identity_votes_needed` (the bar), `identity_votes_in_window` (the most of those votes that fell within one window, counted up to the bar), `identity_votes_window_s` (the window, `5.0`), and — on a recipe row, whose own identity column is empty — `identity_scheme` and `identity_value` (e.g. `"rds-pi"`, `"1704"`). A row whose `identity_votes_in_window` has reached the bar carries `identity_provisional: false` with the same counts; `identity_provisional` is exactly `identity_votes_in_window < identity_votes_needed`, so a row can hold `identity_votes` above the bar and still be provisional. `hk-rds`'s `rds-pi` row also keeps its own `pi_provisional` (the same flag), beside `pi`/`pi_votes`/`pi_total_votes`/`pi_share`. **An emitter with no decoded identity answers the provisional rows linked to it** (a recipe run's target, or the always-on chain's emitter hint), latest per `(decoder, frame_model)` and windowed as above, so the panel can show "PI 1704 (3 groups, provisional)"; with none, `{"decodes": []}`.

**The station's accumulated RDS field view (T-971).** The always-on `wfm-rds` chain no longer decodes one ~4 s window and stops: once that window decoded RDS, the chain keeps demodulating the station — its DDC, pilot PLL and RDS decoder carried on, so nothing is decoded twice — and writes the station's fields **as they stand** onto its emitter as further `hk-rds` / `rds-pi` rows: when a field that matters changes (PI commit, most-sent PS, RadioText, PTY, TP/TA, the AF set; at most one row per 5 s of capture time), as a refresh every 30 s, and once when the follow ends. So the latest `rds-pi` row this route serves is the station's current field view, and a window (`t0`/`t1`) serves the view as it stood then. Besides the fields above every `hk-rds` `rds-pi` row (window or follow) carries: `ps_current` (the latest complete PS frame — stable until a different one completes), `ps_sequence` (a dynamic PS's distinct frames in the order sent, newest 32), `ps_dynamic`, `rt` / `rt_ab` (the latest complete RadioText message, 2A or 2B, and its A/B flag; `null` until a whole message has arrived CRC-valid), `rt_messages` (`[{text, ab}]`, newest 32), `af_mhz` (alternative frequencies heard, 0A method A, ascending) and `ct` (the latest 4A clock time: `{utc_unix_s, offset_half_hours, mjd, hour, minute}`, or `null`). A follow row also carries `follow: {span_start, span_end}` — Unix seconds, the capture time its fields accumulate over (window start to the newest sample) — and is stamped `at = span_end`. Follows are bounded: at most 6 at once and only while their measured cost at the tuned rate fits 1.5 cores together (the first is always admitted), each ending at its spec's `follow_s` (built-in 600 s), when the station's RDS has been silent for 10 s (the emitter ended), or with its stream (retune, gap, overrun, segment stop); `/api/status` counts them under `chains.rds_follow*`. No per-frame row is written for a followed stretch: the field view is the record, not the packets.
...
- **`emitters[]`** lists each emitter with at least one event in the window, once, with its ranked `explanations` — suggestions beside the measurement, never truth (vision step 4). Its `events` and `on_air_s` describe the whole window, not the current page; `count` is the lifetime total, which is exactly where a monotonic counter belongs. Identities are gated as on `/api/inventory` (`identity_value` only when in clear), and each carries the same three **declared identity-label** fields — `identity_label`, `identity_label_share`, `identity_label_meaning` (T-1017, rendered by the same backend code, documented under `/api/inventory` above) — bounded above by the queried window's `t1`, so the catalogue of a period never names a station from a label decoded after it (and, being bounded only above, still names one whose session began before it). An acceptance test asserts the two routes agree field for field on them for every emitter in the window, exactly as it does for `liveness`.
...
```jsonc
{
  "window": { "horizon": "iq-ring", "enabled": true, "reason": null,
              ...
              "buffered": { "t0_s": 1789300890.0, "t1_s": 1789300920.0, "span_s": 30.0,
                            "drops": [ { "at_s": 1789301010.0, "t0_s": 1789300897.5 } ] } },
  ...
  "coverage": {                                   // T-423: record-derived, per drawn cell
    ...
    "sources": [ { "kind": "iq-ring", "spans": 37, "named_spans": 37, "device_known": true, "available": true, "state": "open", "reason": null },
                 { "kind": "observation-log", "spans": 12, "named_spans": 12, "device_known": true, "available": true, "state": "open", "reason": null },
                 { "kind": "open-dwell", "spans": 1, "named_spans": 1, "device_known": true, "available": true, "state": "open", "reason": null } ],
...
- `buffered.drops` (T-845) is the ring's next **whole-slot evictions**, oldest first: when the write head reaches `at_s` the ring's oldest sample jumps to `t0_s`. It is `GET /api/iqbuffer`'s `drops` renamed to this route's `_s` convention. A byte-full ring drops a slot at a time (a sixteenth of the quota: 7.5 s of a 120 s ring), which can put `buffered.t0_s` *ahead* of the retention bound. A client that re-reads this every few seconds can therefore move its IQ horizon past a drop it has not re-polled yet, instead of drawing IQ that was overwritten; the web client honours each drop `DROP_LEAD_S` (2 s) before its `at_s`. `t0_s` is exact. `at_s` is a prediction at the current rate: a gap or retune only makes the real drop later.
...
| Field | Says |
|---|---|
| `oldest_record_s` | The earliest instant **any** consulted source still holds a tune record for — `min` over the IQ ring's buffered start, the observation log's oldest surviving hour and (T-680) the start of any dwell in flight (`sources[].kind == "open-dwell"`, below). `min`, not `max`: a row is knowable if *at least one* record reaches it. `null` when nothing here holds a record. |
| `recording_began_s` | **When this server's memory of recording begins** (T-507): `min` over the IQ ring's buffered start, the start of any dwell in flight (T-680), the observation log's **earliest record** (its first sampled instant — not its oldest hour, which is a filing boundary up to an hour before any sample and is what `oldest_record_s` uses) and the spectrum history's own record of when it began recording (a fact it persists, so it outlives both a restart and the tiles that proved it). `null` when nothing here has ever recorded. Rows wholly before it are `"unobserved"`: before this installation recorded anything, nothing looked. |
...
| `as_of_s` | **How far FORWARD this answer's evidence reaches** (T-532): the newest instant any consulted tune record ends at — over **any** band since T-881, not only this one — clamped into the asked-for window. A record of the radio somewhere else at an instant is evidence that this band was unobserved then, so a band the radio *left* is answered as far as the radio's record reaches, not only to the moment it was left (which left every later row of a straddling tile pending for good, and a later tile with no horizon at all — drawn grey from a held copy). The observation log is consulted for this band only; the IQ ring and the dwells in flight for every band. `null` when no record reaches into the window at all. |
| `rows` | The grid's realised row count, so the band can be read against it. |

**`"unknown"` is what a server recorded and lost, never the default for a young one (T-507).** Until T-507 every row before `oldest_record_s` was `"unknown"`, so a freshly started or reset server painted everything before its first sample in the fourth state — the magenta hatch covered 54 % of a live pane 3 s after a restart and faded only as the window slid past the start. A server that has never recorded has forgotten nothing; its true answer about the time before it started is `"unobserved"`. The fourth state is now exactly three cases: rows between `recording_began_s` and `oldest_record_s` (recording happened, its tune record did not survive — e.g. a short IQ ring evicted it before the observation log's once-a-minute interactive record covered it); every row before `oldest_record_s` when `forgotten` is set; and every row on a server with **no tune history at all** (no IQ ring and no observation log), which cannot say whether it looked. **A ring that has evicted seconds the dwell in flight still covers has lost nothing (T-680)**: that dwell is a tune record the server holds, so its seconds are `"observed"` before the seal exactly as after it. What `recording_began_s` cannot see is a store deleted from disk, or a spectrum history written before T-507 that had already evicted its oldest tiles: its oldest surviving block stands in, a lower bound, which can only widen `"unknown"`, never claim `"unobserved"` over a span that was recorded.
...
**The live edge has a third source, because a record appears only when a dwell *seals*** (T-596). An interactive dwell is written when the tune changes or after 60 s of a steady tune, so for up to a whole dwell after every retune the observation log says nothing about the band the radio is sitting on and measuring. With an IQ ring that gap is covered by the ring journal; **with the ring refused it is covered by nothing**, and T-588 measured the consequence — 18 rows (18 s) of `max_db` served `"unobserved"`, data that exists drawn grey. The ring had been refused *because the disk was full*, which on a portable device is the field failure mode rather than an exotic configuration. So the dwell in flight is served as coverage too, under `sources[].kind == "open-dwell"`. It is **not a weaker claim and carries no mark of its own**: the samples are sampled, the analysis has run over the rows that exist, and only the bookkeeping is outstanding — so it rasterises through the same mapping as a sealed record, declares the same `dc_excluded` notch (`"excluded"`, T-595), and a cell's state does not change when the seal catches up. What is new is the *source*, so a client can see which evidence carried the live edge. It never appears in `GET /api/observations`: a provisional record must not reach the paths that count sealed visits. **And it is one of the records the horizon is computed from (T-680 — a deliberate change of T-596's contract).** T-596 clipped the open dwell to a `horizon.oldest_record_s` computed from sealed sources only, so a provisional record could not paint `"observed"` over rows the same answer calls `"unknown"`. The clip only ever bound **before a seal** — a dwell opens where the previous one closed, so once the log holds any record the open dwell starts after it — and there it contradicted this section's own rule: the open dwell's rows read `"unknown"` until the seal and `"observed"` after it, for samples that never changed. On a young server with a short IQ ring that was a whole band of "recorded, record since lost" over seconds whose record was in memory and under a minute from being written (T-507's contract test asserted exactly that band, and was rewritten with this change); with the ring refused and nothing sealed yet it dropped the open dwell altogether and greyed the live edge, which is the failure this section exists to prevent. So the dwell in flight now counts toward `oldest_record_s` (and `recording_began_s`) like the ring and the log. **It reaches back to its own start and no further**, and the horizon is a `min`, so it can only admit rows it actually covers: a record that was genuinely lost — a seal the log never wrote — lies before the next open dwell and stays `"unknown"`, and the open dwell itself is bounded (replaced every tick, closed on retune and after 60 s of a steady tune). The two horizons are therefore symmetric: the dwell in flight folds into the spans **before** `horizon.as_of_s` is taken (T-532), so `as_of_s` reaches the live edge the plane is drawn from, and into `oldest_record_s` at its own start, so a cell's state does not change when the seal catches up — at either end.
...
**`available: false` never travels alone** (T-920). Each row also carries `state` and `reason`, so a client is never left to guess *which* negative it is: `state` is the IQ ring's own `allocation` (`"allocating"`, `"refused"`, `"locked"`, `"incompatible"`), `"open"` for a source that is contributing, or `null` where the run is simply not configured for one; `reason` is that source's own sentence, the same one `GET /api/iqbuffer` serves. Only `"allocating"` resolves itself, and a client that cannot tell it from `"refused"` cannot tell *wait a moment* from *this device has no ring today* — the same distinction as `"unobserved"` versus `"unknown"` one level down. Both fields are read off status that already exists; nothing new is measured.
...
```jsonc
{
  ...
  "shadow": { "encoding": "column-runs", "runs": 3,
              ...
              "sources": [ { "from": "this-tile", "level": 2, "statement": "…" },
                           { "from": "before-tile", "store": "spectrum-history", "level": 1,
                             "f_cell_hz": 12500.0, "t_cell_s": 60.0, "search": "ladder" } ],
              "edge_s": 1789309800.5, "reach_s": 1789309801.7,
              "search": { "store": "spectrum-history", "before_s": 1789300736.0,
                          ...
                          "own_level": { "store": "view-lattice", "level": 2, "columns_found": 0,
                                         "columns_used": 0, "source_cells": 0, "stages": [],
                                         "record_bound_s": null,
                                         "rule": "…" },
  ...
  "cost": { "build_ms": 11.6, "source_cells": 65536, "chunks": 1,
            ...
            "lock": { "holds": 3, "yielded_ms": 0.0, "hold_ms_total": 9.4, "hold_ms_max": 8.1, "hold_cpu_ms_total": 8.9 },
            "statement": "…" }
}
```
...
- **Past the data edge only as far as the tune record reaches, and only where the radio was not looking (T-881).** `edge_s` is the store's newest **folded** frame, and the fold trails capture. Rows at or after it are covered only up to `reach_s` — this answer's own `coverage.horizon.as_of_s`, or `null` when it reaches no further than `edge_s` — and only at cells the selected coverage plane calls `"unobserved"`: for a departed band that is time the radio spent elsewhere, and the value last seen is exactly as true of it. A column meeting a cell past `edge_s` that is *not* unobserved stops there for good (the radio looked, and its measurement is on its way; an older number must not stand in for it). Nothing at or after `reach_s` is covered: a shadow never paints the future. Before T-881 runs stopped at `edge_s`, and a departed band's newest rows — unobserved, no run — were drawn as THE grey.
- **Where the before-tile value comes from: the last-known LEDGER, read first and never searched (T-1058).** The store keeps, per front end and per finest frequency cell, the newest sample's time, its first sample's time, the tune epoch, and — per time level of the store — the max-hold of the store's own cell holding that newest sample, updated as each row is ingested (a row *is* the newest sample of every cell it reaches). So the value a departed band carries into a later tile is read in O(cells), at the tile's own time cell (the colour its last live row was drawn with — T-911's rule, now exact at every level without reading a tile), **at any zoom and any distance in time**: there is no budget to run out. `shadow.ledger` states what it answered: `store`, `complete`, `columns_known` (the newest sample ends at or before the tile — the value, exactly), `columns_never` / `columns_nothing_before` (nothing ever reached the column / its first sample is at or after the tile — proof only when `complete`, i.e. the ledger has seen every frame its store folded; a ledger begun on older history, or whose file was lost, is not), `columns_later` (seen again at or after the tile's start: the ledger holds only the newest value, so the value *at* the tile is not in it), `columns_answered`, `columns_searched`, `cells_read`, `f_cell_hz` (the ledger's own cell) and `t_cell_s` (the time cell of its max-hold). A run it supplied is labelled `sources[].from = "ledger"` with that `store`, `level`, `f_cell_hz`, `t_cell_s` and **`own_cell`** — `true` when the max-hold is over exactly this tile's cell (the ledger's time cell is the tile's row and a column is a whole number of its cells: T-911's claim, and the client counts it as own-level for T-916's readout), `false` when the tile's row or column is finer than the store's cells, which the client states like any source that did not say. **Only the columns the ledger cannot answer are searched** (`columns_searched`; `search.ran` says whether any search ran and `search.columns` how many columns it was for): the own-level search and the ladder below are kept for exactly those columns — a band looked at again after the viewed time, or an incomplete ledger — as a migration window until the ledger keeps departures too. For every departed band at the live side of history, `search.ran` is `false`, `search.stages` and `search.unsearched` are empty, and the user's "some tiles resolve, not all" cannot arise. The ledger is persisted beside the tiles (`history/s<scheme>/last_known.ledger`, zstd + CRC, rewritten on seal at most every 10 s of data time and on every checkpoint/close) and is **not evicted with tiles**: a band whose tiles the byte budget removed keeps its fog. Its update is measured on the capture thread at **54–85 µs per 3 200-cell row** (a 20 MHz window on the shipped 6.25 kHz schemes, 5 and 8 time levels; ~17–27 ns per cell), **2–4 % of the whole `Pyramid::ingest`** of the same row (`hk-store`'s `ledger_cost_per_row_is_measured`, release build).
- **Where the before-tile value comes from, first: the tile's own level (T-911).** The value a departed band carries into a later tile must be **the cell its last live row was drawn with**, or the shadow changes colour at the first tile boundary after the band is left — which it did: the ladder below answered the view lattice's 586 Hz × 40 ms tiles from a 6.25 kHz × 1 s max-hold, measured 10–15 dB hotter on the departed band's noise floor (up to 26 dB in single columns) than the rows the pane had just drawn. So the search runs first **at the level that answers this tile, in the store that answers it** (the level `resolution.answered` names on the full path; on the coverage short-circuit, the level the full path would read first). When that level is finer in time than the tile, "newest" is decided per **tile row** and every source cell in that row folds by max-hold — the tile's own cell, exactly. The top stage is the tile row holding the newest block's end — the row that drew the band's last cells. It walks newest-first one time block at a time and jumps straight to the newest block holding any tile over this region, by a range scan of the tile index bounded to the last 256 blocks of that level (~11 minutes at the view lattice's finest level; older is the ladder's), so the time a band spent unobserved costs no cell read to cross. **A time block spans far more frequency than a tile (600 kHz at the view lattice's finest level and the default FFT), so after a small retune the new window holds a tile in the departed band's block in every block since (T-1034)** — each was read in full for a band it holds nothing of, the budget ran out ~20 s short of the band's last live row, and the finest tiles over the departed half nearest the new window answered with no run (the user's blank strip between the fog and the tuned window, gone on zoom-out). So the search is also bounded by **the tune record — the same record that decides grey**: per column, the newest instant any radio's record reaches it (a span counts for a column within half a cell of it, for a window's edge bin), plus one tile row; each stage starts no later than the newest such instant over the columns still unresolved, and the time after it is crossed without a read and is not `unsearched` — the record proves it empty. The record is read over the last 15 minutes and makes no claim at all when this server has no tune history, none reaching back into that window, or a source answered with a cut list; `search.own_level.record_bound_s` states the bound (the newest instant over the tile's columns, margin included), `null` when the record made no claim. Each block is read as far as the shared `max_source_cells` budget allows, and a block it cannot afford is listed in `search.unsearched`. It runs only where the level's time cell divides the tile's row: a coarser or misaligned cell would straddle the tile's start, which only the ladder's straddle guard can admit. What it read is `search.own_level` (`store`, `level`, `columns_found`, `columns_used`, `source_cells`, `stages`, `record_bound_s`) — **`null` when no level is affordable for this tile, or its time cell does not divide the tile's row** — and a run it answered is labelled `sources[].search = "own-level"` with that store and level. `search.source_cells` counts both searches, and `search.store` names the ladder's store, or the own-level store when that resolved every column and the ladder never ran.
- **Then the ladder, for the columns the tile's own level does not reach.** A query-time search over the **spectrum-history** pyramid (scheme 1, whose ladder runs seconds → days; a server without one searches the tile's own store at its level 0): newest-first and fine-to-coarse, each stage reading one level over the part of the past the finer stage did not, so the whole retained horizon costs a few hundred rows per column. It stops when every column has a value or nothing older is held. Nothing is maintained for it and capture pays nothing (T-453). `search.stages` lists what was read (empty when the tile's own level resolved every column, and the ladder was not searched), and a run it answered is labelled `sources[].search = "ladder"`. Its cells are the ladder's, so a band departed longer ago than the tile's own level retains carries a max-hold over that coarser box — stated per run, never passed off as the live row's.
- **A ladder-sourced value reads HOTTER than the band's last live row, and the answer says which search found each run (T-916).** `sources[].search` is `"own-level"` or `"ladder"` per run, beside that source's `level`, `f_cell_hz` and `t_cell_s`, so a client can state the resolution it drew rather than presenting two of them as one. The difference is not small: over the stale-departure fixture in `crates/hk-api/tests/tile_cost.rs` the ladder answers a 6.25 kHz × 1 s tile from a **50 kHz × 3600 s** cell, and a max-hold over a box that much larger can only read the same or higher — measured at 10–15 dB (up to 26 dB in single columns) on the mock SDR's departed FM band, which is what T-911 fixed for *recent* departures and cannot fix for old ones. The client's answer is to **say it**: `ui/src/surface/tile.ts` summarises the table per tile, `Surface.render` counts it over the tiles a pane actually drew, and the pane readout carries one clause naming the ladder and the cell (`ui/test/surface-shadow-source.test.ts`). An own-level run gets no clause — it *is* the level the pane already states. A run the answer does not label at all is treated as the coarse case, never as own-level: the claim that says least is "this may be coarser than the row above it".
- **Why the fine level is not simply kept longer for a departed band.** The obvious alternative — retain level-0 cells past the pinned reach so the own-level search always answers — is refused for T-453's reason: retention is paid on the capture thread for every band whether or not anyone looks, and "keep the finest level longer wherever a band departed" grows residency with the number of departed bands, which is every band the radio has ever left. The reach stays bounded (`PINNED_REACH_BLOCKS`, ~11 min at the view lattice's finest level, ~4 h at scheme 1's level 0) and the ladder — which exists precisely because it is time-deep and cheap — answers beyond it, labelled. Honesty here costs one sentence on the readout; the alternative costs the ring.
- **The search is never carried backward, and what was not searched is said.** (The one backward read is the `backward` fill above, which is this tile's own first sample and never a search result.) A coarse cell straddling the tile's start is used for a column only where this tile's own grid proves it held nothing for that column before the cell ended. A window over budget, or whose coarse cell is not folded yet, is listed in `search.unsearched` — including the ladder's whole window when the own-level search left it no budget at all (T-1034; before, that answered silently with nothing) — a column with no run is unobserved in `[searched_from_s, before_s)` **outside** those windows, and nothing is claimed about earlier. Frequency resolution coarsens with age because the ladder is welded; `sources[].f_cell_hz` states it, and a coarser source replicates (T-334's direction).
- **Cost, and the budget's units (T-523).** The whole search — the own-level search and the ladder together, T-911: the ladder gets only what the own-level search left — reads at most `search.max_source_cells` = **`cells` × 512 rows** — the **tile's** scale, not the store's, because the shadow decorates a `cells × cells` grid with at most `cells` column values. Rows rather than an area, so a probe-sized tile reaches as far back as a full one; the ladder is fine-to-coarse, so 512 rows spread over it reach days. That total is a quarter of what `resolution.budget.max_source_cells_per_lock` allows a *single* hold, so no hold this search takes can come near lengthening one (it still takes one per stage/slice, reported as `search.chunks`). It shipped (T-519) with the tile read's own two bounds, which made T-461's coverage short-circuit — the route's *cheapest* answer, where the shadow also runs — one of its most expensive: ~84 ms on an 819 MHz tile on the demo backend, against ~3 ms for the same address before the shadow existed. In a debug-profile test over the departed-band fixture, the same cut takes the 819.2 MHz tile's search from 117.97 ms to 10.70 ms and the worst address of a ten-level zoom burst from 225.18 ms to 13.28 ms, with the same runs per level. Re-measured with T-911's own-level search in front of the ladder (same test, same fixture, a loaded box): source cells read per level 0–9 were 9 216, 131 072 ×4, 53 248, 106 496, 24 576, 49 152, 98 304 — never over the 131 072 budget, which `tests/tile_cost.rs` now asserts on the cells READ — with the 819.2 MHz search at 10.64 ms and the burst's worst at 13.99 ms (level_f 6). **The budget is paid in resolution, never in reach**: a stage the search cannot afford is skipped and the next *coarser* level covers that window, which is why every run states the level that answered it (`sources[].level`, `f_cell_hz`, `t_cell_s`). Over spectrum no tile holds, the search answers from the pyramid's tile index without reading a cell. **Re-measured per SEARCH (T-916), which is the number the own-level search could have doubled.** The same zoom burst now prints the split, and it is the answer the design predicted: at `level_f` 0–4 the own-level search spends everything and the ladder reads **0** cells (9 216 / 131 072 / 131 072 / 131 072 / 131 072 own, 0 ladder); at `level_f` 5–9 no own level divides the tile's row, so the own-level search does not run and the ladder spends everything (53 248, 106 496, 24 576, 49 152, 98 304). Per-level totals are **identical** to the figures above — the own-level search *replaced* the ladder's reads rather than adding to them, at every address of the burst. The one shape where both run is a departure older than the pinned reach, which the burst fixture does not contain: `a_departure_older_than_the_own_level_reach_is_labelled_ladder_and_costs_one_budget` builds it (band recorded, radio 25 600 s away, tile 100 windows later) and measures **5 024 source cells of 131 072 allowed — own-level 0, ladder 5 024**. Zero, because the pinned search crosses the gap by a bounded scan of the tile index and reads no cell when it finds no block: an own-level search that cannot reach costs nothing, and one that can reach replaces the ladder. Measured in `hk-store`'s `last_known_cost_is_bounded_on_a_tuned_and_a_device_wide_viewport` and, per zoom-burst address, in `hk-api`'s `tests/tile_cost.rs`.
...
2. **The level never replicates while one that only folds can answer.** Folding a level whose cells are no larger than the tile's onto the tile's grid can never grey a cell that level holds: the fold is a max and a sum, so an output cell is observed if *any* source cell inside it was. Only the other direction — a source cell **coarser** than the tile's own, which repeats one measured value across output cells — is a claim, and `resolution.fold.<axis>.direction` states it per axis (`exact` / `folded` / `replicated`) and downgrades `source` to `survey-overview`. **Among the levels that fold, the read takes the tile's exact node, else the one with the fewest source cells** (T-1018) — on a store whose coarse nodes are maintained live (the view lattice, T-571/T-585). Max-hold, frame counts and observed seconds compose under folding, so the node answers with the same grid the finest level folds to, for `cells²` source cells in one lock hold instead of up to 16× that: measured through the mock SDR, a `(3, 1)` tile read 1 048 576 level-0 cells in 4 holds before and 65 536 from node `(3, 1)` in 1 after, with the same `max_db` in every cell. `resolution.answered.exact_node` is then `true` wherever the address is a node. **Except at the live edge:** a closed level-0 row folds into the coarse nodes only `seal_lag` (2 s shipped) after the clock leaves it, and the read-time preview covers level 0's open column and each node's in-progress row but not those held-back rows — so a tile (or a `/ws/tiles/rows` block, judged per block when it is read) whose extent reaches into them keeps finest-first and shows every row level 0 holds. The same address takes its node on a later read, once the rows have folded (`hk_store::Pyramid::coarse_lag_start`). The levels that would replicate follow, finest first, and are reached only when every folding level held nothing. A store **without** live coarse nodes (scheme 1's eager ladder, whose open coarse tiles trail the live edge by a producer block) keeps finest-first, so the live edge never waits on a seal.
...
4. **When a level holds nothing the read walks on** through the remaining candidates (T-426's rule, in the direction this route's preference makes meaningful: the byte budget evicts the finest tiles first, §5.5). `resolution.answered.level` is the tier that **actually answered**, `resolution.candidates` every affordable level, in the order the read walks them, and `resolution.tried` every one consulted — a silent fallback would trade one lie for another.
...
- **`cost.lock` is every history-lock hold this request took, timed (T-1021)** — the tile read's chunks, the coverage and address lookups, and the shadow search's steps: `holds`, `hold_ms_total`, `hold_ms_max` (the longest single hold, which is the longest a growing-edge row fold could have waited behind this request) and `hold_cpu_ms_total` (this thread's CPU time inside those holds; the gap to `hold_ms_total` is time the lock was held off-CPU — preempted, or waiting on disk). A cache hit reports the brief lookup hold it took. It is this read's own diagnostic, like `build_ms`, and is nulled before a sealed tile's ETag is computed.
- **Tile reads yield the view lattice's lock to its writer (T-1021).** Before each hold of the view lattice, a tile read waits while the view writer has display rows waiting to fold — at most 50 ms per hold (`TILE_INGEST_YIELD_MAX`), reported in total as `cost.lock.yielded_ms`. Measured with a paced 2.4 Msps run and a 220-tile viewport on four readers: without it the writer lost the lock to tile reads for most of the pass and the lattice's live edge fell ~2 s behind capture on the dev Mac at load ~15 and 14–31 s at load ~33–37; with it, ~0.2 s at load ~12 (baseline ~0.1 s). It costs a read only while rows are actually waiting, and never more than the bound per hold, so a writer that is slow for other reasons delays a tile, never refuses or starves it. `cost.build_ms` includes the wait; `hold_ms_*` do not.
- **`cost.in_flight_limit` is server backpressure**, chosen against `hk-store`'s lock behaviour rather than a browser's connection limit: the history store is behind one mutex, so concurrent tile reads serialise on it anyway and a deeper queue only lengthens the stretch during which ingest competes for it. Over the cap the answer is `503` **naming the cap**, which is what lets a client cancel tiles for a viewport it has left (§5.5's cap (1) is LIFO with viewport cancellation) instead of waiting. **The cap bounds production, not answers (T-581):** a *sealed* tile already held by the server's hot-tile cache is answered `200` even with every slot out, since serving it reads no store — such an answer carries `cost.served_from: "hot-tile-cache"` and an `in_flight_held` that does **not** count it (it holds no slot), so it reports whatever else that client holds, which is `0` for a client with nothing else out. A live tile, or a sealed one not yet cached, still meets the cap. So a steady-state re-poll of an already-drawn screen is never refused behind slow producers.
- **`cost.in_flight_share` is *this client's* cap, and it is the number a client should operate at** (T-630). The server-wide cap is unchanged; what changed is whose request meets it. Measured before this existed: while one tab enumerated a wide viewport it held **all four** slots continuously — it re-asks the instant one frees — so a second tab's *first* request, the one it cannot start without, competed on equal terms with the thousandth request of a tab that is already drawn. The second tab booted in 8.2 s and 11.7 s after 7 refusals, and twice did not boot at all. Raising the cap would only move that failure and would spend capacity the capture thread pays for (T-453), so the cap stays and the **policy** is two rules:
  ...
  - **`cost.in_flight_held` is the number an abandoned read is accounted against** (T-959). It is this client's holdings at the instant the answer was written — this read's own slot included when it took one, and *not* included on a `served_from: "hot-tile-cache"` answer, which takes none. A client cannot compute it: an abort reaches the browser and not `hk-api`, so a walked-away-from read goes on being produced and goes on holding its slot, and the client's own charge for it is a guess at the route's service time (measured 7-9 s for an overview read under load against a mean of a few hundred ms). Subtracting the reads it is still *waiting* for — the one thing the route cannot know — leaves exactly the abandoned reads still in production, so the charge is corrected in both directions instead of estimated. The **`503` states it too**, beside `limit` and `share` (`too many tile reads in flight (limit 4, share 2, held 2)`), because that is what tells the refusal's two causes apart: at `held >= share` a client is refused over its **own** reads (the body says so in words) and backing its cap off would be punishing it for its own slow reads; at `held < share` the slots are another client's and AIMD's multiplicative decrease is exactly right.
...
- **Bounded in bytes AND in entries** — 256 MiB / 600 entries, whichever binds first — so residency does not grow with node count (T-453). Least-recently-*used*, not least-recently-inserted. A body larger than the whole cache is never held. **It is a RAM accelerator, not storage (T-1020/T-1023 ruling):** it holds no data the disk tile pyramid does not, so raising this bound costs RAM only, never durability. It is **sized in viewports**: the tile-latency review (2026-09-25) measured 135-290 tiles per screen against the old 32 MiB bound's ~35-tile capacity (staging: 0 hits / 27 misses on one pan-back); at `planes=compact`'s ~467 kB/tile (T-1019) 256 MiB holds ~560 tiles, past the measured range. On the Jetson Orin Nano target (docs/02) this is under 6% of the smallest (4 GB) module.
...
- **Counters are on `GET /api/status` as `tile_cache`** (`entries`, `bytes`, `max_entries`, `max_bytes`, `hits`, `misses`, `evictions`, `invalidations`, `by_client`) — never in a tile body, where they would change on every read and with them the ETag. **`by_client`** (T-1020) is the same hits/misses split by the caller's declared `client` id (the same identity `/api/tiles?client=`/T-630 uses), e.g. `{"pane-a": {"hits": 12, "misses": 1}}`, bounded to the least-recently-seen 64 client ids the same way the admission share table is — so a pan back over one pane's own just-seen viewport is directly measurable, not only visible in the aggregate counters.

### `GET /api/lastknown` — the fog-of-war's source over a pane's whole frequency window (T-1058, [ADR-0020](adr/0020-last-known-shadow-tier.md) §T-1058)

`?f_lo&f_hi` (Hz, required) `[&cols]` (1–8192, default 256) `[&device]` (one front end; absent or `any` = every one, the newest winning) `[&before]` (Unix s; absent = the newest, whenever) `[&t_cell]` (s: the time cell the max-hold is over; default the store's finest) `[&scheme]` (as `/api/tiles`). One array per column, read from the same **last-known ledger** the tile route's `shadow` reads — no tile read, nothing searched — so a pane can paint fog over its whole frequency window from one response and re-lay it on every frame through its own capture-time mapping, instead of asking per tile.

```json
{ "store": "view-lattice", "device": "any", "region": {"lo_hz": 99.6e6, "hi_hz": 102.0e6},
  "cols": 64, "f_cell_hz": 37500.0, "source_f_cell_hz": 2343.75, "t_cell_s": 0.04,
  "before_s": null, "complete": true, "known": 64,
  "state": [1, 1, …], "states": ["never", "known", "nothing_before", "later"],
  "last_db": [-92.41, …], "last_t_s": [1790413412.36, …], "first_t_s": [1790413396.6, …],
  "epoch": [0, …], "source": ["9c3b…", …], "cells_read": 1024,
  "ledger": {"sources": 1, "cells": 2048, "resident_bytes": 110000, "file_bytes": 9000, "loaded": false},
  "rule": "…" }
```

- **Per column** (`state[i]` into `states`): `known` — `last_db[i]` is the max-hold of the store's `t_cell_s` cell holding the column's newest sample (the value its last row was drawn with), last seen at `last_t_s[i]` (absolute capture time: the end of that sample's finest cell), first seen at `first_t_s[i]`, in tune epoch `epoch[i]` (advances each time that front end's window changes) of front end `source[i]` (its `source_key`, hex); `never` — nothing ever reached the column; `nothing_before` — its first sample (`first_t_s[i]`) is at or after `before_s`; `later` — it was also seen at or after `before_s` (newest at `last_t_s[i]`), and the ledger keeps only the newest value, so the value *at* `before_s` is not here (the tile route searches for it). Absent values are `null`.
- **`never` and `nothing_before` are proof only when `complete`**: the ledger has seen every frame its store folded. A ledger begun on a store that already held history (written before T-1058, or whose file was lost or corrupt) says `false` and knows only what it has seen since.
- **Grey is not decided here.** This is the last-known tier only (ADR-0020); draw it only where `/api/coverage` (or a tile's `coverage`) says `unobserved`.
- `400` for an unknown parameter, a missing or inverted `f_lo`/`f_hi`, `cols` outside 1–8192, or a non-finite `before`/`t_cell`; `404` when the server has no history store.
...
- **Nor before the tune record reaches it** (T-468 review; the tile route's `as_of_s` rule, T-532, applied to a stream). A row is sent once, so its coverage must be final when it goes: a row past the newest tune record would rasterise `unobserved` only because the record has not caught up — with the IQ ring refused (T-588/T-596) the live edge's record is the open dwell, which trails the spectrum by up to a control tick. So the cursor also waits for the record's forward reach over **any** band (a record past a row elsewhere proves the absence here is real). A server that keeps no tune record at all has no such horizon, and every plane there is uniformly `unobserved`.
...
- **The level that answered** is the tile read's own rule — the exact node, else the cheapest level that folds (T-1018, above), walking on only when a level holds nothing (T-426) — stated per block in `answered`.
...
### `GET /ws/changes` — the route-version feed: the server says which route changed (T-1065)

WebSocket; token as for every `/ws/` route (`?token=` on the URL, since a browser cannot set a header on a `WebSocket`). **No query parameters**: there is nothing to subscribe to, because the feed carries every route it covers.

**Not [`/ws/tiles/changes`](#get-wstileschanges--coverage_changed-pushed-on-a-retune-t-1040) above.** That route (T-1040) pushes one *derived* event, `coverage_changed`, about the **tile lattice**: a front end moved, so re-lay the fog and re-fetch the moved tiles. This one is about **HTTP routes**: a counter per route that a write advances, so the panels re-read the handful of bodies that moved instead of polling all twelve. Different questions on different sockets, and a client wants both.

**Why it exists.** Measured in real Chrome over CDP on 2026-09-26: a client following the live edge with **one** pane and nothing moving issued **7.3 requests a second** besides its WebSockets — `/api/outputs` and `/api/pipelines` every second; `/api/control/state` (10 kB), `/api/annotations`, `/api/paths`, `/api/tune-history`, `/api/frontend/events` and the survey coverage every two; `/api/inventory` twice, `/api/timeline` and `/api/recordings` every five. One 24-event trackpad zoom was **264 requests**, each a new TCP connection; the staging tunnel logged 8 622 `Unable to reach the origin service: EOF` in fifteen minutes under them. Freezing the view saved nothing (6.7 req/s) — because **none of those answers changes between polls in the common case**. A poll is a client asking "did it change?" of a server that already knows; this route answers it once, in the direction the knowledge flows.

**The contract.** Each route below has a **version**: a counter, per server process, that the server increments when the state behind that route is written. On connect the socket sends the whole table once; after that it sends one message per route whose version moved, and **nothing at all while nothing changes**.

```jsonc
{ "type": "versions",                      // first, always: every route's version as of the connect
  "routes": { "/api/control/state": 3, "/api/navigation": 1, "/api/coverage": 1,
              "/api/tune-history": 1, "/api/timeline": 0, "/api/recordings": 0,
              "/api/inventory": 0, "/api/annotations": 7, "/api/paths": 0,
              "/api/outputs": 0, "/api/pipelines": 2, "/api/frontend/events": 0 },
  "tick_ms": 250, "t_s": 1790000000.123 }
{ "type": "changed", "route": "/api/control/state", "version": 4, "t_s": 1790000004.567 }
```

- **The route key *is* the path a client fetches**, so nothing has to be mapped: `changed` on `/api/inventory` means re-read `/api/inventory` (with whatever window the client is showing) and file the answer under `version`. The twelve routes are exactly `/api/control/state`, `/api/navigation`, `/api/coverage`, `/api/tune-history`, `/api/timeline`, `/api/recordings`, `/api/inventory`, `/api/annotations`, `/api/paths`, `/api/outputs`, `/api/pipelines`, `/api/frontend/events`.
- **A version is an opaque monotonic number**, not a count of writes to read meaning into, and it is **per process**: a restart resets it, and the client reconnects, takes the new snapshot and re-reads once. `0` means "not written since this server started".
- **Coalesced by construction.** The feed is a *table of versions* that a connection samples every `tick_ms` (250 ms), not a queue of events. Twenty writes inside one tick advance the counter twenty times and produce **one** message carrying the newest version — so a burst can never amplify into a burst of messages, and the ceiling is four messages a second per route however hard the state is written. Nothing is queued per connection, so a slow reader only learns the latest version a little later; it never makes the server hold history for it.
- **An idle feed is silent, and stays open on a WebSocket *ping*** (every 20 s), never on a JSON heartbeat — a heartbeat would be the traffic the feed exists to remove.
- **Refusals** follow the `/ws/open/{name}` convention once upgraded: one `{"type": "refused", "status", "reason"}` and close `4000 + status` — `4503` at the cap of **8** feeds per server (a client needs one). A missing or wrong token is `401` **before** the upgrade; a request that is not a WebSocket upgrade is `426`. Anything the client sends other than a close or a ping is ignored.

**What moves a version — and what does not.** A version moves when a **request writes** the state behind it. The mapping is a pure function (`hk_api::changes::routes_for_write`) applied at the one choke point every mutating control-plane route already passes through, and it is deliberately **generous where a write may or may not have moved a derived route** — a spurious re-read costs one request, a missed change leaves a client showing something that is no longer true, and those are not the same mistake:

| A successful write to | bumps |
|---|---|
| `/api/control/center`, `/rate`, `/window`, `/gains`, `/bias_tee`, `/baseband_filter`, `/scan`, `/scan/stop` | `/api/control/state`, `/api/navigation`, `/api/tune-history`, `/api/coverage` |
| `/api/control/display` | `/api/control/state` (a view change never reached the radio) |
| `/api/control/record/start`, `/record/stop` | `/api/control/state`, `/api/recordings`, `/api/timeline` |
| `/api/outputs/record/start`, `/record/stop` | `/api/outputs` |
| `/api/iqbuffer/clip` | `/api/recordings`, `/api/timeline` |
| `/api/annotations`, `/api/annotations/{id}` | `/api/annotations` |
| `/api/paths…` | `/api/paths` |
| `/api/pipelines…`, `/api/recipes…` | `/api/pipelines` |
| `/api/inventory/{id}…`, `/api/clusters/{id}/promote` | `/api/inventory` |
| `/api/recordings…` | `/api/recordings` |

(Neither `/api/paths` nor `/api/recordings` has a mutating route today — a path is traced by the pipeline, a recording is made by `/api/control/record/*` or `/api/iqbuffer/clip`. The two rows are declared, and tested, so a mutating route added under either prefix later cannot silently skip the feed.)

A refused write bumps nothing (it changed nothing), and a mutating path outside this table bumps nothing at all — it does **not** fall back to "everything".

- **This feed does not report what grows because the radio is running, and a client must still poll slowly for that.** New detections arriving in the inventory, the coverage widening under an already-started sweep, the capture window advancing, a front-end event being judged, a re-plumb moving `run.segment` — those are producer-side events. `hk_api::ChangeFeed::bump` is public precisely so a producer can report them (it is the API T-1040's `coverage_changed` and the pipeline's ingest build on), but **nothing calls it from the capture path yet**, so a route with no producer wired is a route whose *growth* the feed is silent about. Saying so here is the point: a feed that implied it covered the live edge would be the "we have it but didn't render it" bug with the arrow reversed. The waterfall's own rows are already pushed by [`/ws/tiles/rows`](#get-wstilesrows--rows-pushed-to-a-subscription-over-an-address-range-t-468) and are not duplicated here.
- **Cost.** A bump is one relaxed atomic add — no lock, no allocation, no I/O — so it is payable on any thread, including one that gates the ring. A connection costs one thread blocked in `read` with a 250 ms timeout, the same shape as `/ws/tiles/rows`.

Contract: `hk-cli/tests/api_contract.rs::change_feed_and_control_state_validator_answer_as_documented` (a real write over the mock SDR → exactly one `changed` per route it wrote); the feed itself, including 30 s of idleness producing nothing, in `hk-api/tests/version_feed.rs`.
...
| `storage.detection_rows` | number \| null | Per-frame `Detection` rows stored, as of the last count (`null` before the first one) |
| `storage.rollup_rows` | number \| null | Rollup rows (below), as of the same count |
| `storage.detection_rows_counted_s` | number \| null | When those two counts were taken (Unix s). T-913: `count(*)` over `detection` is a full index scan holding a read snapshot — which is what delays a WAL checkpoint — so it is taken on the longer `retention.count_rows_s` cadence (600 s) and immediately after every prune pass, not on every 60 s refresh. The sizes, the oldest row and the watermark beside it are current |
...
| `storage.retention` | object | The policy in force: `enabled`, `max_age_s` (`null` when off), `min_age_s` (600: the floor, ten times the tracker's 60 s idle timeout — a shorter `HK_DETECTION_RETENTION` is clamped up to it), `clamped_from_s` (the age asked for when it was clamped, else `null`), `rollup`, `keep_per_emitter`, `batch`, `interval_s`, `count_rows_s`, `rollup_gap_s`, `rollup_span_s` |
...
|---|---|---|---|---|
| GET | `/api/control/state` | – | `{live, device, tuning, devices, run, scan, scans, display_limits, transmit: {available: false, reason}, audit, routes}` — `devices` (T-511) is every live front end and `scans` (T-1009) every front end's sweep, both `[]` on a replay; `device`/`tuning` are the singular default and are `null` when the run holds more than one | – |
...
| GET | `/api/control/scan[?f_lo_hz&f_hi_hz&dwell_s&step&windows&device_id]` (T-452, `step` T-517, `windows` T-1008, `device_id` T-1009) | – | `{scan, proposed}` — `proposed` prices the named sweep **without starting it**, `null` when none is named; `windows=1` adds each step's slice to both plans (`plan.windows`) | 400 invalid (a range the device cannot reach; `windows` other than `1`/`0`/`true`/`false`), 409 not_live |
| POST | `/api/control/scan` (T-452) | `{"f_lo_hz"?, "f_hi_hz"?, "dwell_s"?, "step"?, "device_id"?}`, or `{"resume": true, "device_id"?}` | `{scan, proposed, device: {commissions, id, commissions_rate_hz}}` **(commissions retunes, and for a coarse step one rate change)**; both plans carry `windows` (T-1008) | 400 invalid, 409 refused (one already running / nothing to resume), 409 not_live |
| POST | `/api/control/scan/stop` (T-452) | `{}` (or empty), `{"device_id"?}` | `{scan}` — **never refused** | 409 not_live |
...
**Since T-817 bookmarks are a compatibility facade** over the reserved `Bookmarks` marker collection ([Marker collections](#marker-collections-t-817-map-17)): a bookmark *is* that collection's frequency-only marker — the same row, the same id — so `/api/bookmarks` and `/api/collections` never disagree. The shapes, validation and audit action names above are unchanged. Rows written before T-817 move into the collection on first use, keeping id, kind, name, note and timestamps.

`tuning`: `{center_hz, sample_rate_hz, gains: {"<stage>": <dB>, …}, bias_tee, baseband_filter_hz}` (`bias_tee` (T-325) is `"unknown"`, `"off"` or `"on"` — three states, never a bool. `"unknown"` means nothing has reported a state: this server has not set one and the device does not have a bias tee. **It must not be read as off** — a bias tee left on into a passive or DC-shorted port is a hardware hazard, and an active antenna's LNA moves the noise floor, so unknown is not a claim that either is safe. Whether the control exists at all is `device.bias_tee`, not this field. `baseband_filter_hz`: `null` without that capability, or before it's been set explicitly — the device's own default is in force). `display`: `{fft_size, averaging, rows_per_s, window}` (`window`: `"hann"`, `"blackman-harris"` or `"flat-top"`; T-067). `run`: `{live, content_class, content_permitted, content_classes, center_hz, sample_rate_hz, segment, replumbing, finished, capture, capture_note, display, recording}` — **`content_classes` (T-991)** is the tuned window's content-class map per sub-band, at the resolution of its allocations: `[{lo_hz, hi_hz, content_class, content_permitted, source}]`, tiling the window in ascending order. A restricted class (`restricted-paging`, `restricted-cellular`) covers only the sub-band of the restricted allocation it comes from (`source` names the 47 CFR section); a band prior (`source` `band prior <name>`: FM broadcast, ADS-B 1090, maritime VHF + AIS, NOAA Weather Radio) is `unrestricted`; anything else is the fail-closed `metadata-only`. `content_class` summarises the map for what covers the whole window (its IQ, spectrum stream and recordings): restricted only when every sub-band is, `unrestricted` only when every sub-band is, otherwise `metadata-only` — so a 152–162 MHz window reads `metadata-only` with four 20 kHz `restricted-paging` sub-bands, never `restricted-paging` whole. A recording carrying its own `hackriff:content_class` reports one entry, `source: "source class"`. `GET /api/status` serves the same map under `control.content_classes`. `segment` increments on every re-plumb (a retune or rate change into a window of another content class) and on every capture restart. **`capture` (T-508)** says whether the front end is delivering samples: `"running"`, `"recovering"` (capture failed — a device read error, e.g. a retune the device refused when it applied it, or a re-plumb that could not complete — and a new segment is being started or has not yet delivered a sample; the live edge is not advancing and that is the true state) or `"ended"` (the run is over; nothing more will arrive). `capture_note` is the cause while `recovering` or after an `ended` that was a failure, and `null` while `running` or after a requested stop or a recording's end. A recovery first re-sends the window the failed segment was built for, then the last window that delivered samples, and gives up after 5 attempts in a row that delivered nothing — so `"ended"` with a note is a front end that is really gone. `center_hz`/`sample_rate_hz` (and `tuning`) follow a recovery: a device that refused a retune and was put back on its previous window reports that window, not the refused one. A client must render `recovering` and `ended` on the surface itself; a frozen edge that looks live is the defect this field exists to prevent. `stats` counts `capture_failures`, `replumb_failures`, `capture_recoveries`, `segments_salvaged` (a re-plumb whose old state a straggling thread still held, completed on a fresh database connection instead of ending the run), **`workers_abandoned` (T-941)** — threads of an earlier segment that had not stopped within the re-plumb's 8 s bound and were left behind. A thread that does not observe its segment's stop is a defect and is served for the same reason `window_settle_timeouts` is; it is **not** a claim that anything was detached, because the run's inventory belongs to the run, so a straggler can delay the next segment's inventory writes and never silence them (before T-941 it could: the inventory was taken from under the straggler and the run continued with a null one, for ever) — and **`worker_panics` (T-541)** — pipeline threads that ended by panicking rather than returning. Always a defect, and served rather than kept internal for the same reason as `window_settle_timeouts`: a fault the system handled is still a fault an operator should be able to see. A panicking thread ends its segment, so it arrives as a `capture_failure` and a recovery (or, if it recurs on every segment, as `"ended"` with the panic named in `capture_note`) rather than as a reader silently missing from a run that still reads `running`. `recording`: `{active, id, label, center_hz, sample_rate_hz, samples, lost_samples, max_s, stored, ended}`.

**`GET /api/control/state` carries a validator: `ETag` + `If-None-Match` → `304` (T-1065).** It is the only route with one besides a sealed `/api/tiles` (T-574), and for the reason the request-volume review measured: **10 kB every two seconds, byte-identical almost every time**. The tag is **content-derived** (a CRC over the uncompressed JSON, so gzip and plain validate under the same tag), and the response is `Cache-Control: private, no-cache` — *not* the `no-store` every other route here sends, because a response nothing may keep can never be revalidated and `If-None-Match` would never be sent. `private, no-cache` is the exact semantics wanted: the browser may keep it, only this client may, and it must revalidate **every** time before reusing it. So a body that really did move (a re-plumb changed `run.segment`, a sweep advanced `scan`) still answers `200` with the new bytes — the validator can never serve a stale state, only skip an identical one — and a `304` carries no body at all. This is the half of the request-volume fix that works for a client that has not adopted [`/ws/changes`](#get-wschanges--the-route-version-feed-the-server-says-which-route-changed-t-1065) yet: the request still happens, the 10 kB does not.
...
**Which radio sweeps (T-1009): one runner per front end, named by `device_id`.** A run holds a sweep *per* front end, because the arbitration below is written about one radio and stays true per radio — a sweep of B has no business yielding to a retune of A. Every scan route therefore takes the same `device_id` selector the six device routes take, and the rules are the ones that selector already has, with one distinction:

- a **read** — `GET /api/control/scan` (the state, and a price) — answers for the run's **default** front end when the selector is omitted, and its own `scan.device_id` says which radio that was;
- a **start** *commissions* a radio for hundreds of retunes, so omitting the selector on a run holding more than one sweepable front end is `400 device_required`, never a guess at whichever was composed first — the same rule as "a client must name a device to move one";
- a **stop** must never be refusable, so an omitted selector there stops **every** sweep ("stop" naming no radio can only mean all of them) and a named one stops exactly that radio's;
- a `device_id` this run does not hold is `404 unknown_device` on every one of them, listing the ids that do resolve — never the default radio as a fallback.

`GET /api/control/state` carries `scans`: one entry per sweepable front end, each the same object as `scan` and each naming its own `device_id`. `scan` stays the **default** front end's — what a single-SDR run has always meant by it — so an existing client is unchanged, and `scans` is the enumeration a multi-SDR client picks from, exactly as `devices` is beside `device`. The map's Measure box is the caller this exists for: *"Scan this region with &lt;device&gt;"* (T-1009) is the user picking which radio sweeps the region they drew.
...
- `scan`: `{state: "idle"|"running"|"yielded", available, unavailable_reason, device_id, plan, budget, progress, yielded}` — `device_id` (T-1008) is the front end this runner sweeps (the run's default one; `null` when the source reports no identity), so a client can say which radio a plan will commit before the button.
  ...
  - `budget`: `{steps, dwell_s, pass_s, revisit_s, span_hz, step_span_hz, duty, dwell_total_s, step_overhead_s, overhead_measured, statement}`. `pass_s` **is** the revisit interval, and `duty` = `dwell/pass` — the honest headline of the trade, not a detection probability.
    - **`step_overhead_s` / `overhead_measured` / `dwell_total_s` (T-965): a pass costs more than its dwells, and `pass_s` now says how much.** A step is a **retune** — the synthesiser reprogrammed, the stream restarted, and at a class or rate boundary the run re-plumbed behind it — and none of that advances the sample clock the dwells are measured on. So a pass priced from `Σ dwell` alone prices it at exactly **zero**, which is what made `Scan everything (fast)` state **125.4 s** for 418 steps × 0.3 s and take **237 s** on the live HackRF (1.9×; `source.source_dropped` rose 3.5 G samples over the same pass — 182 s of air the steps never heard, the same quantity counted a second way). `dwell_total_s` is the listening alone (the old `pass_s`); `step_overhead_s` is the **measured** mean wall time one of this front end's retunes takes, timed by the scan worker around the one `set_center`/`set_rate` pair a step makes, and counted only for a step that succeeded (a refused step is retried and has not happened yet); `pass_s` is `dwell_total_s + steps × step_overhead_s`, and `duty` falls accordingly, because a retuning band is not a listened-to band.
    - **`step_overhead_s: null` is not zero.** Until a retune has been timed on this front end there is no measurement, `overhead_measured` is `false`, `pass_s` equals `dwell_total_s` — and `statement` says in words that the figure is *"a floor, not the answer: expect longer"*, the same rule `Coverage::Unobserved` and `BiasTee::Unknown` obey. The figure is never guessed from a constant (T-453: per-step cost is measured, not assumed). It outlives one scan, because it is a property of the front end rather than of a pass, so the second pass is priced from the first one's measurement.
  - `windows` (T-1008) — **only** on the scan routes' answers (`GET` with `windows=1`, and every `POST /api/control/scan` answer), never on `/api/control/state`, which is polled every couple of seconds while a full-range fine pass is thousands of steps: `[{step, lo_hz, hi_hz, center_hz}]`, one per step **in visit order** — the slice of the range that step is responsible for (the slices tile the compiled range with no gap and no overlap) and the centre it tunes to on an undithered pass (a dithered pass, T-173, is offset by the plan's DC dither; `progress.center_hz` is where the radio really is). It exists so the map's scan overlay (T-1008) draws **the steps the engine will execute** rather than a tiling of its own: the tiling depends on the rate in force, the device's ranges, RF-path boundaries and clipping, none of which a client knows.
  - `progress`: `{step, steps, pass, steps_done, center_hz, dwell_step, started_s, step_started_s, next_step_in_s, elapsed_s, measured_step_overhead_s, measured_pass_s}`, `null` when idle. `step` is the index of the step to tune **next**; `dwell_step` (T-1008) is the step being dwelt on now — the one whose retune last succeeded — and is `null` before the first step and whenever the sweep is not running (a yielded sweep is dwelling nowhere, whatever it last tuned). **T-965:** `elapsed_s` is how long this scan has been running, `measured_step_overhead_s` the same measurement `budget.step_overhead_s` carries and `measured_pass_s` the pass length it implies (`steps × (dwell_s + it)`) — so a client can put the stated pass beside the measured one without recomputing either. The last two are `null` until a step has been timed, and the running scan's `budget` is **repriced from this scan's own retunes on every read**, so the number converges while the user watches instead of staying at the preview's.
...
Contract (T-1009): `crates/hk-cli/tests/api_contract.rs::t1009_a_scan_and_a_clip_are_addressed_to_a_named_front_end` asserts, on the mock, that `/api/control/state` enumerates a sweep per front end in `scans`, that a price and a start naming this run's own radio are accepted and answer with that `device_id`, and that a `device_id` this run does not hold is `404 unknown_device` on the price, on the start (with nothing commissioned) and on a clip.

Contract (T-1008): the same test asserts `windows=1` returns twelve slices tiling 88–108 MHz, that the start answer carries them, that `device_id` names the mock, that `progress.dwell_step` is the step whose centre is tuned, and that **every per-step observation record's centre is one of the drawn windows' centres** — the plan drawn is the plan executed.
...
`GET /api/selections/{id}/watch` answers `{selection_id, watch, armed, alerts, suppressed, alerted_total, suppressed_total}`. `alerts[]` is `{anomaly_id, emitter, f_lo, f_hi, t, reason}` and `suppressed[]` is `{emitter, reason ("deferred" | "already-alerted"), relation ("suppressed-by" | "duplicate-of" | "artifact-of" | "retune-sibling-of" | "multipath-of" | null), artifact ("image" | "harmonic" | "intermod" | null), source, t, explanation}`, both oldest first and bounded (256 alerts, 64 suppressed). Counters are per run and also appear in `/api/status` as `watch_alerts` and `watch_suppressed`. `404 not_found` for an unknown selection, `503 unavailable` with no watch service.
...
- **The emitter's persisted analysis** (T-546): a bare **`{"emitter_id"}`** answers `200` with the emitter's latest analysis — the `emitter_synthesis` row (ADR-0015 §5.4) the pipeline wrote while it ran. This form **reads**; it never starts DSP, and every call is answered synchronously. Its producers are the trunking chain (`hk_pipeline::synth`) and attached region-analyze jobs (T-860). It is audited `analyze`. **On a withheld-identity emitter it answers exactly as it does for a never-analysed one** (T-884): `pipeline: null`, empty `evidence`/`trace`, `resolution.kind: "not-searched"`, and no `job` block — the same withholding `GET /api/inventory` applies to `synthesis` (T-159/T-163: a template id, a job id or a decode count must not confirm a withheld identity indirectly). There is **no marker** saying something was withheld, because such a marker is itself an oracle (as at `/api/inventory/{id}/classification`).
...
**There is no production search backend yet.** The job manager, admission, acquisition, cancel, stream and trace are real, the search engine (`hk_synth::engine`, M-3) runs behind a seam, and `Block::evidence` + `run_window` (M-2) exist; but nothing implements the engine's `Evaluator` over acquired IQ, so the server's backend — chosen in one place, `hk_pipeline::synth::jobs::server_backend()`, which `hk serve` and ADR-0015 §7's acceptance suite (T-863) both read — is `None`. So on this build every job **acquires its window for real — `window` says exactly what it read — and then ends `failed` with `error.code: "no_evaluator"`** and `resolution.kind: "not-searched"`. Nothing is presented as searched that was not.

| Method | Path | Body / query | Response |
|---|---|---|---|
| POST | `/api/analyze` | `{"emitter_id"}` | `200 AnalyzeResult` (the persisted analysis) |
| POST | `/api/analyze` | a target (`{"selection_id"} \| {"emitter_id"} \| {"band": {"f_lo", "f_hi", "t_lo"?, "t_hi"?}}`) + optional `profile`, `max_wall_s`, `source`, `live_s`, `templates`, `attach` | `202 {"job": AnalyzeJob}`, `Location: /api/analyze/{id}` |
| GET | `/api/analyze` | `?state=` | `{"jobs": [AnalyzeJob]}`, newest first |
| GET | `/api/analyze/{id}` | – | `AnalyzeJob` |
| DELETE | `/api/analyze/{id}` | – | `{"job": AnalyzeJob, "forgotten": bool}` — cancels a queued or running job (`state: "cancelled"`, partial results kept), or forgets a finished one (`forgotten: true`); audited `analyze_cancel` |
| GET | `/api/analyze/{id}/trace` | `?stage=&outcome=&family=&tried=&limit=` | the search trace (ADR-0021 §4.2) |
| GET | `/ws/analyze/{id}` | `?token=` | the job's `hackriff.analyze/1` stream (also `/ws/open/analyze?id=…` and TCP `open/analyze?id=…`) |

### Starting a job

- **Target.** Exactly one of `selection_id`, `emitter_id`, `band` (T-190's forms, unchanged). A selection supplies its `f_lo`/`f_hi` and, if bounded, its `t_lo`/`t_hi`; an emitter supplies its measured centre ± half its bandwidth (at least 1 kHz wide) and, as a **default** window, its span of appearances; a band gives `f_lo`/`f_hi` and optionally **both** `t_lo` and `t_hi` (Unix s, `t_lo < t_hi`).
- **`profile`**: `quick` · `standard` (default) · `deep` — the ADR-0015 §3.3 budgets. How much search happens is budgeted **by count** (evaluations, proposal-operator calls, assist operations — T-552/docs/27); wall and CPU time are backstops.
- **`max_wall_s`** (> 0, ≤ 600): lowers the profile's wall backstop; it never raises it.
...
**Acquisition** reads through the ring read API (`IqBufferService::read`, T-857) into a bounded-region read ledger: the first 60 % of the window is the search window and the rest the hold-out (ADR-0015 §3.1). A window longer than **2 s** keeps its newest 2 s (the ADR-0015 §6 IQ cap, applied to every window) and says so in `warnings`. When a search runs, the IQ it reads is first pinned as an `analyze` clip (`window.clip_id`).

### `AnalyzeJob`

`{id ("a<n>"), href, state, end_reason, error, target, profile, source, live_s, attach, templates, window, channel, budget, used, progress, results, trace_summary, replay_key, resolution, emitter_id, decodes, confirm, content_class, created, started, ended, warnings}`:
...
- `error`: `{code, message}` when it failed — `no_iq`, `evicted`, `source_ended` (a live collection whose capture stopped advancing), `no_evaluator`, `failed`.
- `window` (**filled from the read ledger, never from the request**, ADR-0015 §14.5): `{source ("ring" | "live"), t_lo, t_hi (Unix s), segments, samples, gaps, skipped_ns, bursts, bursts_missing, clip_id}`.
- `channel`: `{center_hz, bandwidth_hz, sample_rate_hz}`.
- `budget`: `{wall_s, cpu_s, max_evaluations, max_proposal_calls, max_assist_ops, max_iq_samples, max_cache_bytes, threads}`; `used`: `{wall_s, cpu_s, evaluations, hypotheses, proposal_calls, assist_ops, cache_bytes}`.
- `progress`: `{state, stage_max, beam, pruned, deferred, tried, not_tried, elided, evaluations, proposal_calls}` — `tried` and `not_tried` are kept apart (ADR-0021 §3).
...
- `trace_summary` (ADR-0021 §4.1): `{decisions, nodes_recorded, nodes_elided, truncated, by_outcome: {<outcome>: n}, by_stage: [{stage, tried, not_tried, best_bits}], replayable}`. `decisions = nodes_recorded + nodes_elided`; `replayable` is false when a wall/CPU backstop, a wall-measured plateau, a cancel, a throttle, a thermal pause or a power change decided something; the trace nodes such a cause left behind (`deferred_budget` after a time-caused stop, `refused_power`) carry `nondeterministic: true`.
- `replay_key` (ADR-0021 §5): `{engine, templates: [{id, version}], blocks: [{name, version}], calibration_hash, window, profile, budget: {max_evaluations, max_proposal_calls, max_assist_ops, max_cache_bytes}, seed_ref}` — what the trace can be reproduced from. `window` is `{clip_id, t_lo, t_hi}` of what the job read (from the read ledger); `seed_ref` is the SHA-256 of the canonical seeding (roots, unsupported suspicions, solve rule, trace bounds). Given an identical key and a job that stopped on a count, a re-run makes the same decisions node for node. `null` until the engine hands back; also on `resolution.replay_key` and the trace fetch.
- `resolution` (ADR-0021 §7A.2): present whenever the job finished without a `solved` result — the same object as `AnalyzeResult.resolution` below, with `coverage` for a finished search. `unknown` is written **only** by a `done` job. The sealed object (`job.sealed_resolution`) carries three more fields a summarised row does not (T-567): `suspected: {structure, missing_block, suspected_by, posterior?}` with `suspected_by` ∈ `classification | features | template | operator | unattributed`; `ruled_out: [{template, version, deepest_stage, best_bits}]`, the named formats this look tried and did not solve, once each; and `retry` (ADR-0021 §10) — `{worthwhile, on: [...], not_on: [...], reason_code, not_before?}`, where `reason_code` ∈ `budget-not-binding | budget-binding | snr-improved | more-support | unsupported` and the conditions are `more-budget | longer-window | new-block | new-template | new-engine | snr-improved | more-support`. The seal decides only the codes that are facts of the finished search (`budget-binding` when the queue was non-empty at the cap, `unsupported` for a missing block, `budget-not-binding` otherwise); the two data-driven codes are measured later against this object's `coverage.window`, never guessed here. `not_before` is the job's end plus the 1 h floor: no automatic re-analysis of the same emitter before it.
...
- `confirm` (M-9): `{rule, outcome, evidence_bits, reason}` on a `done` job, `null` otherwise. `rule` is `hk-pipeline/confirm-synth@2`; `outcome` is `confirmed` (this job confirmed the emitter), `already` (it would have, and the emitter was already confirmed — or a user deleted it, and a user delete wins), `insufficient` (the gate refused; `reason` names the clause) or `not-attached` (nothing to attach to, `attach: false`, or the server has no inventory); `evidence_bits` is the confirm key, the analytic hold-out bits. `reason` is backend-rendered and, on a confirm, is the lifecycle reason itself (below). Whenever the rule was evaluated (`confirmed`, `already`, `insufficient`), `confirm` also carries `decision_rate: {decisions_7d, assumed_per_week, budget_claim}` (ADR-0022 §8, T-575): the rule's evaluations over the last seven days of the device's clock, this one included, against the 20 000 a week the 24-bit threshold was derived for; `budget_claim` is `holds`, or `void` above that rate, in which case the reason also says so. The thresholds do not change with the count; the claim "at most one wrong Confirmed emitter per unattended week" does.
...
A `messages` stream (`message_schema: "hackriff.analyze/1"`, `content_class: unrestricted` — every field is in the record's `metadata`, and `frames_preview` is gated before a job is visible at all). Records are idempotent snapshots and **drop, never block** (ADR-0015 §5.2); `GET` is always authoritative. Each record's `metadata` is `{type, job_id, …}`:

- `progress` — `{job}`, at most once a second, and on each state change; the first record of every stream is the job as it stands.
- `stage` — `{stage, job}` when the deepest stage reached rises.
- `best` — `{results}` (the top 3) when they change.
- `trace` — `{stage, tried, not_tried, by_outcome, nodes}` once per stage (ADR-0021 §4.3), `nodes` its ≤ 8 highest-bits retained nodes.
- `done` — `{job}`, the final job; the stream then ends. Subscribing to a finished job yields exactly this record.

### `AnalyzeResult` (the bare `{"emitter_id"}` read)

`AnalyzeResult`: `{emitter_id, provenance ("synthesized by output analysis"), engine, t_ns, verdict, stage_reached, pipeline, evidence: [StageEvidence], trace: [TraceNode], resolution, receiver, job?}` — `job` only on a row a region-analyze job wrote (above). For a never-analysed emitter — **and for a withheld-identity one, whatever storage holds** (T-884) — only `emitter_id`, `pipeline` (null), `evidence` (empty), `trace` (empty) and `resolution` are present.
...
- `resolution` (ADR-0021 §7A.2), present whenever the verdict is below `solved`: `{kind, deepest_verdict?, reason?, suspected?, summary, explanations?}`. `kind` ∈ `unknown | structured-unidentified | unsupported-structure | not-searched`; `reason` ∈ `no-signal | nothing-scored | tied | budget-exhausted | unsupported-structure`. **`suspected` (T-567, ADR-0021 §7A.6) is present on exactly the `unsupported-structure` rows** and on no other kind: `{structure, missing_block}` — the structure the search suspected and the **stable id of the block this build lacks**, as fields rather than as a phrase in `summary`, because without the name the absence of (say) a LoRa decode reads identically to a LoRa signal decoded as noise, and because ADR-0021 §9.4's backlog ("3 emitters are waiting on `psk_demod`") is then a group-by rather than a text search. Who suspected it and with what posterior are in the job's `sealed_resolution`.

**`explanations` (T-569, ADR-0021 §9).** Ranked known-signal **suggestions about** the resolution — omitted when empty, and never present on a `not-searched` row (there is no result to explain). Each is `{source, identity, score, distance_hz?, status, data_age_days?, reasoning}`, with `source` ∈ `alloc | band-plan | licence | catalogue | history` and `status` ∈ `expected | unexpected | no-reference-data`. **A suggestion explains a result; it never becomes one.** It is computed by `hk-context` from the **measured** centre, bandwidth and classified family *after* the resolution is sealed, and modifies nothing else on the row — not `kind`, `reason`, `summary`, `verdict`, `stage_reached`, the job's `evidence_bits`/`prior_bits`/`check`, the emitter's measured `f`/`BW`, its lifecycle or any classification. An `unknown` carrying three high-scoring explanations is still `unknown`, and a `tied` result is still `tied`: a client renders the list **subordinate to** the resolution, never in its place and never as the row's headline. `distance_hz` is the measured centre minus the nearest channel of the named plan, so an emission 150 kHz off the FM raster reads `status: "unexpected"` with `distance_hz: -150000` **and keeps its measured centre** — the mismatch is the interesting case, not an error to correct. `no-reference-data` means nothing in the loaded table covers the measurement, which is not "nothing is expected here". The direction is structural, not editorial: `hk-synth` does not depend on `hk-context`, so the search cannot read a suggestion at all (`crates/hk-synth/tests/explanation_boundary.rs`).
- `receiver` (docs/19 §7.6a): `{grid_hz, offset_hz, concentration, ppm}` — the receiver's **fitted** offset from the channel grid, a property of the radio rather than of the signal. It is known **modulo the grid spacing**, which is all a periodic grid can say. `alias` (T-628, optional; absent on older rows, which reads as not tried): `{state, evidence, offset_hz?, ppm?, bound_ppm, candidates, targets, occupied, runner_up}` — which **absolute** offset that modulo-grid fit really is, needed to reach an absolute frequency such as a trunking grant. `state` ∈ `not-tried | resolved | unresolved` (ADR-0021: not tried is a different answer from tried and unsettled); `evidence` ∈ `clock-bound` (the receiver's ±`bound_ppm` crystal bound admits one alias) · `granted-channel-energy` (of the `candidates` aliases the bound admits, exactly one found transmissions on more of the `targets` granted channels than any other: `occupied` vs `runner_up`) · `no-granted-channel` · `no-reference` · `no-alias-in-bound` · `nothing-occupied` · `tied`. `offset_hz`/`ppm` are present **only** when `resolved`; an unresolved alias carries no offset, and nothing is followed from it.

Give exactly one of `selection_id`/`emitter_id`/`band`, else `400 invalid`; an unknown field anywhere in the body (or in `band` or `templates`), a malformed job field, `band.f_lo >= band.f_hi`, or only one of `band.t_lo`/`band.t_hi` is `400 invalid`. `404 not_found` for an unknown selection or emitter, including a malformed id — `{id}` may name an entity since merged into another and resolves to the live emitter, exactly like `GET /api/inventory/{id}`. `503 unavailable` when this server has no selection store or no inventory. `POST` and `DELETE` are authenticated and audited like every other mutating control route (same bearer-token, header-only and cross-origin rules); the bare emitter read records nothing and changes nothing. Messages never echo request values.
...
- **Storage: a pre-allocated ring** (`<data dir>/iqbuffer/`).
  ...
  - **Whose ring (T-1009).** Every front end of a run keeps its **own** ring, so with two radios *"the IQ of this region"* has two answers. `POST /api/iqbuffer/clip` takes `"device_id"`: omitted, it is the run's **default** front end's ring (the only one a single-SDR run has, so an existing body is unchanged); named, it is that front end's ring; a name no front end with a ring holds is `404 unknown_device`, listing the ones that do — never another radio's samples handed back under the name asked for. `GET /api/iqbuffer` still reports the default front end's ring.
...
| POST | `/api/iqbuffer/clip` | one range: `{"t0", "t1"}` (Unix s), `{"t0_ns", "t1_ns"}` (integer Unix ns) or `{"global_index", "samples"}`; plus `"band"?: {"f_lo", "f_hi"}, "label"?, "run"?, "device_id"?` (T-1009) | `{"recording": Clip}` (audited `iqbuffer_clip`) |

**`IqBufferStatus`**: `{enabled, reason, dir, retention_s, max_bytes, quota_bytes, max_clip_bytes, chunk_bytes, chunk_files, slot_count, allocated_bytes, allocation, allocation_progress, preallocated, persisted, run, recovered_segments, discarded_slots, head_slot, head_offset_bytes, wrap_count, fs_free_bytes, fs_total_bytes, min_free_bytes, paused, pauses, paused_samples, t0, t1, span_s, drops: [{at, t0}], bytes, disk_bytes, samples, segments_total, segments_omitted, segments: [Segment], gaps: [Gap], evicted: {chunks, segments, samples, bytes}, dropped_samples, gated_samples, write_errors, failed_samples, sync_errors, poisoned_samples, allocation_skipped_samples, error}`.
...
- `drops` (T-845) are the ring's next whole-slot evictions, oldest first. When the write head reaches `at` (sample clock, Unix s), the oldest slot is overwritten and `t0` jumps to the entry's `t0`. The first evictions after any unused slots are included; a drop that would not move `t0` (duration retention already trimmed past it) is left out. The schedule covers `DROP_LOOKAHEAD_S` (60 s) past the write head, plus the first drop beyond that. It holds at most `DROPS_MAX` (32) entries: a longer schedule is thinned, keeping the later drop of each group. It is empty when there is nothing to drop. `t0` is exact; `at` is predicted at the open segment's rate.
...
- **Status records** (flat metadata, ~4 Hz): `position_ns`, `playing`, `speed`, `generation`, `analysis: "recorded"`, `iq`, `iq_source` (`ring`, `recording`, or `none`), `iq_until_ns` (end of the raw-IQ window in use), `next_iq_ns` (when `iq` is false and IQ resumes later), `frames`, `squelched_frames`, `windows`, `rds_groups`, `rds_pi` (WFM with RDS decoded), and the demodulator's `level_dbfs` (T-966: the delivered audio's own level, a ~50 ms meter sample (`level_tau_s`) on this ~250 ms tick rather than an interval RMS, after demod/AGC/clamp — never above 0 dBFS, and the mean power over both channels on a two-channel stream, with the smoothing computed per audio frame so stereo keeps the same time constant as mono. **While `squelch_open` is false it reads the documented silence floor** (T-1015: `hk_demod::audio::SILENCE_FLOOR_DBFS`, -120 dBFS), never the discarded demod output, and restarts from the delivered audio the next time squelch opens), `squelch_open`, `agc_gain_db`, `snr_db`.
...
| GET | `/ws/changes` | token (`?token=`) | The **route-version feed**: `changed {route, version}` instead of a client poll — see [its section](#get-wschanges--the-route-version-feed-the-server-says-which-route-changed-t-1065) (T-1065) |
...
**Pipeline** JSON: `id` (`p<n>`), `recipe_id`, `recipe_version`, `edit_rev` (0 = as started), `owner` (`explicit` \| `session`, ADR-0015 §12.3: `explicit` is a pipeline somebody started here, which outlives every consumer; `session` is an **ephemeral** one the `listen` opener started for a listener — never saved, stopped when its last listener detaches, and an opener whose target already has a running audio pipeline attaches to it rather than starting a second), `state` (`running` \| `ended`), `end_reason` (`stopped`, `source-ended`, `segment-ended`, `retune: …`, `rate-change: …`, `error: node <id>: …`), `target`, `channel: {center_hz, bandwidth_hz, sample_rate_hz}` (the channel **in force**: the target's at start, then as `input` edits and applied refinements move it), `content_class`, `emitter_id`, `started` (Unix s), `nodes: [{id, block, outputs}]` (topological order), `outputs: [{id, kind (inspector \| stage \| messages \| audio), stream_id}]` (a `messages` output is listed only while it offers a stream, see below), `status` (the latest status tick: flat `<node>.<metric>` keys, ADR-0011 §1.3), `stats: {samples, chunks, frames, gaps, discontinuities, skipped_samples, live_edge_seeks, edits, status_ticks, decodes, decodes_dropped}` (`decodes`: Decode rows the `messages` outputs stored; `decodes_dropped`: frames dropped because a writer queue was full; `live_edge_seeks`: seeks a `live-edge` reader made to stay within its backlog bound, T-866), `warnings`, `follow_hops`, `liveness` (T-866, ADR-0011 §8.5: `{mode: "live-edge", max_backlog_s}` or `{mode: "throughput"}` — the recipe's `input.liveness`, else `live-edge` at 0.6 s for a recipe with an `audio` output and `throughput` otherwise; a `live-edge` reader further behind the writer than `max_backlog_s` seeks to the live edge, counts it and flags the next chunk `DISCONTINUITY`; a `throughput` reader keeps the 2 s ring-protection skip every pipeline has; an edit of `liveness` alone re-plumbs nothing), `refinement` (T-870, below; `null` unless the recipe declares `refine.objective.builtin`).

**Refinement** (T-870 = ADR-0015 LP-6; ADR-0011 §8.7, ADR-0015 §12.6). A recipe may declare `"refine": {"objective": {"builtin": "wfm-pilot"}, "tune": ["center_hz", "bandwidth_hz"]}` (schema 3; `builtin` names a registered objective — `wfm-pilot`, T-070's pilot/occupied-bandwidth/RDS objective over the **channel IQ window** — and stands alone: a `refine.objective` is either `{builtin}` or `{node, metric, goal}`, never a mix; an iq input on one channel, not follow-hops). The pipeline then refines its channel from the demodulated output with the same loop, settings and hysteresis as Listen: once at start from the target (the target is only the search's start — nothing consults a band plan) and again every 15 s in the background, off the pipeline thread. Each **locked** result that passes the hysteresis is **applied as an ordinary hot edit**: the refined centre becomes the channel centre and the refined bandwidth the running revision's `input.bandwidth_hz` (only the axes `tune` lists), so the channel is re-plumbed and every node rebuilt at a chunk boundary exactly as for an `input` edit — capture and the ring reader carry on, unchanged outputs keep their streams and consumers, `edit_rev` advances by one and `POST …/save` saves the refined revision. A result that did not lock never retunes. It is stored like Listen's: an `emitter_refined_tuning` row (`source: "recipe"`, provenance `refined by output analysis`) on the target emitter or the inventory emitter at the refined channel, whose measured centre and bandwidth are never overwritten. `refinement` is `{objective: {builtin}, tune, attempts, updates, applied_edit_rev, current, last, last_refusal}`: `current` is the applied result (Listen's header `audio.refinement` record plus `locked`), `last` the latest attempt `{locked, accepted, center_hz, bandwidth_hz, quality, stop, t}` (`t` capture-clock Unix s of its window), `last_refusal` the edit's refusal code when an accepted result could not be applied (e.g. `outside_window`). An `audio` output's status records carry `refined_center_hz`, `refined_bandwidth_hz` and `refine_updates` from it, as Listen's do. A `{node, metric}` objective is validated but not yet run (T-870 serves `builtin` only).
...
- An `input` edit re-plumbs the channel DDC and rebuilds every node. So does an applied refinement (T-870, "Refinement" above), which is the same edit made by the pipeline itself.
...
| `audio/<pipeline>/<output>` | `/ws/audio/<pipeline>/<output>` or TCP `audio/<pipeline>/<output>?token=…` (a recipe's `audio` output, T-866, ADR-0011 §8.2) | the stream contract §12.2 **audio profile, unchanged**: `kind: audio`, `ri16_le`, 48 kS/s, 960-sample type-1 records (a closed squelch or a live-edge seek is a `sample_index` jump plus `DISCONTINUITY`), and one type-3 status record per ~250 ms tick carrying the §12.2 audio keys **and** the `<node>.<metric>` batch. Header: the §12.2 `audio` object, whose `mode` is the output's `profile.mode` with `mode_rules: "recipe-declared"` and `mode_confidence: 0` (declared, not estimated), plus the additive `pipeline_id`, `recipe` (`<id>@<version>`), `output_id`, `edit_rev`. Audio is content: a recipe with an `audio` output runs Listen's pre-attach gate (`listen_class`) on its channel at start (and at an edit that keeps audio) before any ring read — its refusal is the start's (`403 gated`) — and a gated audio record ends the pipeline (fail closed) |
...
| GET | `/ws/{stream_id}` | **Served (T-866):** a running audio pipeline offers its output as the always-on stream `audio/<pipeline>/<output>` ("Recipes and pipelines" above). |
| POST | `/api/pipelines` | **Served (T-866):** accepts a recipe with an `audio` output and runs the pre-attach content gate. *Still planned (LP-8):* admission against the listener budget — until then an audio pipeline is admitted as a `recipe` chain only. |
| GET | `/api/pipelines/{id}` | **Served (T-866):** `outputs[]` has `kind: "audio"`. *Still planned (LP-5):* session-owned pipelines report `owner: "session"`. |
| — | audio stream header | **Served (T-866):** the `audio` object gains `pipeline_id`, `recipe` (`<id>@<version>`), `output_id`, `edit_rev`. Existing keys keep their names and meanings; unknown-key-tolerant readers are unaffected. |
| — | audio status records | **Served (T-866):** keep every audio key and additionally carry the per-node `<node>.<metric>` batch (ADR-0011 §1.3) on the same tick. |
...
## Marker collections (T-817, MAP-17)

Named, toggleable collections of **time-frequency markers** — the durable research state the map UI's collection layer and table read ([`docs/25 §3`](25-spectrum-research-workflow.md), the store contract in §10; RESEARCH-003). Code: `crates/hk-api/src/collections.rs` over `crates/hk-model/src/repo/collections.rs`; tests: `crates/hk-api/tests/collections_api.rs`, `api_contract.rs::marker_collections_answer_as_documented`. Same token, audit and error rules as bookmarks and selections, stored in the run's database so they survive a restart. **Nothing here feeds blind detection** (docs/25 §10.7): a marker mints no candidate, sets no family and never pre-populates the inventory.

| Method | Path | Body / query | Answer | Errors |
|---|---|---|---|---|
| GET | `/api/collections` | `limit`? (500, max 2000), `cursor`? | `{collections: [Collection, …], count, matched, limit, next_cursor}`, creation order (the reserved collection first) | 400 invalid (limit/cursor) |
| POST | `/api/collections` | `{"name", "note"?, "color"?, "visible"?, "id"?}` | `Collection` (`201`) | 400 invalid, 409 conflict (a supplied `id` that exists) |
| GET | `/api/collections/{id}` | – | `Collection` | 404 not_found |
| PUT | `/api/collections/{id}` | any create field but `id` (`null` clears `note`/`color`); `{"visible"}` alone toggles the layer | `Collection` | 400, 404 |
| DELETE | `/api/collections/{id}` | – | `{"deleted": Collection, "members_deleted": N}` — its markers go with it | 400 invalid (the reserved collection), 404 |
| GET | `/api/collections/{id}/markers` | `limit`?, `cursor`?, window box `f_lo`&`f_hi`? (Hz), `t0`&`t1`? (Unix s) | `{collection_id, markers: [Marker, …], count, matched, limit, next_cursor}`, creation order | 400 invalid, 404 (no such collection) |
| POST | `/api/collections/{id}/markers` | `{"name", "f_center_hz", "view", "bandwidth_hz"?, "t_center_s"?, "duration_s"?, "note"?, "id"?}` | `Marker` (`201`) | 400 invalid, 404, 409 conflict |
| GET | `/api/markers` | as the per-collection list, over **every** collection (the canvas layer's query) | `{collection_id: null, markers, …}` | 400 invalid |
| GET | `/api/markers/{id}` | – | `Marker` | 404 |
| PUT | `/api/markers/{id}` | any create field but `id` (`null` clears `bandwidth_hz`/`t_center_s`/`duration_s`/`note`; clearing `t_center_s` clears `duration_s` too); a `view` re-stamps provenance, without one it is kept | `Marker` | 400, 404 |
| DELETE | `/api/markers/{id}` | – | `{"deleted": Marker}` | 404 |

`Collection`: `{id, name, note, color ("#rrggbb" or null), visible, reserved, member_count, created_s, updated_s}`. `visible` is the stored default the layers menu toggles (a collection *is* an overlay layer); `reserved` is true only for the built-in `Bookmarks` collection.

`Marker`: `{id, collection_id, name, note, f_center_hz, bandwidth_hz, f_lo_hz, f_hi_hz, t_center_s, duration_s, t_start_s, t_end_s, provenance, created_s, updated_s}`.
- **A marker is a time–frequency place** (ADR-0017). `t_center_s: null` is a **frequency-only pin** — a band marked for all time, drawn full height; with a time and no `duration_s` it is a **point**; with both it is a **box** of `duration_s` centred on `t_center_s`. `t_center_s` is on the **capture clock**. `f_lo_hz`/`f_hi_hz` and `t_start_s`/`t_end_s` are computed here, so a client never derives an extent from two fields (the [shared time axis](#one-shared-time-axis-t-337) rule); both time edges are `null` for a pin.
- **The window box** matches markers whose frequency extent overlaps `[f_lo, f_hi]` and whose time extent overlaps `[t0, t1]`; a frequency-only pin matches **every** time window. Each pair is both-or-neither. Paging is the `/api/events` contract: `next_cursor` is `null` on the last page.
- **`provenance` is stamped by the server, never accepted** (docs/25 §10.2): `{device_id, center_hz, span_hz, sample_rate_hz, t_capture: [t0, t1], tier, authored_s, actor, authored: true}`. The client sends only `view`, the pane it was on: `{"center_hz", "span_hz", "t_capture" (a capture-clock instant or [t0, t1], Unix s), "tier" ("live-iq"|"spectrum-history"|"survey-overview"), "device_id"?}` — all but `device_id` required on create. The server adds `actor` (the token id, **never the token**), `authored_s` (wall clock — kept apart from the capture-clock `t_capture`), `authored: true`, and `sample_rate_hz` when `device_id` (or, with it omitted, the run's only front end) resolves to a live front end, else `null`. A `view` carrying `actor`, `authored_s`, `authored` or `sample_rate_hz` is **`400 invalid`**. A marker written through `/api/bookmarks` has a view-less stamp (`null` view fields, its `actor` and `authored_s` only).
- **No answer or audit entry carries a `device` key**: authoring is a view act and reaches no radio.
- **The reserved `Bookmarks` collection** (fixed id `00000000-0000-7000-8000-000000000b00`) is the one `/api/bookmarks` is a facade over. It cannot be deleted and holds **frequency-only markers only** — a time on one of its markers is `400 invalid`, because a bookmark has no time. A marker a client adds to it through `/api/collections/{id}/markers` is listed by `/api/bookmarks` (as `kind: "bookmark"`), and a marker in any other collection is not a bookmark (`/api/bookmarks/{id}` answers 404).
- Limits: names 1–120 characters (trimmed), notes ≤ 2000, `device_id` ≤ 200; at most 10 000 collections and 100 000 markers per collection. Audited as `collection_create`, `collection_update`, `collection_delete`, `marker_create`, `marker_update`, `marker_delete`; without an audit log every mutating route answers `503 unavailable`, and without a store every route does.

## Reserved: the map-UI research routes (MMAP, T-800 / ADR-0023)

**These routes are SPECIFIED and RESERVED, not yet served** — except the MAP-17 rows, **served since
T-817** and documented in full under [Marker collections](#marker-collections-t-817-map-17). T-800 (MAP-00) fixed their shapes so the
...
| GET/PUT/DELETE | `/api/collections/{id}` | MAP-17 (**served**) | any create field; `{visible}` toggles | `Collection` / `{deleted, members_deleted}` |
| GET/POST | `/api/collections/{id}/markers` | MAP-17 (**served**) | `{name, f_center_hz, bandwidth_hz?, t_center_s?, duration_s?, note?, view}` | `{markers, …}` / `Marker` (201, audited) |
| GET/PUT/DELETE | `/api/markers/{id}` | MAP-17 (**served**) | any create field (`null` clears optionals) | `Marker` / `{deleted}` |
