...
- `scan`: `{state: "idle"|"running"|"yielded", available, unavailable_reason, device_id, plan, budget, progress, yielded}` — `device_id` (T-1008) is the front end this runner sweeps (the run's default one; `null` when the source reports no identity), so a client can say which radio a plan will commit before the button.
  ...
  - `progress`: `{step, steps, pass, steps_done, center_hz, dwell_step, started_s, step_started_s, next_step_in_s}`, `null` when idle. `step` is the index of the step to tune **next**; `dwell_step` (T-1008) is the step being dwelt on now — the one whose retune last succeeded — and is `null` before the first step and whenever the sweep is not running (a yielded sweep is dwelling nowhere, whatever it last tuned).
...
- `resolution` (ADR-0021 §7A.2), present whenever the verdict is below `solved`: `{kind, deepest_verdict?, reason?, suspected?, summary}`. `kind` ∈ `unknown | structured-unidentified | unsupported-structure | not-searched`; `reason` ∈ `no-signal | nothing-scored | tied | budget-exhausted | unsupported-structure`. **`suspected` (T-567, ADR-0021 §7A.6) is present on exactly the `unsupported-structure` rows** and on no other kind: `{structure, missing_block}` — the structure the search suspected and the **stable id of the block this build lacks**, as fields rather than as a phrase in `summary`, because without the name the absence of (say) a LoRa decode reads identically to a LoRa signal decoded as noise, and because ADR-0021 §9.4's backlog ("3 emitters are waiting on `psk_demod`") is then a group-by rather than a text search. Who suspected it and with what posterior are in the job's `sealed_resolution`.
