...
- **A receiver artefact is explained as one.** A row the receiver made — a standing `artifact-of` relation, or the `artifact` tag the inventory writes when a track's member detections were mostly suspect (DC/LO leakage, a reference or clock harmonic, a comb tooth, an image, IMD, clipping) — carries exactly one explanation, `service: "receiver-artefact"`, label `Receiver artefact`, with an `evidence[]` entry of kind `artefact` naming what said so. No service is offered for it, because nothing was on the air.
