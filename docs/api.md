...
**Provisional identity (T-962).** A decoded identity whose scheme cannot carry it on one frame needs a **vote** before any decoder writes it: an RDS PI becomes an identity only after **10 agreeing CRC-valid groups** (`hk_model::RDS_PI_COMMIT_VOTES`; 11.4 groups/s is under a second of real lock), the same bar for the built-in `hk-rds` decoder and for every recipe `messages` output naming `rds-pi`. Below it the row is still written and served, but **with no identity** (no emitter is created, keyed or confirmed by it) and these fields: `identity_provisional: true`, `identity_votes` (agreeing CRC-valid frames so far — groups for `hk-rds`; for a recipe output, its frames, each at least one group), `identity_votes_needed` (the bar), and — on a recipe row, whose own identity column is empty — `identity_scheme` and `identity_value` (e.g. `"rds-pi"`, `"1704"`). A row at or above the bar carries `identity_provisional: false` with the same counts. `hk-rds`'s `rds-pi` row also keeps its own `pi_provisional` (the same flag), beside `pi`/`pi_votes`/`pi_total_votes`/`pi_share`. **An emitter with no decoded identity answers the provisional rows linked to it** (a recipe run's target, or the always-on chain's emitter hint), latest per `(decoder, frame_model)` and windowed as above, so the panel can show "PI 1704 (3 groups, provisional)"; with none, `{"decodes": []}`.
