...
- **Retention: oldest first**, whichever limit is reached first: disk bytes (whole oldest chunk files are deleted) or duration (`t1 − t0` on the sample clock, trimmed to the sample). Every eviction is counted in `evicted`.
- **Defaults and disk impact.** On for every run except a lossless (unpaced) replay, which is a recording already. Quota **2 GiB or 10 min**, whichever comes first. Overrides: `HK_IQ_BUFFER=0` disables it, `=1` forces it on (lossless replays too), `HK_IQ_BUFFER_BYTES=<bytes>`, `HK_IQ_BUFFER_S=<seconds>` (0 disables). At 2 bytes/sample the writer sustains `2 × rate` bytes/s: 4.8 MB/s at 2.4 Msps (2 GiB ≈ 7.5 min), 40 MB/s at 20 Msps (2 GiB ≈ 54 s); 10 min fits in 2 GiB only below ≈ 1.79 Msps. Continuous writes at high rates wear flash: at 20 Msps that is ≈ 3.5 TB/day, so size or disable the buffer on storage with limited endurance.
- **Never blocks capture.** The buffer is one more ring reader on its own thread; the ring never waits for it on a live source. A disk too slow for the rate laps the reader: the lost samples are counted in `dropped_samples` and show as a gap before the next segment. A failed write drops that data (counted, `write_errors`, `error`) and the buffer carries on.
...
| POST | `/api/iqbuffer/clip` | `{"t0", "t1", "band"?: {"f_lo", "f_hi"}, "label"?}` | `{"recording": Clip}` (audited `iqbuffer_clip`) |

**`IqBufferStatus`**: `{enabled, reason, dir, max_bytes, max_s, chunk_bytes, t0, t1, span_s, bytes, disk_bytes, samples, segments_total, segments_omitted, segments: [Segment], gaps: [Gap], evicted: {chunks, segments, samples, bytes}, dropped_samples, gated_samples, write_errors, error}`.
- `enabled: false` with a `reason` when the run has no buffer (disabled, lossless replay, or the directory could not open); every count is then 0. `max_bytes` is the enforced disk quota (at least two chunks).
...
- **`Segment`**: `{id, t0, t1, samples, global_index, center_hz, sample_rate_hz, bandwidth_hz, lna_db, vga_db, amp_on, device_id, antenna_port, overload, content_class, dropped_before}`. `global_index` is the stream index of the first retained sample; `dropped_before` the samples this buffer lost to overruns just before the segment.
...
- **`Clip`**: `{id, label, meta_uri, data_uri, meta_path, data_path, t0, t1, samples, bytes, sample_rate_hz, center_hz, band ([f_lo, f_hi] or null), content_class, captures: [{sample_start, samples, global_index, t0, segment, center_hz, sample_rate_hz, bandwidth_hz, lna_db, vga_db, amp_on, device_id}]}` (`*_uri` relative to the data directory).
- **Errors** `{"error", "code"}`: `400 invalid` (bad or missing `t0`/`t1`, `t0 ≥ t1`, bad `band`/`label`, unknown field or query parameter, a clip over 4 GiB), `404 not_found` (nothing buffered in the range and band, e.g. already evicted), `409 conflict` (the range spans a sample-rate change: SigMF has one rate per file, so export each side), `503 unavailable` (the run has no buffer, or this server has none), `500 failed` (storage), `405` other methods. Messages never echo values.
