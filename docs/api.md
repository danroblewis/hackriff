...
**An open session ends with a real close frame, never a bare hang-up (T-954).** Every `/ws/open/<name>` session — `listen`, `bits`, `symbols`, `iq`, `playback`, `inspector`, `stage`, every opener — shares one close path (`ondemand.rs`, T-066's liveness watcher), which now sends a code and reason before the socket goes down: `1000` (`closed`) for a client-initiated close frame or a producer that finished normally; `1000` (`no response within the peer timeout`) for a peer this server reaped for silence (§ above, `ondemand_peer_timeout`); `1002` (`unexpected message from a consumer`) — consumers never write, so any data breaks the contract; `1011` (`transport reset`) is attempted best-effort on a faulted socket, which may already be unwritable. Before this a session simply shut the TCP connection down once it decided the peer was gone, which every browser reports as `1006` ("abnormal closure") — indistinguishable from a real fault — however clean the actual reason was. The one exception is a session that ended before any byte ever reached the peer (the producer's first publish never arrived): there the `101` upgrade itself is incomplete on the wire, so no close frame is sent either — the raw hang-up is the honest answer there, same as before.
