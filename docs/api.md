...
- **A receiver artefact is explained as one — but never over a measurement.** A row the receiver made — a standing `artifact-of` relation, or a majority of the row's newest linked detections flagged by a **receiver-mechanism** rule (spur candidate: DC/LO leakage, a reference or clock harmonic, a comb tooth, a spur-map entry; IQ image; intermodulation) — carries `service: "receiver-artefact"`, label `Receiver artefact`, with an `evidence[]` entry of kind `artefact` naming what said so. Three qualifications a client can rely on: `clipped` and `compressed` are **not** counted (they say the measurement could not be trusted, not that the receiver invented the signal, so a strong station that clipped the ADC is never relabelled); a row that *does* carry demodulator, decoder or classifier evidence keeps that evidence at rank 1 and gets the artefact only as a ranked alternative below it; and the verdict is **derived on every read from the current detections and never stored**, so a line that stops being flagged stops being an artefact. Only a row with no such evidence gets the artefact as its single explanation, because nothing was on the air to allocate.
