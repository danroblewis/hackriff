...
**Planned, not served yet.** None of the routes below are in `ROUTES` today (T-119's sites, baselines, candidates and weights have landed and moved to [their own section](#sites-baselines-candidates-and-score-weights-t-119)). They are named here so the parallel M2 tasks and the M2 UI hooks (T-123) code against one surface. When an owning task lands, it moves its rows into a normal section with request/response shapes and contract tests.
