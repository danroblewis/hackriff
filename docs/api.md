...
**Stage evaluation over IQ is not built yet (MAUTO M-2).** The job manager, admission, acquisition, cancel, stream and trace are real, and the search engine (`hk_synth::engine`, M-3) runs behind a seam; what evaluates a pipeline stage over IQ (`Block::evidence` + `run_window`) has not landed. So on this build every job **acquires its window for real — `window` says exactly what it read — and then ends `failed` with `error.code: "no_evaluator"`** and `resolution.kind: "not-searched"`. Nothing is presented as searched that was not.
