...
| GET | `/api/control/scan[?f_lo_hz&f_hi_hz&dwell_s&step&windows]` (T-452, `step` T-517, `windows` T-1008) | – | `{scan, proposed}` — `proposed` prices the named sweep **without starting it**, `null` when none is named; `windows=1` adds each step's slice to both plans (`plan.windows`) | 400 invalid (a range the device cannot reach; `windows` other than `1`/`0`/`true`/`false`), 409 not_live |
| POST | `/api/control/scan` (T-452) | `{"f_lo_hz"?, "f_hi_hz"?, "dwell_s"?, "step"?}`, or `{"resume": true}` | `{scan, proposed, device: {commissions, id, commissions_rate_hz}}` **(commissions retunes, and for a coarse step one rate change)**; both plans carry `windows` (T-1008) | 400 invalid, 409 refused (one already running / nothing to resume), 409 not_live |
