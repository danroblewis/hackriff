...
A job that finished **`done`** with results, asked to `attach`, attaches on the server (`hk_pipeline::synth::attach`); cancelled and failed jobs never do. A failed attach is a `warnings` entry, never a failed job.

- **The emitter.** An emitter target is that emitter. Otherwise the inventory emitter nearest the searched channel **among those seen in the analysed window** (centre within half the band, at most twice as wide; a user-deleted row is never chosen). If there is none, the job's stored decodes create a candidate through ordinary decode ingestion.
