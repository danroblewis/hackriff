  ...
  "entries": [
    {
      ...
      "identity_value": "A1B2",
      "identity_label": "KROQ", "identity_confidence": 0.92,
...
**`identity_label` / `identity_confidence` (T-967).** The decoded identity, in the form a list row actually wants to show — a human-readable label a decoder committed (RDS's PS station name for an `rds-pi` identity, and the same shape for any decoder that names a comparably-purposed field: `callsign`, `flight`, `station`), and the decoder's own confidence/vote share behind its latest value (RDS's `pi_share` — how many of the recent groups agreed), when it recorded one. `identity_value` alone is the bare code (an RDS PI is a 4-hex-digit number, unreadable at a glance); this is what a client renders next to it. Both are read from the identity's most recent matching decodes — a decoder is not consistent about whether it writes the label into a decode's `metadata` or its (gated) `content`, so both are checked — and both are `null` together when none of them carries a recognised field, and always on a withheld-identity row or a row with no decoded identity at all, exactly as `estimated_params`/`cluster_id` are (T-159/T-163): the field can never confirm a withheld identity by appearing. No parsing happens in the UI (CLAUDE.md thin-client rule) — this is the backend's own rendering, the same discipline `/api/inventory/{id}/decode`'s `fields` already follows.
