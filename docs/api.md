...
- **New emitter (T-136):** fed from the inventory's first sightings at each occupancy close. Every emitter first seen in the last hour carries the site's new-emitter novelty for that hour (the Poisson tail of the first sightings against the site's usual rate). `subject` is the cells covering the new emitters' measured extents, merged like other cell alarms, so a burst on one channel is one alarm. `detail.observed` is the first sightings in the hour, and `baseline_mean` the count expected at the usual rate. One new emitter on its own is rarely novel. While the rate is immature (< 24 h observed at the site) the evidence is counted as `immature-baseline`.
