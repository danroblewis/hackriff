...
- `SIGNAL-085` **Automatic control-channel discovery** — Find a control channel with no prior research: flag continuous 100%-duty 4FSK/C4FM on the LMR raster, confirm by frame sync, and propose "this looks like P25 Phase 1, follow it?". The hunt reads no band: it runs on whatever the radio is tuned to, and the built-in registry *spends* it only in the land-mobile allocations — a dwell budget, not a search prior (T-615); a plan widening that budget to 1 MHz–6 GHz finds the same channel anywhere. `VHF / UHF / 700 / 800 MHz` · `RX`
...
### Paging & Amateur Packet Radio (added 2026-09-25 from an explorer-agent live capture, T-949)

A 2026-09-25 explorer-agent HackRF capture at 930.8 MHz found three live FLEX paging channels (929.608/929.933/931.158 MHz) that blind detection flagged but the app does not yet decode (fixture: `flex-pagers-930p8`, T-935/T-949); no existing use case covered terrestrial paging. APRS was added alongside it: SIGNAL-036 already covers the ISS digipeater relay specifically, but the everyday terrestrial network (same AFSK1200/AX.25 modem, no Doppler) had no id.

- `SIGNAL-088` **Terrestrial paging (FLEX/POCSAG)** — Blind-detect 929–932 MHz paging carriers as time-frequency regions, classify FLEX vs POCSAG from sync-word search (FLEX `0xA6C6AAAA`, POCSAG preamble) and measured symbol rate/level count (2FSK/4FSK at 1600/3200/512/1200/2400 bps), and decode messages once a bitstream decoder is wired in. `929–932 MHz` · `RTL-SDR/HackRF` · [FCC 47 CFR Part 22 Subpart E](https://www.ecfr.gov/current/title-47/chapter-I/subchapter-B/part-22/subpart-E) [sigidwiki FLEX](https://www.sigidwiki.com/wiki/FLEX) [sigidwiki POCSAG](https://www.sigidwiki.com/wiki/POCSAG)
- `SIGNAL-089` **Terrestrial APRS (AFSK1200/AX.25)** — Decode the primary North American APRS frequency: AFSK1200 demodulated to AX.25 frames carrying station callsign/SSID, position, course/speed, weather telemetry and free-text status, direct and digipeated (WIDE1-1/WIDE2-1) — the same modem SIGNAL-036 uses for the ISS digipeater, without satellite Doppler. `144.390 MHz` · `RTL-SDR` · [aprs.org](http://www.aprs.org) [sigidwiki APRS](https://www.sigidwiki.com/wiki/APRS)

### Conventional Land-Mobile Voice, Airband & Marine Voice (added 2026-09-25 from an explorer-agent live capture, T-985)

A 2026-09-25 explorer-agent window-3 sweep of 460–470 MHz land-mobile/GMRS, 118–136.975 MHz airband and 152.4–162.4 MHz marine VHF (fixtures: `lmr-461p125-nbfm-ctcss`, `dmr-464p6125-bs`, T-985) found a real 8.1 s NBFM burst carrying a 233.6 Hz CTCSS tone and a strong bursty 4-level-FSK DMR repeater, both detected only as unclassified candidates with no tone field, no DMR sync search and no classification anywhere in the app; the same pass found the app already attaching an unmeasured "Aviation voice (VHF AM)" band-plan explanation to noise 52 times in 8 minutes, and misclassifying the whole marine VHF window `restricted-paging` — with no use case for plain airband or marine voice to test either finding against.

- `SIGNAL-090` **NBFM land-mobile/GMRS voice with blind CTCSS/DCS identification** — Blind-detect narrowband FM land-mobile voice bursts (Part 90 business, GMRS/FRS and similar) as time-frequency regions, and identify the sub-audible CTCSS tone or DCS code from the FM discriminator output alone, matched against the standard tone/code tables, without ever pre-tuning to a known repeater's PL tone. `460–467 MHz` · `HackRF/RTL-SDR` · [sigidwiki CTCSS](https://www.sigidwiki.com/wiki/CTCSS) [FCC Part 90](https://www.ecfr.gov/current/title-47/chapter-I/subchapter-D/part-90) [FCC Part 95](https://www.ecfr.gov/current/title-47/chapter-I/subchapter-D/part-95)
- `SIGNAL-091` **Conventional DMR (Tier II) blind 4FSK detection plus sync identification** — Blind-detect a bursty 12.5 kHz 4-level FSK land-mobile channel and identify it as DMR by finding the ETSI TS 102 361-1 BS/MS data and voice frame sync patterns at 4800 sym/s — the simpler conventional single-repeater case SIGNAL-082's trunked-system following does not cover. `450–470 MHz` (also VHF/700/800) · `HackRF/RTL-SDR` · [ETSI TS 102 361-1](https://www.etsi.org/deliver/etsi_ts/102300_102399/10236101/) [sigidwiki DMR](https://www.sigidwiki.com/wiki/DMR)
- `SIGNAL-092` **Airband AM voice (ATC/ATIS/AWOS)** — Blind-detect and demodulate AM voice on the civil aviation band, including scheduled ATIS/AWOS information loops, distinct from the ADS-B/UAT/ACARS/VDL2 data links SIGNAL-001–004 already cover. `118–136.975 MHz` · `RTL-SDR` · [sigidwiki Airband](https://www.sigidwiki.com/wiki/Airband)
- `SIGNAL-093` **Marine VHF voice** — Blind-detect and demodulate NBFM voice on the international marine VHF channels (Ch 16 distress/calling, working and duplex channels), distinct from the AIS/DSC digital signalling SIGNAL-015/017 already cover on the same band. `156–162.425 MHz` · `RTL-SDR` · [ITU/USCG marine VHF channel plan](https://www.navcen.uscg.gov/marine-vhf-channels)
