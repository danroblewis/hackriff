...
| C40 | `signal-relationships` | Assert that one detection is related to another rather than independent: **geometric receiver artifacts** (image `2*LO-f`, harmonics `n*f`, intermod `a*f1+/-b*f2` of strong emitters) predicted deterministically and attributed to their source, and **content-correlated multipath** (same RDS PI / ADS-B ICAO / audio cross-correlation) with the lag giving path difference. | Emitters + tuning history + decoded identity -> ranked `Relationship` with evidence | Low (arithmetic prediction); medium for correlation | Same idea as the ADR-0012 §2.6 DC twin rule. Geometric half folds into overlap dedup (T-219, M2/M3); content half rides on M3 fingerprints and MAUTO decode identity (T-222). User request 2026-09-15. |
...
`tx-experiments` (C37) is primary for 35 use cases but is deliberately out of the receive-first core: it is gated on half-duplex scheduling, and belongs to a later, opt-in milestone.
...
- **Jamming is out of scope even against your own devices.** RESEARCH-027 (RollJam: jam + record + replay) is `out-of-scope`. Capture-and-analysis of the same key fobs without jamming stays in scope (SIGNAL-047, RESEARCH-028 RollBack is replay-only).
