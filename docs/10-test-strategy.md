...
**A kind-2 test is usually a SPLIT, not a move (T-1036).** Most throughput tests measure something *and* assert something deterministic about the result; only the bound belongs in the tier. `hk-model::detection_write_throughput_is_reported` — 173 of the gate log's mentions, 9.6 s alone, 37.7 s with four agents on the box, 59.3 s at load 30 — became two tests over one shared measurement helper: `detection_writes_are_all_readable_and_the_rate_is_reported` gates (ADR-0006's dense batched writes are all readable afterwards, row for row through the region query, and the rate is computed from those rows and stated — 1.4 s on a quiet box, no wall clock asserted), and `the_detection_write_rate_clears_its_floor` is the tier's (the unchanged 1000 rows/s floor, run alone by `just timing`). The bound is not loosened, nothing is `#[ignore]`d, and the gate keeps a test that goes red if the reporting breaks — proved both ways before the split landed (a rate computed from the wrong denominator; a report line that stops stating the rate). Prefer this shape: ask what the bound is *besides* a number, and leave that in the gate.
