...
- **Receive chain (T-303):** `ChainKey` is `unknown` or one front end's `device_id` hash. A noise floor is a property of **one receive chain** — its antenna, cable, LNA and mixer — so two front ends at one site have genuinely different floors and must not share a baseline; without the chain their levels averaged and novelty fired, or was suppressed, on the mixture. The gain-state split does **not** cover this: `GainState::key` hashes LNA/VGA/amp only, so two identical front ends on their default gains produce the *same* gain key. `CalKey::Calibrated` separates devices only incidentally (a calibration version is per device); `CalKey::Uncalibrated` is one value for every uncalibrated front end, which is exactly where the pooling happened.
  ...
  - **End to end (T-314).** The key alone was not enough: the occupancy close read history with `Pyramid::query` (`OriginFilter::ANY`), so a per-chain key sat over a measurement that had pooled every front end in the pyramid. Every occupancy read — the 15-min close, `span_stats`, and the provenance-step read behind the alarm explanations — now goes through `engine::OriginLevels` under `engine::chain_filter(chain)`, the T-133 source filter built from the same `device_id` hash the key uses (`ChainKey::of_device` and `hk_store::history::source_key` are deliberately the same function, pinned by a test). The key and the measurement are therefore the same value by construction.
    ...
    - **What remains pooled (filed):** a level-0 cell's stored `occupancy` was decided at *ingest* against the pyramid's own tracked noise floor, which every front end feeds. A tile that is pure in origin can therefore carry an occupancy decided against another chain's floor, and `local_floors` then reads a dense neighbourhood as a **suspect floor**, whereupon `channel_level_excess` declines and the baseline folds nothing. The read-side filter cannot reach this; the fix is per-origin floor tracking at ingest. The failure direction is safe — the row withholds its excess rather than mis-stating it, and never reports the other chain's value — and it is pinned by `occupancy_ingest_time_floor_is_still_pooled_across_front_ends` (T-377).
