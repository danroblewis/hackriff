...
- **Mixed cell shapes are recorded, not rejected.**
  ...
  - **Mixed tiles use a Gamma-mixture bias (T-141).** Before T-141 they gave no `floor_db`, so every level-0 tile of the default-scheduler scene (72/72) fell back to the §2.2 80 % method.
    ...
    - *Measured (T-141, `hk-pipeline` `scheduler_history`, same 2 h scene).* With the scheduler, 72/72 level-0 tiles are mixed and all 72 report a floor (0 before). The pre-registered check (median level-0 `floor_db` over the central half of the span within 0.5 dB of the fixed-tune run) **fails at 1.01 dB**: scheduler −99.80, fixed tune −100.81 dB/Hz. The offset is upstream of the bias model.
      ...
      - Fix in progress (hk-core mock `dsp::Dequant`): a static linear-phase FIR designed from the first 65 k samples takes the recording's rounding noise out of rendered IQ. It keeps unit gain within ±3/64 of the recording rate of emissions, and the noise fill leaves out rounding noise. The mock unit test passes: floor and tone within 0.2 dB after same-rate retunes and at 2 Msps. `scheduler_history` improves to scheduler −100.19 vs fixed −100.81 (0.62 dB): most of the span now reads −100.8, but −125 to +25 kHz stays ≈ −99.8, the guard zones around emissions. The check still fails, and the threshold is not loosened. See the T-141 handoff for the next step (adaptive STFT-domain correction).
