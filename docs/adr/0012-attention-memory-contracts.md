...
- **Bias-tee state (T-333):** `BiasTee` is `unknown` / `off` / `on` and **completes the receive chain**. A bias tee powers an *external LNA* on the antenna port, so the floor and the gain structure move the instant the DC arrives — the same physics T-331 keyed the floor tracker's `GainKey` on. The comparison is therefore **declined, not disclosed-and-pooled**: the states do not share a key, so a capture with an active antenna powered is never scored against one without it. Disclosure alone would not do, because a disclosed-but-pooled comparison still **masks**: T-303 measured a real 12 dB rise scoring 1.000 against its own cohort and 0.000 against the pooled one, and T-333 reproduces exactly that (1.000, z 12.0 / 0.000, z −0.04) for a 25 dB bias-tee step. The failure is invisibility, not false alarms.
  - **`Unknown` is a cohort, not `Off`.** Compared exactly: unknown-with-unknown pools (one recording regime that recorded nothing is at least mutually consistent), unknown-with-`off` and unknown-with-`on` do not. Reading unknown as off would reintroduce one layer up the defect T-325 removed. The honest consequence is measured, not assumed: **every** baseline on disk and **every** fold today is `Unknown`, because nothing carries the state to the occupancy path yet — so "refuse anything unknown" would refuse everything, and the unknown cohort must go on comparing normally.
  ...
  - **Carried to the tile, not yet to the row (T-332).** Unlike the chain, this is not a service-lifetime value: a bias tee is switched *during* a run, so the state belongs to the measurement. T-332 laid that carrier — `FrameInput::bias_tee` → `FrontEndState::bias_tee` → `ProvenanceSummary::bias_tee` / `bias_tee_mixed` (tile format 5; v1–v4 read `unknown`, which is what they are) — and made a switch a `ProvenanceStep::BIAS_TEE`, listed in a report's `provenance_steps` as a `bias-tee` step (§6.3). The remaining hop is **T-359**: `VisitSample` takes its `gain_key` from `grid.provenance.dominant_gain_key()` at the point it can equally read `grid.provenance.bias_tee`, and carrying that into `SubjectContext` / `OccupancyStat` replaces the pipeline's `FOLD_BIAS_TEE = unknown` with the row's own value. Until then every fold is still `unknown` — which is what every stored baseline already carries, so nothing is orphaned.
