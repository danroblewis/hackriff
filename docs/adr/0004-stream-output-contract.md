...
- **Amended (T-388, the user's live-testing report): a stream may carry a *state extension* as well as content, and when it does, the extension is a measurement.** The `presence` stream ([stream contract §15](../stream-contract.md#15-the-presence-stream-live-box-growth-12-t-388)) publishes how far each open emitter's presence **has been observed**, so a live signal's box on the waterfall tracks the live edge instead of stepping forward once per 5 s poll. Three rules make that an extension of this contract rather than an exception to it:
  - **The stream says what was measured, never what is presumed.** The time published is the end of the last burst the detector measured, never a clock read. An emission that stops therefore stops extending. A client may **not** interpolate towards the live edge between records: a box drawn to now on the assumption a signal is still there is a claim about air nobody measured, and the reason this is a push rather than a client-side guess. This is the same honesty rule the coverage map applies to grey — "never looked" may not be spelled "looked and it was quiet".
  - **A push never creates, only extends.** Records address an emitter that already has an inventory row; the row is still created, arbitrated, merged, confirmed and *windowed* by the control/query API. The stream is a faster edge on a surface the API already owns, not a second inventory.
