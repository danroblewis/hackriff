...
**A fourth rule the decode surfaces add (T-384): re-deriving a window must not re-decode it.** §1.2's decode invariant — *live decoding extends the region's time extent and decodes only the newly-arrived part, never re-decoding what is already done* — constrains how the three rules above may be satisfied on an output or decode panel. Showing a past window's decoded fields is a **read of what was committed**, not a replay of the decoder: `GET /api/inventory/{id}/decode` gained `t0`/`t1` that select stored `Decode` rows by their own capture-clock `at`, so a scrub costs a query and no DSP. The window is applied *before* the latest-per-`(decoder, frame_model)` collapse, or a window holding only an older row would report nothing — rule 1's "all the data it has for that window", lost to an ordering. A design that answered a scrub by re-running the decoder over already-decoded frames would satisfy the display rule and break this one, and is explicitly not what the route does.

The same clock rule (2) had a second live violation here: the decode workbench's frame tallies windowed on `Date.now()`, against frame `t_ns` stamps that are capture-clock nanoseconds. Wrong-clock windowing does not always empty a surface — with the offset the other way it *widens* it, turning a sixty-second tally into an all-time one under a sixty-second label, which is rule 1 broken in the direction that looks healthy.
...
**A fourth consequence, added by T-362: the mapping must also be evaluated on the axis's own clock.** The user reported, from live testing the day T-337 landed, that the boxes *drift then jump*. Nothing above was wrong — the boxes were placed by capture time, through the rows' own timestamps. They were placed **on the ~1 s inventory poll**, while the waterfall scrolls every animation frame, so between polls a box stood still as the rows scrolled under it and the poll snapped it back. **One mapping evaluated on two clocks is two mappings**; the invariant held instant by instant and failed over time, which is the only way it can fail while still looking satisfied in a screenshot.

So (1) above is sharpened: *a box's placement is a pure function of its capture time **and of the view's current scroll state**, and it is computed where and when the rows are* — in the waterfall's own render pass, from the same ring head and the same zoom window, every frame. A box handed to the renderer is a region (band fraction × capture-time span) with **no screen coordinate**, so supplying one is a data update and never a placement; there is no stored pixel that a poll could be late in refreshing. Consequence (2)'s prohibition is unchanged and now load-bearing twice over: a nominal rows-per-second would look right for the first second after every poll, which is exactly long enough to survive the check that would catch it.

The line this draws for anything added later: **an overlay with a time extent is drawn in the render pass; an overlay without one stays in the DOM.** The presence box (§1.2) and a timed selection are the first kind; TM-4's focused-row band box and a frequency-only selection are the second, and neither can drift because neither has a time axis to sit on. Hit-testing follows the boxes rather than any drawn rectangle — containment in the same two axes — so interaction cannot become the second opinion this removes. UI: docs/14 "The boxes are drawn in the waterfall's render pass (T-362)".
...
**Consumed by the edge navigators (T-340).** Each waterfall axis carries a navigator parallel to it: the **time** bar down the side, spanning the retained capture window (§2.4.4, never the pyramid's longer horizon), and the **frequency** bar along the bottom, spanning the device-available spectrum. Each pans and zooms its own axis, and a region dragged on either zooms the main view to it — resolved through the grid above, so a region inside the tuned band is a display zoom that snaps to nothing. The frequency bar lights one segment per **reported** active capture window (`GET /api/navigation`'s `windows`, docs/07 §4.3), which is where several simultaneous front ends and survey coverage will appear without re-shaping anything. Setting the centre stays a **device action** (T-343): no *continuous* navigator gesture reaches the radio, at any pan distance.

**The asymmetry between the axes (T-392).** T-340 made a region dragged *outside* the tuned band an **offer** to retune, symmetric with a pan to the band edge. The user corrected that: *"selecting a region on it **retunes the front end to cover that region** (snapped to an achievable config), not merely offering a retune — this is the one navigator action that commands the radio, because unlike time (always a view over already-captured data) a frequency outside the current window can only be reached by tuning there."* The asymmetry is in the model, not the UI. **Time is a view over data that exists**: every instant on the time bar is inside the retained window by construction (§2.4.4), so a time gesture can always be answered from the ring or the pyramid. **Frequency is not**: no view state produces IQ the front end was never on, so "show me 432 MHz" is a request for a capture that has not happened, and the only way to answer it is to make it happen. The consequence for this ADR's grid: a region-select resolves to a whole realizable **configuration** — the snapped centre *and* the smallest `spans_hz` entry that covers the region from it — rather than to a centre alone, and it is applied rather than proposed. The one thing it may return instead is a refusal, and **only** when the grid admits no configuration at all: a centre outside every `frequency_ranges` band (on a replay, the recording's own extent, since a replay reports the recording as its band) or a span beyond `max_live_span_hz`. Anything the grid can realize retunes; over-refusal is the failure mode here, the mirror of over-claiming above. The gesture still builds a gated, audited `DeviceAction` with `device_id` recorded — the path is unchanged, only the trigger is new — and pan and wheel on either bar still reach nothing.

**The two bars control *different* axes (T-367).** The user's correction: the vertical bar is **time for the currently-selected frequency range** and the horizontal bar is **frequency across the survey**; wiring both to scrub time is a bug. This has one consequence for the contracts above and one for the client. For the contracts: the time bar's two inputs come from different places and stay independent — its **extent** is the capture window (§2.4.4), while its **picture** is that window folded over the frequency range the main view is on, which is `GET /api/timeline`'s existing `f_lo`/`f_hi`. The route already had the right shape (window server-side, band caller-side), so T-367 added no parameter; T-340 simply sent no band, and the route's honest answer to *which frequencies?* — `grid: null`, since an unscoped overview is a different measurement rather than a default — left the bar blank. For the client: each bar's gestures resolve to a type that can name only its own axis (`TimeTarget` / `FreqZoom`), applied by one writer each, so the other axis's state comes out of a gesture bit-identical. The frequency bar's *own* content beyond the lit segments — most-recent sample / occupancy across the surveyed spectrum — waits on the coverage map (T-368, the user's time/waterfall invariant 5, not yet an ADR section): painting unobserved spectrum as quiet is the same class of lie as an interpolated pixel.

**What this does *not* settle.** The capture timeline is §2.4.4. A zoom *inside* the tuned band is a display zoom over live IQ and correctly snaps to nothing — only a gesture that would become a capture state snaps. Wire form: `docs/api.md` "The achievable (centre, span) grid"; data model: docs/07 §4.3; UI: docs/14.

#### 2.4.4 The timeline is the capture window, and it is a visualization (T-338; the user's time/waterfall invariant 2)

§2.4.1 settled how a span is served and §2.4.3 what a window is evidence of. Both are about the *main* view. This is about the **scrubber**, and it is the one surface where getting the horizon wrong is invisible: *the scrubbable capture-history timeline spans exactly the configured recording/retention duration — no more (not the longer, lossy spectrum-history retention), no less — and grows or shrinks when that duration is reconfigured. It is itself a data display: a compressed "sideways" overview waterfall of the retained capture, never an empty box.*

Two consequences for this ADR's model.

1. **The scrubber's axis is the ring's window, and nothing else.** This device carries two retention horizons of deliberately different lengths — the lossless IQ ring ([ADR-0014](0014-iq-capture-ring.md), which owns it) and the lossy spectrum-history pyramid (§2.4.1). The scrubber is the ring's. A scrubber sized from the pyramid — or, as the client had it, from a 48 h constant — offers positions that resolve to times **no capture exists for**, and it draws them identically to positions that do. That is the same failure as §2.4.3's interpolated pixel, one level up: not a wrong number, a picture that implies data nobody has. The window is therefore served as its own object naming its own horizon (`GET /api/timeline`), and the client takes its span from that with no fallback.
2. **The band is a measurement surface, so the fold belongs in the backend.** §1.2's point is that ephemeral emissions are first-class; the scrubber is where a burst becomes visible *before* you scrub to it (TM-7's marks) and where you see that a stretch is worth scrubbing to at all. An empty frame wastes that. Drawing it needs a grid that is fine in time and coarse in frequency at once, which the pyramid's coupled ladder cannot give — so the tier is chosen on the time axis and folded to the drawn cells **here**, carrying only statistics that fold exactly (max of max-holds, max of peak occupancy, summed frames, mean coverage) and the grid's own observed range. A percentile cannot be folded from cell values, so it is not offered. `resolution.source` still says which tier drew it (never `live-iq`), beside `horizon` saying which retention it spans: two different questions with two different answers.

Wire form: `docs/api.md` "The capture window, and the overview drawn on it"; data model: docs/07 §4.1.1; UI: docs/14 "The timeline is the capture window".
...
### 6.4a The contract, in code (T-265, 2026-09-22)

§§6.2–6.4 are rules about a pipeline that does not exist yet, which is exactly the shape of rule that gets read once and then quietly broken by the first implementation. They are therefore written as types in `hk_pipeline::region` (**contract only** — no engine, no scheduler, no ring read; ADR-0015 §14 states the same contract for the analyze engine that will consume it):

- **`ReaderPolicy`** is `live-edge {max_backlog_ns}` | `bounded-region`, and it is **one field** of `PipelinePlan` and of `ReadLedger`. A pipeline cannot declare both, which is §6.2's ruling made structural rather than remembered.
- **`ReadLedger`** records the spans a reader *actually read*. Coverage is the union of those spans and nothing else, so **extent is not coverage**: eviction, a segment boundary and abandoned work all leave a flagged hole rather than being rounded up. `read` refuses a span overlapping one already read (invariant 5); `credit` refuses evidence from a span that was not read (§6.4).
- **`RegionJob::extend_to`** enqueues only `[old_end, new_end]` and answers `None` for an end already handed out, so the union of every work item it ever hands out is the extent, each sample exactly once.
- The **two fusions of §6.3 are two named errors**: `PlanError::AudioOnBoundedRegion` (audio would acquire the batch job's backlog) and `LedgerError::SkipForbidden` (a bounded-region reader taking the live-edge skip).
- **`RegionJob::hand_over`** is the only transition out of `RegionPhase::Region`, and it yields a `Handover {at, live_from, gap, abandoned_ns, flags}`. After it the job owns no samples; continuing means a **new** job, never the old reader re-pointed at the live edge.
...
| **TM-10** | **Decode region extension (Rules L/I, §6).** The incremental contract for bounded-region pipelines, the two-reader rule, and the explicit handover state. | A region job's coverage is exactly the samples it read, and audio stays live throughout. | **Blocked on the analyze engine.** `POST /api/analyze` answers `501` and MAUTO is unscheduled (ADR-0015). Attempting this earlier means building an incremental scheduler for a pipeline that does not exist. §6 is a **contract now**; code when MAUTO is scheduled. **The contract half landed (T-265, 2026-09-22):** [ADR-0015 §14](0015-decoder-synthesis-contracts.md#14-amendment--the-incremental-region-decode-contract-t-265--adr-0017-tm-10-2026-09-22) and `hk_pipeline::region` — `ReaderPolicy`, `PipelinePlan`, `ReadLedger`, `RegionJob`, `RegionPhase`/`Handover` — with no engine, no thread and no ring read. The engine half is still blocked. |

**Waves** (≤ 4 Rust builders, per CLAUDE.md T-144): (1) TM-1; (2) TM-2; (3) TM-3, TM-4; (4) TM-5, TM-7; (5) TM-6, TM-8; (6) TM-9. TM-10's **engine** is unscheduled; its **contract** landed with T-265 (§6.4a, ADR-0015 §14).
