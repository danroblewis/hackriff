...
- **Revival appends, never duplicates.** A returning signal that entity resolution places on the same emitter gets a **new interval on the same `emitter_id`** — automatically, because `emitter_observation` is keyed by `(source_kind, source_id)` and a new track is a new source row. This is the "revives rather than duplicates" requirement, and it comes free.
...
**Unverified in this ADR:** the `idle_gap` value (it must come from the detector's revisit period, not be dialled in); that the interval-overlap query stays interactive over a full retention window; that co-onset/co-offset actually separates skirt fragments from adjacent stations on the user's own 99.6 MHz scene; and the claim that TM-3 alone resolves most of T-251 (TM-9 measures it).
...
4. **`idle_gap`.** Derived from the detector's revisit period (proposed), or a per-band setting? It decides whether a chatty burst source reads as one interval or fifty.
