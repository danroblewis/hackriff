...
**The weak-carrier rule, decided here (T-869; the NOAA finding) — and it is *not* behind the flag.** At 162.4008 MHz — a permanently-keyed NBFM carrier at ~5 dB SNR — Listen answered `4422 no-analog-mode` because C13's OBW99 abstained at low SNR. The refusal was honest and the wrong product answer. The chooser now demodulates a **narrow** selection (≤ 50 kHz) with **measured** energy (a measured `snr_box_db` ≥ 3 dB) as NBFM with the squelch armed from the measured noise power — silent until the channel rises, never a refusal — while carrying the probe's own (zero) mode confidence, so the header never claims a mode was recognised. Noise, where the estimator's presence test fails and nothing is measured, is still refused: LP-1 §6's `4422` holds, in both modes. The chooser runs on **every** request, flag or no flag — there must not be two places a mode is decided — so this is the one way the default-off opener differs from before stage 4, deliberately: it is a product decision (a weak NFM channel is listenable), not a consequence of the switch. It is covered by the chooser's own unit tests on the rule's arithmetic; an opener-level test needs a weak-NBFM fixture and is follow-up work.
