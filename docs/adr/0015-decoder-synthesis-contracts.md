...
| Stopping | Solved (ADR-0022's inequality on hold-out: ≥ 24 analytic bits, a check ≥ 8 bits wide carrying ≥ 16 bits after `L_check`; the frame count is derived per job), budget, plateau, beam exhausted, cancelled. There are always ranked partial results. |
...
| Confirm-by-decode | New `ConfirmPolicy.synthesized` rule (§5.5, derived by ADR-0022 §6): hold-out validation, ≥ 24 analytic hold-out bits each net of its own stage's look-elsewhere, `check_bits = width × differences − L_check ≥ 16`, check width ≥ 8, null control for a searched check, and a monitored decision rate. Decodes are provenance-marked `synthesized`. |
...
- **New rule** `ConfirmPolicy.synthesized` (actor `hk-pipeline/confirm-synth@2`; derived in [ADR-0022](0022-false-confirm-budget.md) §6, applied by T-860/T-575) confirms a candidate only when all of these hold, in order:
  ...
  2. The check is at least `min_check_width` = **8** bits wide — an **assumption**, not a derivation (T-577 measures it).
  3. `check_bits = width × differences − L_check ≥ hard_check_floor_bits` (**16**): a confirmation always carries a check, never sync excess alone. `differences` is the chance-corrected count — frames valid **without** FEC correction (T-210), each payload counted once however often it repeats, short-period payloads not counted — never `distinct_valid`, which over-credits a beacon repeating its payload. An evaluator's larger figure is clamped to that bound, and the excess comes off the analytic total.
...
| M-9 | Attach (`emitter_synthesis`), synthesized decode provenance, `ConfirmPolicy.synthesized` with hold-out — the gate as derived by ADR-0022 §6 (analytic hold-out bits ≥ 24 with per-stage look-elsewhere, `differences` not `distinct_valid`, check width ≥ 8 and a 16-bit hard check floor, the laundering rule, the §8 decision-rate counter; T-860 + T-575) | M-8 | Opus (core) | W |
...
- **Promotion is the lifecycle event.** `status → promoted` triggers the §5.5 `ConfirmPolicy.synthesized` check; if it passes and the emitter is a `candidate`, `change_emitter_lifecycle` (`repo/lifecycle.rs:172`) confirms it with a backend-rendered reason naming the pipeline and the arithmetic. The thresholds are §5.5's, as derived by ADR-0022 §6: **check ≥ 8 bits wide; `check_bits = width × differences − L_check ≥ 16`; analytic hold-out bits ≥ 24**, each analytic stage net of its own look-elsewhere; a searched (or discovered-template) check needs the null control passed; plus front-end trust; `quick` never confirms. The frame count is derived per job, not stored. (The BCH row of ADR-0022 §4.2's table: 10 parity bits, template-fixed, needs 3 codewords; searched, 4.)
...
| CP-4 | Promotion → `ConfirmPolicy` wiring: the ADR-0022 §6 hold-out rule (analytic hold-out bits ≥ 24, `check_bits` ≥ 16 over `differences`, width ≥ 8, null control for a searched or discovered check), corrected-group exclusion (T-210), one promoted row per `output_kind` | CP-3 | Opus, core_interface | CP-W |
