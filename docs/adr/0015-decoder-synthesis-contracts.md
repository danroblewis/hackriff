...
- **New rule** `ConfirmPolicy.synthesized` (actor `hk-pipeline/confirm-synth@2`; derived in [ADR-0022](0022-false-confirm-budget.md) §6, applied by T-860/T-575) confirms a candidate only when all of these hold, in order:
  ...
  2. The check is at least `min_check_width` = **16** bits wide — **measured** by T-577 (ADR-0022 §4.3.1), not assumed. Three degenerate nulls reach the gate below it: zero-padded **shifts** of one no-code burst (g ∣ P ⇒ g ∣ x^k·P) clear it at every width ≥ 8 on one 2⁻ʷ event, 78× the budget at w = 8 and inside it only at 16; and a payload repeated at lag ord(x mod g) passes a 4-bit check with certainty. The floor returns to 8 only when §4.3.1's count (below) is re-measured by T-577's harness, and never goes below 8.
