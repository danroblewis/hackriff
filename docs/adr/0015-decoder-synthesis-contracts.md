...
| Stopping | Solved (ADR-0022's inequality on hold-out: ≥ 24 analytic bits, a check ≥ 16 bits wide (T-577) carrying ≥ 16 bits after `L_check`; the frame count is derived per job), budget, plateau, beam exhausted, cancelled. There are always ranked partial results. |
...
| Confirm-by-decode | New `ConfirmPolicy.synthesized` rule (§5.5, derived by ADR-0022 §6): hold-out validation, ≥ 24 analytic hold-out bits each net of its own stage's look-elsewhere, `check_bits = width × differences − L_check ≥ 16`, check width ≥ 16 (T-577), null control for a searched check, and a monitored decision rate. Decodes are provenance-marked `synthesized`. |
...
- **Stored decodes.** The rank-1 candidate at `Validate` runs over the hold-out window with `messages` outputs through the T-111 ingestion path.
  ...
  - Each Decode row carries `provenance: {kind: synthesized, job_id, holdout: true, evidence_bits, hypotheses, analytic_holdout_bits, check_bits, l_check, check_searched, template_provenance}` (docs/07 §2.15 delta; ADR-0022 §11.3), so a confirmation's arithmetic is reconstructible from the stored row.
...
- **New rule** `ConfirmPolicy.synthesized` (actor `hk-pipeline/confirm-synth@2`; derived in [ADR-0022](0022-false-confirm-budget.md) §6, applied by T-860/T-575) confirms a candidate only when all of these hold, in order:
  1. The rank-1 verdict is `solved` on hold-out, and the profile is not `quick`.
  2. The check is at least `min_check_width` = **16** bits wide — **measured** by T-577 (ADR-0022 §4.3.1), not assumed. Below it, degenerate nulls reach the gate: zero-padded **shifts** of one no-code burst (g ∣ P ⇒ g ∣ x^k·P) clear it at every width ≥ 8 on one 2⁻ʷ event, 78× the budget at w = 8 and inside it only at 16; and a payload repeated at lag ord(x mod g) passes a 4-bit check with certainty. §4.3.1's count shipped and **was** re-measured by that harness (T-921, ADR-0022 §4.3.2): it closed both artefacts, including the searched half the null control cannot see, but a frame holding two *different* counted bursts (`x^a·A + x^b·B`) is valid by construction and costs 0.31–84 × the budget at w = 8 against 0.0013 × at 16, so the floor **stays at 16**. It never goes below 8.
  3. `check_bits = width × differences − L_check ≥ hard_check_floor_bits` (**16**): a confirmation always carries a check, never sync excess alone. `differences` is the chance-corrected count, never `distinct_valid`, and since T-575 it is ADR-0022 §4.3.1's count: a frame valid **without** FEC correction (T-210) adds a trial only if it is not **degenerate** — short-periodic as a whole *or with up to w register bits trimmed from either end*, which closes the init-cancel frame `1^w ‖ 0…0` (and `init ‖ 0…0 ‖ xorout`) that no floor closes — and its zero-trimmed polynomial is neither a multiple nor a divisor of one already counted (a zero-padded shift, a doubled copy or a repeat of one burst is one trial); the count is then capped at the GF(2) affine rank + 1 of the frames, per frame length. An evaluator's larger figure is clamped to `width × differences − L_check`, and the excess comes off the analytic total; `width` and `differences` are read off the same check block.
  4. `analytic_holdout_bits ≥ min_analytic_holdout_bits` (**24** = 14.3 bits of budget + 9.7 of stated model margin): the sum over the winning prefix of the **analytic-null** stages only (`check_distinct_valid`, `sync_excess`, `field_fit`, `identity_recurrence`), each **net of the look-elsewhere charged at its own stage**. The calibrated metrics (§2.2's second list) rank, prune and are reported; they pay for no confirmation. The job-total `coverage.look_elsewhere_bits` (ADR-0021 §7A.2) is a reporting field and **never** an input — it would double-charge, and make a squitter stop confirming because the job searched a busy window elsewhere.
  5. A **searched** check — an open search, or a template an earlier search **discovered** — needs ADR-0021 §8.2's null control to have run and not capped. A discovered template inherits its discovery's look-elsewhere (`discovery_look_elsewhere_bits`, recorded at save-as-template) as part of `L_check` at every use, and a row whose `L_check` falls short of it is refused (ADR-0022 §5.1, the laundering rule). Only `builtin` or user-authored templates that fix the whole check are template-fixed (`L_check = 0` beyond the S5 stage's own count).
  6. Front-end trust holds: ≤ 50 % suspect detections, at least one detection of the emitter in the window, and the IQ known not overloaded.

  There is **no** `min_evidence_bits` (the 64 was the wrong currency) and **no** `min_distinct_valid`: the frame count is derived per job, `min_differences = max(1, ⌈(24 + L_check) / width⌉)` — 1 for a template-fixed CRC-24, 3 for a searched CRC-16 (ADR-0022 §4.2's table). A missing number refuses.

  **The decision-rate counter** (ADR-0022 §8). The threshold delivers the budget only at the assumed `assumed_decisions_per_week` = 20 000. Every evaluation of the rule is counted, durably, over a rolling seven days on the device's clock; above the assumed figure the **budget claim is void** and every decision says so (`confirm.decision_rate.budget_claim: void`, and a sentence in the reason). The thresholds do not move with the count — a running per-session look-elsewhere term is the rejected alternative, because it would make identical evidence worth less on a device with longer uptime.

  The lifecycle reason is backend-rendered and names the arithmetic, e.g. "decoded by synthesized pipeline `generic-fsk-framed`: CRC-16 (searched), 3 differing frame(s) valid on hold-out without correction, 48.0 − 21.0 = 27.0 check bits, 30.0 analytic bits against a 24-bit threshold; null control passed with a 11.4-bit margin".
...
- **Derived by [ADR-0022](0022-false-confirm-budget.md) (T-548; applied by T-575).** The gate above replaces this section's original 64 bits / 3 distinct valid frames / width 16, which were guesses on a one-way door. The numbers are ADR-0022's, derived from the user's budget of one wrong Confirmed emitter per unattended week; 24 and 16 are derived, the width floor is T-577's measured 16 (8 only after §4.3.1's count is re-measured by T-577's harness), and none of them may be loosened except by an explicit ADR-0022 amendment (T-547 can buy recall that way, never soundness). §10 open question 1 is **settled** by ADR-0022 §4.2 plus U1 = B.
...
| M-9 | Attach (`emitter_synthesis`), synthesized decode provenance, `ConfirmPolicy.synthesized` with hold-out — the gate as derived by ADR-0022 §6 (analytic hold-out bits ≥ 24 with per-stage look-elsewhere, `differences` not `distinct_valid`, check width ≥ 16 (T-577) and a 16-bit hard check floor, ADR-0022 §4.3.1's count (init-cancel guard, multiples of one burst counted once, affine-rank cap), the laundering rule, the §8 decision-rate counter; T-860 + T-575) | M-8 | Opus (core) | W |
...
- **Promotion is the lifecycle event.** `status → promoted` triggers the §5.5 `ConfirmPolicy.synthesized` check; if it passes and the emitter is a `candidate`, `change_emitter_lifecycle` (`repo/lifecycle.rs:172`) confirms it with a backend-rendered reason naming the pipeline and the arithmetic. The thresholds are §5.5's, as derived by ADR-0022 §6: **check ≥ 16 bits wide (T-577); `check_bits = width × differences − L_check ≥ 16`; analytic hold-out bits ≥ 24**, each analytic stage net of its own look-elsewhere; a searched (or discovered-template) check needs the null control passed; plus front-end trust; `quick` never confirms. The frame count is derived per job, not stored. (A 10-parity-bit BCH codeword is under the 16-bit floor until §4.3.1's count is re-measured.)
...
- Single-burst behaviour follows §5.5: one frame confirms only through the one inequality — in practice a template-fixed check of ≥ 24 bits (U1 = B; ADR-0022 §4.2). §10 open question 1 is settled.
...
| CP-4 | Promotion → `ConfirmPolicy` wiring: the ADR-0022 §6 hold-out rule (analytic hold-out bits ≥ 24, `check_bits` ≥ 16 over §4.3.1's `differences`, width ≥ 16, null control for a searched or discovered check), corrected-group exclusion (T-210), one promoted row per `output_kind` | CP-3 | Opus, core_interface | CP-W |
...
| **4. Opener switch, flag-gated (default off)** — *landed, T-869; see the stage-4 note below* | `HK_LISTEN_PIPELINE=1` makes `/ws/open/listen` run chooser → ephemeral pipeline. Stage 0's conformance test runs in **both** modes. | Identical audio and UI with the flag on; instant revert by unsetting it. **This is the stage the user live-tests.** |
...
**Stage 4 implemented (T-869 = LP-5, 2026-09-25).** The chooser is `hk_pipeline::audio::choose`: a **pure function of the probe** (`crates/hk-pipeline/src/audio.rs`) that answers `recipe` (ranked by `hk_recipe::matching::rank`, the `/api/recipes/match` arithmetic, so `freq_hz` carries weight zero and only a `Fit` no second candidate ties is taken), `legacy` (the plan today's chain runs — no recipe fits, or none exists for the mode, or `channels=2`, since a recipe's `audio` output is mono, §12.13) or `refuse` (`422 no-analog-mode`). With `HK_LISTEN_PIPELINE=1` (default off; `ListenManager::set_pipeline_audio` is the per-opener seam a test uses instead of the process environment) `/ws/open/listen` serves a chosen recipe as an **ephemeral pipeline**: `owner: "session"` in `GET /api/pipelines`, seeded with the measured channel *and* with `agc.enabled` from the plan (FM is scaled by deviation, so the chooser turns the recipe's AGC off for it — the "seed parameters" half of §12.2), reporting **what was measured** in the header (mode, rules version, confidence, SNR, parameters, refinement, `provenance_ref`, `squelch.noise_dbfs`) rather than the recipe's declaration, and stopping when its last listener leaves. **Attach, don't duplicate** (§12.3) lives in `recipes/session.rs`: a second listener on a station attaches to the running pipeline's `audio` output — one DDC, one row — and a listener leaving never stops an `explicit` pipeline. A recipe path that cannot start for a reason that is not the client's falls back to the legacy chain rather than inventing a refusal code. Tests: `crates/hk-pipeline/tests/listen_pipeline_switch.rs` plus the chooser's own unit tests.

**The weak-carrier rule, decided here (T-869; the NOAA finding) — and it is *not* behind the flag.** At 162.4008 MHz — a permanently-keyed NBFM carrier at ~5 dB SNR — Listen answered `4422 no-analog-mode` because C13's OBW99 abstained at low SNR. The refusal was honest and the wrong product answer. The chooser now demodulates a **narrow** selection (≤ 50 kHz) with **measured** energy as NBFM with the squelch armed from the measured noise power — silent until the channel rises, never a refusal — reporting **mode confidence zero** and a `mode_rules` marked `+weak-carrier`, so the stream never claims a mode was recognised (the selector's own confidence is its confidence in *unknown*; reporting it beside `mode: nbfm` would claim what nothing measured). Such a plan never picks a recipe either: with no measured family there is nothing to rank one against, and ranking on an assumed family would invent the evidence the ranking exists to weigh. Noise, where the estimator's presence test fails and nothing is measured, is still refused: LP-1 §6's `4422` holds, in both modes. The chooser runs on **every** request, flag or no flag — there must not be two places a mode is decided — so this is the one way the default-off opener differs from before stage 4, deliberately: it is a product decision (a weak NFM channel is listenable), not a consequence of the switch.

**Which measurement — the correction the review forced (2026-09-25).** The first cut asked for a measured `snr_box_db` and was **dead code**: that estimate abstains for *any* band reason, and "no occupied band" is exactly the abstention that produced the refusal, so the rule could never fire on its own trigger case. `hk_pipeline::audio::BoxEnergy` reads what C13 **does** publish when the band estimate gives up: its **presence statistic** (the band stage attaches it to `obw99_hz` as evidence only once the presence test has passed — so its existence is the estimator's own "something is here", at its own threshold, with no new one invented), the occupied bandwidth the mode selector measured **between adjacent emissions** when C13 abstained (T-099), and the strongest line's SNR (each SNR judged against the 3 dB floor; the presence statistic is not re-judged). Proved both ways over a probe of synthetic IQ in `hk_pipeline::audio`'s own tests — a line-free emission filling a 20 kHz drag's box has **no** `snr_box_db` and **is** demodulated, and the test goes red on the first cut; noise measures nothing and is still refused — and through the real opener in `listen_pipeline_switch.rs` §7.

**What stage 4 does NOT yet deliver, measured.** Running LP-1's freeze with `HK_LISTEN_PIPELINE=1` leaves **1 of 5 tests green** (the refusal codes; the header/record, retune and re-plumb ones fail), for two reasons worth stating before stage 5's default flip:
1. **The audio clock.** On the recipe path `sample_index` counts the frames the sink published, so a closed `squelch` is fewer samples rather than the flagged **jump** LP-1 §5 freezes, and `sample_index == 960 × (frames + squelched_frames)` (§4) does not hold. This is §12.12's "unverified" squelch item, now observed. Deriving the index from capture time was tried and **rejected with evidence**: it makes both conformance assertions pass but injects a 20 ms splice into audio that is continuous, dropping T-868's legacy-versus-recipe correlation from 0.96 to **0.62** — so the fix belongs in the audio branch's capture-time bookkeeping, not in the sink's arithmetic.
2. **An attached listener joins an ongoing stream**, so its first record need not be `sample_index` 0 (it is flagged). That is what attach-don't-duplicate *means*, and the freeze was written for a fresh per-consumer chain. The stereo test's mono/stereo header comparison likewise compares a pipeline header (with §12.4's four additive keys) against a legacy one.

Listener accounting is Listen's throughout: the opener's admission slot is held for the attachment's life, `listen.running`/`attached`/`detached` count sessions whichever path serves them, and a pipeline that ends on a re-plumb or a retune is counted `retune_ends` + `closed_segment`/`closed_retune`, not "the client went away" (T-633's rule, applied to the pipeline path).
...
- **ADR-0022's gate.** `min_analytic_holdout_bits = 24`, `hard_check_floor_bits = 16`,
  `min_check_width = 16` (T-577, measured; re-measured on the shipped count by T-921 and kept
  at 16 — ADR-0022 §4.3.2, never below 8) and §4.2's frame formula all stand untouched. None reads a calibrated
