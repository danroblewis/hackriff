...
**Proposed amendment (T-858 = M-7, 2026-09-24) — PENDING THE USER; not accepted.** `hk_synth::objective` implements §2.3 as written with one exception and one reading, both forced by measurement and both for the user to accept or reject (the coordinator raises it). Until then the code carries them as provisional:
- **Proposed exception — S0 is reported but not counted in `quality` or the lock.** Every calibration null is white noise at the prefix's input rate fed to its own S0 filter, and the tables are tight (`lowpass@1` credits 6 bits at +0.1 dB, n = 16 384). Measured: a channeliser whose passband rolls off at the band edges made pure noise score 6 bits at S0 — so S0, a whiteness test of the objective's own channel output, is not evidence about the emission. (A prefix reaching only S0 is refused.)
