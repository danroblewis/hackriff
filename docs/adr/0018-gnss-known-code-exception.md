...
- `hk-gnss` is **not wired into `hk-pipeline`**. Nothing schedules an L1 dwell, so the crate is currently reachable only from its own tests. This is deliberate: wiring is where leakage risk lives, and it belongs to a follow-up task that can be reviewed on its own terms.
...
**PROVISIONAL.** Confirmed by a spike or by the user, on two points: that the crate boundary (rather than a lighter in-crate mechanism) is the right cost, and that leaving the crate unwired is the right staging for opening M5.
