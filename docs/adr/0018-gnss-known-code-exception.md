...
- **Wired by T-322, around the boundary rather than through it.** `hk_pipeline::gnss` is the one place in the product that names both `hk_gnss` and the pipeline, and it is not on the blind path. C04 is offered a recurring `ScheduledDwell` at L1 — a centre, a rate and a duration, nothing GNSS-shaped from the scheduler's side — and **only when the run's own scan plan already covers L1**, which is the gate rather than a flag. Applying a granted step arms the `hk-gnss` reader for that window; the reader takes one contiguous window off the ring, down-converts L1 to baseband and runs `acquire`. `assess_jamming` turns the result plus in-band power into a verdict, which crosses into `hk-context` as `GnssServiceEvidence` — **scalars and a verdict, no codes** — and `hk_context::gnss_service` appends it as an `Explanation` (`Cause::OwnHistory`, `Evidence::Value` rows) to anomalies **the blind path already opened**. It has no branch that opens one. No `hk-detect → hk-gnss` edge appeared; §3's guard stays green and gained a clause: neither carrier file may name a `Detection`, an `Emitter`, an inventory or `hk_detect`, so the exception cannot reach the inventory by the second door either.
  - A consequence worth recording, found by the wiring and **fixed in the crate by T-414**: `AcquisitionConfig`'s old default `threshold_ratio` of 2.5 was a bare peak-to-mean ratio sized for a short profile, but the search size is the rate's business — code-phase cells are the samples per 1 ms period (2046 at the minimum rate, 4000 at 4 Msps, 20 000 at 20 Msps), times the Doppler bins. Left fixed it acquires **all 32 satellites out of pure noise**, which would report an intact constellation on every dwell and silently disable the jamming assessment. **A capability that always says "fine" is worse than one that is absent, because absence is visible** — so the fix is both a sound default and a refusal, not either alone. `AcquisitionThreshold` replaces the bare ratio: `FalseAlarm(p)` (the default, `1e-4` per satellite) derives the bar inside `hk-gnss` from the search's own geometry, so no caller can get it wrong; `PeakToMean(r)` stays available for experiments but `acquire` **refuses** a ratio noise would clear more often than not (`AcquireError::ThresholdUnsound`), because a default is a value someone can override without noticing while a refusal is a conversation. The cell count deliberately treats every code phase × Doppler bin as an independent look — an over-estimate, since neighbours are correlated — so the derived bar is an upper bound; a correlation-discounted count measured about 5× optimistic against the real correlator. Controls: noise-only IQ at 2.046, 4 and 20 Msps yields **zero** acquisitions at the default while the old 2.5 would have taken **32 of 32** off the same profiles, and T-322's e2e still recovers planted PRNs 7/11/19 by number from IQ ~20 dB under the floor.
