...
- **The output/decode panels are views over the window too (T-384), and a surface with no way to ask is the API's bug.** `GET /api/inventory/{id}/decode` carried **no time parameter**, so the RDS/decoded-summary panels could not have been window-scoped however they were wired: scrubbed back, they kept rendering the live edge's fields under a past window's heading — *the wrong window's data*, which is the harder half of "we have it but didn't render it" because it looks correct. The route now takes `t0`/`t1` like every other windowed one, the panels send `viewWindow()`, and `decode/plots.ts`'s three `Date.now() * 1e6` frame windows became that same window on the capture clock.
  - **Route choice, settled rather than assumed.** `/api/inventory/{id}/decode` is **emitter-keyed and serves committed decode fields**; `/api/captures/{id}/frames` is **capture-keyed and serves raw stream records**, and already has `from_t`/`to_t`. The store has no emitter→capture key, and re-assembling an RDS station name from raw group records client-side would be a thin-client violation *and* a re-decode, so the window went on `/decode` and the captures route kept the frame-level scrubbing it was built for (which is what the workbench's tallies now use).
  - **Re-deriving never re-decodes.** A `Decode` row is what the decoder already committed, with its own capture-clock `at`; a window filters stored rows, so ADR-0017's incremental-decode invariant holds with no decoder running. The filter is applied **before** the latest-per-frame-model collapse — the other order reports "nothing decoded here" for a window that holds an older row.
  - **A panel subscribes to `windowKey`, not to `s.time`.** Watching only the cursor leaves a mounted panel answering about the window it was mounted in while the live edge advances or a capture window arrives; one exported definition of "the window moved", so two surfaces cannot disagree about when it did.
...
| 9 | Selection "Watch: alert on new activity" | **Region watch**: a selection-scoped alarm rule raising anomalies and stream messages for new activity in its extent | hk-context, hk-api | large — **done**: T-166 (`hk_context::watch` rule, `watch` on a Selection, `GET /api/selections/{id}/watch`, alerts on the `anomalies` stream; never alerts on a row the T-219 rules record as suppressed, duplicate or artifact) |
