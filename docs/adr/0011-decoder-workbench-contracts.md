...
| Frame length | Fixed, or variable: `length_from` (a field decides it, e.g. the ADS-B DF → 56/112) and `terminator` (a closing word, e.g. ACARS ETX, plus trailer bits; `reopen` lets `sync_search` open the next frame on it, T-1054), with `frame_bits` as the maximum; shared by `sync_search` and `ppm_demod` (§1.5). |
...
| framing | `sync_search` | bits → frames | pinned: `mode` (`sync-word`/`offset-words`) + per-mode keys; sync-word: `frame_bits` (fixed, or the maximum), `bit_order` (`lsb` reverses 8-bit characters), `polarity` (`normal`/`either`: the complemented word also syncs and complements its frame; T-108), `length_from`, `terminator` (`terminator.reopen`, T-1054, **default `false`**: after a terminator the sync search restarts; `true` = shared-flag HDLC — the closing word also opens the next frame, with the polarity it synced on — for AIS/AX.25 on a continuous line, where a noise-born flag would otherwise spend a real burst's opening flag closing a junk frame; needs `trailer_bits: 0`, no `include_sync`, and `step_bits` a multiple of 8 with `bit_order: lsb`; `sync_search` only — `deframe`/`ppm_demod` refuse it) |
...
- `messages`: Decode messages (§5.1), also stored as Decode rows. The `decode` mapping names the frame model, an identity field (feeds the Emitter identity, docs/07 §2.11), metadata field paths and content field paths.
  - **Mapping** (T-111, T-1017): `{frame_model, identity?: {scheme, field, format: hex | dec}, identity_label?: {field, confidence?: {field, from: field | vote-counts, meaning: vote-share | crc-valid-rate | decoder-score}}, metadata[], content[], require[], service?}`. Validation checks the scheme (a known scheme, or a token read as `other:<scheme>`), the format, that at least one field is mapped, and that `service` is a token. Values are the fields' decoded values; keys are the last path segment (the whole path when two mapped paths share it). The identity takes its scheme's canonical form (`adsb-icao` 6 lower-case hex digits, `rds-pi` 4 upper-case). **`identity_label` (T-1017) is how a recipe DECLARES the identity's human-readable name** — the field the list surfaces serve as `identity_label`, with its confidence and that confidence's declared meaning (`docs/stream-contract.md` §9.1, `docs/api.md`). It is opt-in and there is no inference from field names: a recipe that declares none is served none. Validation requires `decode.identity` (a label names an identity) and requires `field`/`confidence.field` to be `metadata` paths of this same mapping whose stored key is unambiguous — a recipe can never declare a field its writer does not store.
...
**Status:** PROVISIONAL. **Implemented by T-866 (ADR-0015 LP-2, 2026-09-24):** §8.2 (the `audio` output kind, `audio/<pipeline>/<output>`, the additive header keys inside the `audio` object), §8.3 (the pre-attach gate at start and at an edit that keeps audio; the validation rule), §8.4 (the four blocks, `crates/hk-blocks/src/blocks/audio/`), §8.5 (`input.liveness`) and §8.6 (schema 3, with version 2 still read). **§8.7 implemented by T-870 (LP-6, 2026-09-24):** `refine.objective` is exactly one of `{builtin}`, `{node, metric, goal}` or T-858's `{evidence}` (a mix is a validation error on `refine.objective`); `builtin` is schema 3, names a registered objective (`wfm-pilot`), needs an iq input on one channel, and runs in `hk_pipeline::recipes::refine` on a per-pipeline worker with Listen's settings and hysteresis; every accepted result is applied as an ordinary hot edit (ADR-0015 §12.6: refined centre → the channel, refined bandwidth → `input.bandwidth_hz`, every node rebuilt at a chunk boundary). The `{node, metric}` form is still validated but not run. **Not yet:** §8.8 listener-budget admission (LP-8, T-872 — an audio pipeline is admitted as a `recipe` chain until then), §8.9's built-in recipe (LP-3, T-867). Two implementation facts the text below did not fix: `squelch` also takes a hot `noise_dbfs` (the `snr` mode's reference at its own port; absent, it stays open — Listen's rule without a noise estimate), and the FM noise squelch must precede de-emphasis (§8.9's worked example therefore de-emphasises in a `deemphasis` node after `squelch`, not in `fm_demod`). `throughput` keeps the runtime's 2 s ring-protection skip that every pipeline already had; it never seeks *for latency*. Source: [docs/15 §10](../15-decoder-synthesis.md) ("Listen is just a decode pipeline with an audio sink"), written by the user after live testing. §§1–7 stand unchanged. This section adds what a recipe needs in order to *be* an audio decoder; [ADR-0015 §12](0015-decoder-synthesis-contracts.md) says how today's Listen path migrates onto it without going dark.
...
|---|---|---|---|
| `squelch` | real → real | `mode` (`snr`/`fm-noise`), `open_snr_db`, `hysteresis_db`, `attack_s`, `hang_s`, `noise_dbfs` (T-866: the `snr` reference) — all hot | `hk_demod::audio` squelch (C19 §"Squelch") |
...
- **Deliberately not invented here.** There is **no `ssb_demod` and no `cw_demod`**, and Listen serves `usb`/`lsb`/`cw` today. Writing them properly (carrier estimate, raster snap, clarifier — C19 §"SSB carrier") is real DSP work this amendment does not fund. Their absence is exactly why the migration is **per-mode** (ADR-0015 §12.5) rather than all-at-once. **Stereo** was also out here: `stereo_decode` would need a block *and* a wire change (`channels`, interleaved `ri16_le`). **Superseded 2026-09-23:** the user wants stereo (docs/20 U5); ADR-0015 §12.13 puts it in scope as LP-9 (block) and LP-10 (wire change, mono unchanged).
...
**T-938: the shipped WFM recipes declare the builtin, and only `center_hz`.** §2.2's worked example uses the `{node, metric}` form, and `recipes/rds.recipe.json` / `recipes/analog-wfm.recipe.json` shipped that way — but that form is *validated and not run* (T-870 registered `{builtin}` only), so every RDS chain the explorer agent attached on 2026-09-25 stayed on its detection centre, 10–25 kHz off the carrier (98.925 MHz for a station at 98.900). Both recipes now declare `{"builtin": "wfm-pilot"}` (and schema 3). `tune` lists **`center_hz` alone**, deliberately: on a strong station the objective's bandwidth search legitimately picks ~123 kHz, which is fine for audio and fatal for RDS — the 57 kHz subcarrier is outside it. A recipe whose chain needs the whole multiplex refines its centre and keeps its declared `input.bandwidth_hz`.

**Lock on a tracked pilot, not only on C/N₀ (T-938).** `WfmObjective` locked on pilot C/N₀ ≥ 40 dB-Hz, and `accept_update` never applies an unlocked outcome. A real but weak station therefore kept its biased centre although the loop had already found the carrier: on the 98.900 MHz capture the pilot tracks at 18999.85 Hz ± 0.01 Hz with C/N₀ 35.4 dB-Hz. So a **validation** measurement whose stereo PLL tracks the pilot — present, within `pilot_search_hz` of 19 kHz, uncertainty under 1 ppm — locks down to a `lock_pilot_cn0_floor_dbhz` of 30 dB-Hz. Measured on the same captures: a carrier tracks its pilot at 35–62 dB-Hz, while off-carrier and empty spectrum reach 27 dB-Hz and track no pilot at all, so the floor does not admit noise.

**Why the detection centre is biased in the first place.** A detection's `f_center_hz` is the power-weighted centroid of the CFAR excess over the component's bins (`hk_detect::detector::build_record`). Over a 200 kHz WFM multiplex at 2.4 Msps that centroid is not the carrier: the instantaneous FM spectrum is programme-dependent and asymmetric over a short dwell, and the component's edges take in the neighbour's skirt and (in a hot band — these captures are 12 % and 98 % clipped) front-end IMD. A few percent of asymmetry across 200 kHz is the 10–25 kHz observed. The fix is not a better first guess but the closed loop above (CLAUDE.md "tune from the processed output").
...
**Status:** PROVISIONAL, contract only when written (T-606); since then `descramble` (T-608), `psk_demod` (T-609), `viterbi`/`viterbi_frames` (T-610) and `reed_solomon` (T-611) are implemented with their parameters pinned. No ADR status changes. Source: the T-554 disposition audit, [docs/18 §§6–8](../18-decoder-coverage.md) (§7 is the ranking behind every row here; §8 is the delta this section absorbs). §§1–8 stand unchanged. §8 added the audio sink; this section adds the blocks that 25 of the audit's 36 native families are waiting on, and answers the one real contract question they raise.
...
| symbol | `bitstuff` | bits\|frames → same | `flag` (0x7E), `stuff_after` (5), `direction` (`destuff`/`stuff`), `abort_ones` (7), `bit_order` (`msb`/`lsb`, frames and `destuff` only: octets sent LSB first — T-963; `lsb` with `stuff` is refused, T-1054, since the reversal runs after the rule) | T-613 |
...
| fec | `reed_solomon` | frames → frames | **pinned by T-611:** `n`, `k` (a shortened code is `n < 2^m − 1`: DVB RS(204,188), P25), `symbol_bits` (default 8), `poly` (primitive field polynomial, required), `fcr` (required), `prim` (default 1, coprime with `2^m − 1`), `dual_basis` (CCSDS Berlekamp basis; field `0x187` only), `depth` (interleaved codewords, 1–255: CCSDS I, DAB+ s), `strip`, `span` (added: the coded part of the frame, as `viterbi_frames`), `drop_invalid` (hot). Errors only; one or more `depth × n`-symbol code blocks per frame; the channel bits it changed go into `FrameInfo::corrected_bits` (the wire's `fec_corrected_bits`), symbol counts into status extras | T-611 (**implemented**) |
...
| `reed_solomon` | **nothing: native** (`fec/rs.rs`: GF(2^m) tables, Berlekamp–Massey, Chien, Forney, the CCSDS dual basis from its trace definition). T-607 measured liquid-dsp's Reed–Solomon to be a `libfec` wrapper, absent without `libfec` | **yes**, in-repo (T-611) |
...
- **T-875 added `psk_demod`'s burst mode (2026-09-24)**, the M-14 residue ADR-0015 §10 names: `burst: true` (cold, default `false`, so the streaming contract above is unchanged). The block finds each burst by energy, confirms it blind by its M-th-power line, measures carrier frequency and phase, symbol timing and level **feed-forward over the burst's own samples**, and tracks it from its first sample. It emits only burst symbols, and **each burst's first item starts an output chunk flagged `DISCONTINUITY`**, as §9.2's burst-boundary rule asks. Since flags and the time map are per chunk, one output chunk carries at most one burst: a second burst ready in the same call waits for the next call, and one that cannot get a chunk before `END` or a FIFO overrun is counted in `bursts_dropped`. The status reports `bursts` in place of the streaming path's `samples_per_symbol` and `symbols`. The measured before/after (shortest burst decoded whole: none up to 2 048 symbols → 8–64) and the algorithms are in `crates/hk-blocks/src/blocks/iq/psk_burst.rs`.
...
- **T-612 landed `mlevel_slicer` (2026-09-23).** Parameters pinned as in §9.1; `crates/hk-blocks/src/blocks/symbol/mlevel.rs` is normative. The levels are equally spaced, so `auto` estimates two numbers (lowest level, spacing) by equal-spacing k-means over the last `window` symbols, re-fitted every `window/8` symbols on the symbol count since restart (chunking-invariant), so C4FM deviation and DC drift are tracked, not hand-set. The fit doubles as the eye measurement in *both* modes: `quality = 1 − 4σ/s` and `lock` once a full window reads ≥ 0.3, so an input with no M-level structure (one constant level, noise) still emits k bits per symbol for the time map but reads `quality 0`, `searching`. Two additions to the sketch: `mapping: table`, because the P25/DMR/NXDN C4FM dibit map (−3 → `11`, −1 → `10`, +1 → `00`, +3 → `01`) is neither Gray nor natural nor a mirror of either; and `bit_order`. `invert` mirrors the levels (for `levels: 2` it is exactly `slicer`'s `invert`, tested).
