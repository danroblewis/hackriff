...
| fec | `parity` | frames → frames | pinned (T-087; `zero` T-096): `unit_bits`, `parity`, `position` (first/last), `span`, `strip`, `zero` (replace the parity bit with 0 in place instead of removing it — unit width unchanged, for a check field computed over the pre-parity data padded back to the unit width; exclusive with `strip`), `drop_invalid` (hot) |
...
- **ACARS** (T-096 tuned this against the T-098 synthetic fixture; no `acarsdec` oracle exists yet, see [docs/tutorials/03-acars.md](../tutorials/03-acars.md)): `am_demod` → `subcarrier` (1800 Hz) → `msk_demod` → `clock_recovery` 2400 Bd → `slicer` (no `diff_decode`: MSK mark/space maps straight to bit 1/0, verified bit-exact in `crates/hk-blocks/src/blocks/iq/tests.rs::acars_path_am_subcarrier_msk_recovers_hidden_bits`) → `sync_search` (`SYN SYN SOH` = 0x160116 over 24 bits, no `bit_order` reversal — the synthetic generator sends each character MSB, i.e. its parity bit, first — terminator ETX 0x83 / ETB 0x97 at 8-bit steps + 16 trailer bits) → two branches: `unwrap` (`crc`, real parity bits kept) → `fields` (7-bit + odd parity characters); and `pz` (`parity`, `zero: true`) → `crc` for the block-check status. The block check is CRC-16/XMODEM (poly 0x1021, init 0, not reflected) over the *pre-parity* data padded back to 8 bits with the parity position forced to 0 (`py/hkpy/synth/acars.py` `crc16_acars`), not over the as-sent bytes — `parity`'s new `zero` mode (as opposed to `strip`) reproduces that convention without disturbing the real parity bits `fields` needs (`crates/hk-blocks/src/blocks/fec/tests.rs::parity_zero_then_crc_matches_pre_parity_check_t096`).
