...
- **Gain is a second policy over the same one front end (T-945).** Automatic front-end gain management ([docs/28](../28-front-end-gain-management.md)) converges on the gain state whose *processed output* is best, by probing: each probe is a dwell, so a gain run spends the same attention this scheduler allocates, and each probe is one gated `DeviceAction::Gains`, so it cannot interleave with a hop. Two consequences bind here: a gain run must be **scheduled as a dwell of its own** rather than fired from a detection callback (not yet built — the policy is off by default and driven by its caller), and detections made mid-run are made under deliberately-wrong gains for part of it, which the existing provenance/overload machinery already marks rather than needing new suppression.
