...
type LayerId =
  ...
  | "tune"            // overlay— the DEVICE's own route: the recorded retune history (T-898)
...
- **`tune` (T-898, docs/23 §10.6 rule 2)** is the same primitive turned on the radio itself: an `overlay` layer at `z` 26, just above the measured paths, drawing **the device's own route through frequency** — a dwell as a vertical run, a retune as a vertex — from `GET /api/tune-history` (`hk_store::tunepath` over the tune records the coverage map is already built on). **One line per front end**, in its own ink, named by `device_id` in the layer's key (the several-SDRs rule); `unknown` is its own route. A gap no record covers **breaks** the line rather than being drawn across, the honesty of the coverage grey applied to the route. It strokes through the same `ui/src/surface/paths.ts` machinery, so the byte-identical guard covers it unchanged.
...
- **Default visible:** `base`, `coverage`, `detections`, `paths`, `tune`, `rules`, `pins`, and every collection whose stored `visible` is true. **Default hidden:** `tier` badges, `artifacts`, `priors`, `research`. Unknown and Candidate detections are **never** hidden by default, and any "explained-only" filter is an explicit, reversible opt-in that says it is hiding data.
...
**Amended 2026-09-25 (T-1028): `retune mode`.** The user asked for a mode in which panning or zooming retunes: *"For areas that are too large and can't be tuned, use the largest possible size instead of denying them … hold down a certain key while panning to retune."* The sentence above therefore holds **with the mode off**, which is the default and is what every existing empty-call-list test exercises. With the mode **on** — turned on deliberately, and visible while it is (a lit chip, a banner, a per-pane status line; `R` tapped latches, `R` held is one gesture) — a pan/zoom that **settles** issues **one** retune through the same gated path, latest settled view winning, a request in flight never cancelled. The mode is what the amendment makes discrete and explicit; the gesture inside it is not, and that is the whole of the change. A view wider than one capture window **narrows to the largest achievable span centred on it** rather than refusing (clamped into the tunable range at the edges), with the pane still showing the wider view and the coverage fog stating which part was captured; the remaining refusal is a view with no overlap at all with the tunable range. Nothing in the mode touches time, the ring or detection. The client computes the narrowed window from the grid `GET /api/navigation` already reports and posts the ordinary `POST /api/control/window`, so **no route gains an option** and `docs/api.md` is unchanged.
...
| **Navigation = view arithmetic in time + a device command in frequency** | §4: the whole new control vocabulary — pan, wheel, pinch, pause, scrub, split, follow, layer toggles, sheets, menus, tool modes, every authoring action, every research write — leaves the spy-client call list **empty** (MAP-25) **with retune mode off, which is the default**. Two exceptions, both through the gated `DeviceAction` path: the discrete press on the retune offer, and (T-1028) a settled pan/zoom while retune mode is explicitly on. §5 refuses saved views an exemption from either. |
