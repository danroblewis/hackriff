...
**Landed (T-576), and where each assertion lives.** The suite is `tests/e2e/tests/acceptance/mauto_false_confirm.rs` in the `acceptance_mauto` binary: `false_confirm_budget` (A1 + A2, over T-568's N1/N2/N4 populations through the mock SDR at `standard` and `deep`), `false_confirm_recall_control_*` (A3), and the degeneracy detectors that prove each can go red. Four things it fixes in the text above, none of them a change of decision:

- **The `18.0` is scaled, per docs/22 §3, not frozen.** The suite asserts `max ≤ log₂(n_actual) + 7.4` and reports both, so a shrunken negative population is judged *harder* rather than reading green with no power. `18.0` is exactly that at docs/22's frozen n = 1600.
- **Targets include an ad-hoc band, not only emitters.** N1 must produce **no** emitter (§10.1's own `emitters_created == 0`, and T-568's rule), so an emitter-only suite would make **zero** confirm decisions over thermal noise and pass by having asked nothing. The band comes from `GET /api/iqbuffer`'s segments — the window the device reports it sampled — never from truth.
- **The illustrative block's two numbers do not follow from each other.** "missed by ~137×" and "~2.4 wrong Confirmed emitters per week" are inconsistent: the threshold is set to deliver one wrong row per week, so a factor of 2^(M − realised margin) *is* the wrong rows per week (≈ 137, not 2.4; the nominal 1-per-840-weeks figure times the total optimism 2^16.8 gives the same 137). The suite prints the derived factor with the chain shown.
- **A2 is armed, not measured, until a search backend exists.** Nothing implements the engine's `Evaluator` over acquired IQ (`hk_pipeline::synth::jobs::server_backend()` is `None`), so every negative job acquires its window and ends `failed / no_evaluator`: there is no rank-1 hold-out result, `n = 0`, and the report says A2 **is not measured** instead of printing a reassuring maximum of zero. A1's zero is likewise a zero over **zero searched decisions**, which the report states in those words. The assertions bind, unchanged, the moment a backend searches.

**The n ≈ 60 000 question, answered (T-576).** **Worth building, as a milestone/nightly batch on a quiet box (`just timing` class), and not yet.** At the ~1–3 s a negative region-analyze job costs, 60 000 decisions is 17–50 h serial and 3–8 h at the six-way concurrency the e2e group already runs at — a real cost but not an unreachable one, and two orders of magnitude outside a 20-minute merge gate, so never in CI. It waits on the search backend for a reason that is not cost: while nothing searches, a negative *job* is not a negative *decision*, and 60 000 jobs ending `no_evaluator` would bound nothing at all while producing a very convincing number — the docs/17 §1 four-decimal error with a bigger `n`. And it buys the **rate** claim (A1) only: the margin claim is A2's, which docs/22 §3 shows is better served by n = 1600 plus the tail slope. The two must not be conflated when someone decides to spend the hours. The corpus declaration for the full-n N1 run is therefore `UNFILED` rather than pointing at a closed ticket.
