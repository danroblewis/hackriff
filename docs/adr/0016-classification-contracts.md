...
- **Computation** (`hk_model::classify::fuse`, pure):
  1. `u = open_set_score`. Unknown posterior = `max(u, likelihood[unknown])`. Priors never touch it. **Amendment (T-953, 2026-09-25):** that value is capped at `MAX_UNKNOWN_CONFIDENCE` = 0.9 (strictly below a family's 0.999) before step 2, because the χ² tail saturates to 1.0 for anything the dev-grid densities never saw and `unknown` read 0.999 on live FLEX bursts. The cap bounds the reported number only: `open_set_score` stays uncapped on the row, a saturated open set still wins the label, priors still never touch it, and the posterior still sums to 1. At `u = 1.0` the classifier's known likelihoods are all 0, so the residual ≥ 0.1 is spread uniformly and ranks nothing. Consequently the §3 invariant "posterior with a uniform prior equals the likelihood" holds wherever `L[unknown] ≤ 0.9`.
