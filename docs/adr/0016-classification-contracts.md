...
- **Computation** (`hk_model::classify::fuse`, pure):
  1. `u = open_set_score`. Unknown posterior = `max(u, likelihood[unknown])`. Priors never touch it. **Amendment (T-953, 2026-09-25):** that value is capped at `MAX_UNKNOWN_CONFIDENCE` = 0.9 (strictly below a family's 0.999) before step 2, because the χ² tail saturates to 1.0 for anything the dev-grid densities never saw and `unknown` read 0.999 on live FLEX bursts. The cap bounds the reported number only: `open_set_score` stays uncapped on the row, a saturated open set still wins the label, priors still never touch it, and the posterior still sums to 1. At `u = 1.0` the classifier's known likelihoods are all 0, so the residual ≥ 0.1 is spread uniformly and ranks nothing. The same cap bounds an abstention the classifier builds outside the fusion (§4.4 amendment T-970 B): one constant, one rule. Consequently the §3 invariant "posterior with a uniform prior equals the likelihood" holds wherever `L[unknown] ≤ 0.9`.
...
*Amendment (T-970, 2026-09-25): a pre-classification rule, and a cap on what an abstention may claim.*

**A. The broadcast-FM pilot rule** (`hk-classify/src/fm.rs`). Steps 2–4 score a snippet against class-conditional densities over feature-vector *shape* statistics, every one of which is gated on C13's in-band SNR of the whole channel. A broadcast FM station carries something far more specific: a **19 kHz stereo pilot** and, where it transmits RDS, a **57 kHz suppressed-carrier subcarrier**, both in the FM discriminator's output. The explorer's live 2026-09-25 window measured stations with a locked pilot and a CRC-valid RDS decode classifying `unknown` at 0.999, because C13 measured no in-band SNR at all or one under `analog`'s 10 dB gate — and §2's "not measured is not ruled out" then rightly moved the mass to `unknown`. The gate is about the densities; a tone's detectability is set by the integration time, not by the channel's SNR.

So a rule runs **beside** the gate, on its own measurement, and supplies a **likelihood** that the existing fusion and abstention machinery then treat like any other, rather than short-circuiting them as a hard-coded family would. Its residual mass goes wholly to `unknown`: a 19 kHz pilot says nothing about whether an emission is FSK or PSK, so the alternative to "this is a broadcast multiplex" is "the measurement misled me and I do not know what this is". That also keeps the measurement safe from the band plan — §3 reads dominance off the **runner-up known family**, which is then 0, so a disagreeing prior is always tempered back. A prior explains a measured station; it never relabels one. (Spreading the residual over the other families instead was a defect found in review: a pilot-only call at 0.90 : 0.10 is 9:1, *inside* the 10:1 a prior may reorder within, and the family could move off `analog` while the class stayed `wfm` — a row `Classification::validate` rejects.) The within-family class follows the **reported** family and never the rule, for the same reason. It fires only on all of: a station-shaped C13 OBW (90–340 kHz); an FM broadcast RMS deviation; and a 19 kHz line whose **false-alarm probability** against its own local null is ≤ 1e−9, with the null's χ² tail corrected for every bin searched. The claim is then **priced by that measurement**, held under a ceiling (0.97 with a corroborating 57 kHz or L−R band, 0.90 with the pilot alone) that prices the step from the tone to the label, which no χ² tail can measure.

Its safety property is a **harmonic-suppression test**, and it is physics rather than tuning: an emission whose discriminator output is periodic (a repeating chirp, a Costas hop) produces a harmonic comb whose teeth fall as `1/n`, so the strongest of a tooth's own 2× and 3× teeth sits ~6 dB below it whatever the SNR — measured 5.6–5.8 dB across 4–25 dB on the dev grid — while a multiplex, whose 38 kHz and 57 kHz subcarriers are suppressed by construction, reads 11.9–22.8 dB. Nothing in the rule reads a band plan, a channel raster or a frequency: it fires identically on a station 150 kHz off raster, and the allocation stays an explanation offered afterwards by C17. A **mono** station has no pilot, so the rule stays silent on one — `None` means "this rule has nothing to say", never "not a station".

**B. `MAX_UNKNOWN_CONFIDENCE` = 0.9.** §4.4's abstention conditions are unchanged, but the number an abstention *reports* now is. `unknown` used to reach `MAX_CONFIDENCE` (0.999) whenever no family scored at all — "100 % unk" on the explorer's screen — which states more certainty about the world than any positive call is allowed to. An abstention exists because something could not be measured, so the residual above the cap is moved to the families the tree left **admissible**: precisely those that were not ruled out and could not be measured. Nothing here turns an abstention into a family, and the relative order below `unknown` is preserved, so §8's MAUTO seed ranking is unaffected. **One rule with §3 step 1 (T-953, same day):** the constant is the one `hk_model::classify::MAX_UNKNOWN_CONFIDENCE`, and the rule is simply that **`unknown`'s reported posterior never exceeds 0.9**, wherever it is computed. The fusion (§3 step 1) applies it first, and there the residual follows the likelihood's order (uniform at `open_set = 1.0`); the classifier's hold is the same cap applied to any abstention it builds outside the fusion, with the excess going to the admissible families as above. Neither changes which label wins, and `open_set_score` stays uncapped on the row.
