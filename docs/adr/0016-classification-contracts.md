...
So a rule runs **beside** the gate, on its own measurement, and supplies a **likelihood** that the existing fusion and abstention machinery then treat like any other (it is not a verdict, and a C17 prior may still argue with it — it simply dominates by more than §3's evidence ratio). It fires only on all of: a station-shaped C13 OBW (90–340 kHz); an FM broadcast RMS deviation; and a 19 kHz line whose **false-alarm probability** against its own local null is ≤ 1e−9, with the null's χ² tail corrected for every bin searched. The claim is then **priced by that measurement**, held under a ceiling (0.97 with a corroborating 57 kHz or L−R band, 0.90 with the pilot alone) that prices the step from the tone to the label, which no χ² tail can measure.
