...
//!
//! No inference engine is a dependency, in the default build or otherwise, so this crate costs
//! CI and the Jetson cross build nothing until T-203 chooses one at the bake-off. [`CpuReference`]
//! therefore answers [`MlError::NotImplemented`] to every load: a caller that asks for inference
//! today is told so, rather than silently getting a fabricated answer.
...
/// compared against).
///
/// **Stub.** It reports its identity and that it is not yet conformant, and refuses every load
/// with [`MlError::NotImplemented`]: no engine is a dependency until T-203's bake-off chooses one.
/// A consumer therefore falls back to the classical stage, which is what it must do whenever a
/// provider is unavailable anyway.
...
impl MlProvider for CpuReference {
    ...
    fn load(
        ...
        Err(MlError::NotImplemented(
            "hk-ml is the ADR-0016 §6 contract; T-203 lands ONNX loading and the conformance suite",
        ))
    }
}
...
mod tests {
    ...
    #[test]
    fn the_cpu_reference_reports_itself_and_refuses_to_load_until_t203() {
        let p = CpuReference;
        assert_eq!(p.kind(), MlProviderKind::CpuTract);
        assert!(
            !p.conformant(),
            "an unconformant provider is never used for a decision (ADR-0007)"
        );
        match p.load(&manifest(), &[]) {
            Err(e) => assert!(matches!(e, MlError::NotImplemented(_)), "{e}"),
            Ok(_) => panic!("the stub provider must not pretend to load a model"),
        }
    }
