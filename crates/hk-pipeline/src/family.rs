...
fn rank_all(
    ...
    // T-980: the emitter row carries no analysis resolution, so the CW-line call is refused
    // here (`None` is "resolution not known"); it is made where the track's `bin_hz` is known.
