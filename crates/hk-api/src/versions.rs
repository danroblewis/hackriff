//! `GET /ws/changes` — the **route-version feed** (T-1065): the server says *which route's answer
//! changed*, so a client stops asking.
//!
//! **Not [`crate::changes`].** That module (T-1040) pushes `coverage_changed` on `/ws/tiles/changes`
//! — one *derived* event, computed from the tune record, about the tile lattice's coverage. This one
//! is about **HTTP routes**: a counter per route that a write advances, so a client can re-read the
//! handful of bodies that moved instead of polling all twelve. They are different questions on
//! different sockets, and a client wants both: the tile surface follows `coverage_changed`, the
//! panels follow these versions.
//!
//! # Why
//!
...
//! # The contract
//!
//! Each route in [`Versioned::ALL`] has a **version**: a counter, per server process, that the server
...
//! that a connection samples every [`VERSION_TICK`] (250 ms). Twenty writes inside one tick advance the
...
//!
//! Those are producer-side events. [`VersionFeed::bump`] is public precisely so a producer can
...
//! count is capped at [`MAX_VERSION_FEEDS`] per server. An idle feed sends **no messages**; it sends
...
// T-1040 put the shared `101 Switching Protocols` handshake and the send/close/liveness helpers in
// `rows`; this route is the third caller (`/ws/tiles/rows`, `/ws/tiles/changes`, `/ws/changes`) and
// uses them rather than hand-rolling a second upgrade.
use crate::rows::{FeedSlot, accept, ping, refuse, send};
...
pub const VERSION_TICK: Duration = Duration::from_millis(250);
...
pub const MAX_VERSION_FEEDS: usize = 8;

/// One route with a version. The variant's [`Versioned::path`] is the **path a client fetches**, so
...
impl Versioned {
    ...
    /// [`VersionFeed`]'s table, which is why it is written out rather than derived.
    pub const ALL: [Versioned; 12] = [
        Versioned::ControlState,
        Versioned::Navigation,
        Versioned::Coverage,
        Versioned::TuneHistory,
        Versioned::Timeline,
        Versioned::Recordings,
        Versioned::Inventory,
    ...
    pub const fn path(self) -> &'static str {
        match self {
            Versioned::ControlState => "/api/control/state",
            Versioned::Navigation => "/api/navigation",
            Versioned::Coverage => "/api/coverage",
            Versioned::TuneHistory => "/api/tune-history",
            Versioned::Timeline => "/api/timeline",
            Versioned::Recordings => "/api/recordings",
            Versioned::Inventory => "/api/inventory",
            Versioned::Annotations => "/api/annotations",
            Versioned::Paths => "/api/paths",
            Versioned::Outputs => "/api/outputs",
            Versioned::Pipelines => "/api/pipelines",
            Versioned::Frontend => "/api/frontend/events",
        }
    }

    /// The variant for a route key, or `None` for a route the feed does not carry.
    pub fn from_path(path: &str) -> Option<Self> {
        Versioned::ALL.into_iter().find(|c| c.path() == path)
    }
    ...
    fn slot(self) -> usize {
        ...
        Versioned::ALL
            .iter()
            .position(|c| *c == self)
            .expect("every variant is in Versioned::ALL")
    }
}
...
///
/// A path that maps to nothing bumps nothing: the feed carries [`Versioned::ALL`] and says so, and a
...
pub fn routes_for_write(path: &str) -> &'static [Versioned] {
    use Versioned::*;
    ...
    const DEVICE: &[Versioned] = &[ControlState, Navigation, TuneHistory, Coverage];
    match path {
        "/api/control/center"
        | "/api/control/rate"
        | "/api/control/window"
...
fn prefixed(path: &str) -> &'static [Versioned] {
    use Versioned::*;
...
/// The version table: one counter per [`Versioned`], shared by cloning the [`std::sync::Arc`] on
/// [`ApiState`].
///
...
pub struct VersionFeed {
    versions: [AtomicU64; Versioned::ALL.len()],
    /// Open `/ws/changes` connections, against [`MAX_VERSION_FEEDS`]. Counted with T-1040's
    /// shared [`crate::rows::FeedSlot`], which is why it is an `AtomicUsize` and not a slot type of
    /// its own: `/ws/tiles/rows`, `/ws/tiles/changes` and this route all cap the same way.
    feeds: AtomicUsize,
}

impl VersionFeed {
    /// This route's current version. `0` until its first write.
    pub fn version(&self, c: Versioned) -> u64 {
    ...
    pub fn versions_json(&self) -> Value {
        ...
        for c in Versioned::ALL {
            m.insert(c.path().to_owned(), json!(self.version(c)));
        }
        Value::Object(m)
    }

    /// The counter [`crate::rows::FeedSlot`] takes a slot from.
    pub(crate) fn slots(&self) -> &AtomicUsize {
        &self.feeds
    }

    fn all(&self) -> [u64; Versioned::ALL.len()] {
        Versioned::ALL.map(|c| self.version(c))
    }
}
...
fn versions_message(feed: &VersionFeed) -> Value {
    json!({
        "type": "versions",
        "routes": feed.versions_json(),
        "tick_ms": VERSION_TICK.as_millis() as u64,
        "t_s": now_s(),
    })
}

fn changed_message(c: Versioned, version: u64) -> Value {
    json!({
        "type": "changed",
        "route": c.path(),
        "version": version,
...
pub(crate) fn serve(stream: TcpStream, state: &ApiState, headers: &[(String, String)]) {
    let Some(mut ws) = accept(stream, headers) else {
        return;
    };
    let feed = &*state.versions;
    let Some(_slot) = FeedSlot::take(feed.slots(), MAX_VERSION_FEEDS) else {
        return refuse(
            ws,
            &ApiError::new(
                503,
                format!("{MAX_VERSION_FEEDS} version feeds are already open on this server"),
            ),
        );
    };
    ...
    loop {
        if !crate::rows::peer_alive(&mut ws, VERSION_TICK) {
            break;
        }
        let now = feed.all();
        let mut said_something = false;
        for (i, c) in Versioned::ALL.into_iter().enumerate() {
            if now[i] != seen[i] {
                seen[i] = now[i];
...
pub fn validated(path: &str) -> bool {
    path == Versioned::ControlState.path()
}
...
mod tests {
    ...
    fn every_route_is_a_real_get_route_and_keys_are_unique() {
        for c in Versioned::ALL {
            assert!(
                ROUTES.iter().any(|(m, p)| *m == "GET" && *p == c.path()),
                "{:?}: {} is not a GET route the server serves",
                c,
                c.path()
            );
            assert_eq!(Versioned::from_path(c.path()), Some(c));
        }
        let mut keys: Vec<&str> = Versioned::ALL.iter().map(|c| c.path()).collect();
        keys.sort_unstable();
        let n = keys.len();
        keys.dedup();
        assert_eq!(keys.len(), n, "route keys must be unique");
        assert_eq!(Versioned::from_path("/api/status"), None);
    }

    #[test]
    fn slots_are_distinct_and_within_the_table() {
        let mut slots: Vec<usize> = Versioned::ALL.iter().map(|c| c.slot()).collect();
        slots.sort_unstable();
        assert_eq!(slots, (0..Versioned::ALL.len()).collect::<Vec<_>>());
    }

    #[test]
    fn a_bump_moves_exactly_one_version() {
        let feed = VersionFeed::default();
        for c in Versioned::ALL {
            assert_eq!(feed.version(c), 0, "{c:?} starts unwritten");
        }
        assert_eq!(feed.bump(Versioned::Inventory), 1);
        assert_eq!(feed.bump(Versioned::Inventory), 2);
        assert_eq!(feed.version(Versioned::Inventory), 2);
        for c in Versioned::ALL
            .into_iter()
            .filter(|c| *c != Versioned::Inventory)
        {
    ...
    fn collection_writes_map_by_prefix_including_their_ids() {
        assert_eq!(
            routes_for_write("/api/annotations"),
            &[Versioned::Annotations]
        );
        assert_eq!(
        ...
        assert_eq!(routes_for_write("/api/pipelines"), &[Versioned::Pipelines]);
        assert_eq!(
            routes_for_write("/api/pipelines/p1/channels"),
            &[Versioned::Pipelines]
        );
        assert_eq!(routes_for_write("/api/recipes/r1"), &[Versioned::Pipelines]);
        assert_eq!(
        ...
        // without the feed.
        assert_eq!(routes_for_write("/api/paths/p1"), &[Versioned::Paths]);
        assert_eq!(
    ...
    fn only_control_state_is_validated_today() {
        ...
        for c in Versioned::ALL
            .into_iter()
            .filter(|c| *c != Versioned::ControlState)
        {
            assert!(!validated(c.path()), "{c:?} is not validated yet");
        }
    }

    #[test]
    fn the_feed_route_is_in_the_table_and_is_not_t1040s() {
        assert!(
            ROUTES
                .iter()
                .any(|(m, p)| *m == "GET" && *p == "/ws/changes")
        );
        // T-1040's coverage feed is a different route on a different question; neither replaces the
        // other, and both are served.
        assert!(
            ROUTES
                .iter()
                .any(|(m, p)| *m == "GET" && *p == "/ws/tiles/changes")
        );
    }
