...
//!   default). Bands narrower than half a span are offset-tuned off DC. Dwell-only regions
//!   contribute long region windows instead of sweep hops. The pass visits hops by region
//!   priority, then frequency, and repeats.
...
//!   weighted round robin on `interestingness × region priority`.
...
//!   clock harmonics. [`Verification`] pairs the captures and feeds the trust tests through
//!   [`TrustEvaluator`], skipping gain-step inference when either block clipped.
...
//! - **RF paths:** every step carries `rf_path`; sweep hops never straddle a path boundary, so
...
//! ([`SyntheticClock`] in tests, [`WallClock`] live). [`Scheduler::next_step`],
...
pub use core::{Poi, ScheduleStats, Scheduler, SchedulerError, TxSlotRequest, UserIntent};
pub use plan::{
