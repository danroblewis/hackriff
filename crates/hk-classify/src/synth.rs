...
///
/// **Only `2fsk`, because the other three do not share the bias (T-908, measured).** On dev
/// seeds 0–23 at 25 dB, C14's `cyclic_db` over the first 64 / 112 / 256 symbols against the whole
/// record, medians: `gfsk` 36.5 / 37.1 / 35.6 against **29.4**, `msk` 35.9 / 37.4 / 38.7 against
/// **30.1**, `4fsk` 37.7 / 39.6 / 42.4 against **28.4**. A short packet of those classes reads
/// 7–11 dB *higher* than the record the density was fitted on — the opposite sign to the wide-
/// deviation `2fsk` burst above, and the negative length slope `features::symbol_features`
/// records for all four classes (T-327). So the same window would not correct a bias there; it
/// would move three densities by an amount no device scene has measured, and pooling C14's window
/// at fit time is the held-out-recall trade T-364 left open for the user. They keep their whole
/// records until a device measurement says otherwise.
fn packet_window(
    view: Vec<Complex32>,
    view_rate_hz: f64,
    record_s: f64,
...
mod tests {
    ...
    fn the_2fsk_symbol_view_is_a_packet_of_drawn_length() {
        ...
        for seed in 0..40 {
            ...
            assert!(
                view_s <= record_s * 1.01,
                "seed {seed}: view {view_s} s of {record_s}"
            );
