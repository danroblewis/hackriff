...
/// ADR-0022 §4.3.1's independent-trial count for a check stage, and its degenerate-frame
/// guard. They live in [`hk_model::synth::tally`], below `hk-estimate`, because the searched
/// path (`hk_estimate::assist::codes`) and T-577's measurement harness must run **this** code
/// and not a copy of it (T-921); the `crc` / `bch` / `parity` / `checksum` blocks reach them
/// through this module, where the rest of their evidence lives.
pub(crate) use hk_model::synth::tally::CheckTally;
