...
//!
//! **No modulation-structure feature (T-594).** T-233 added `structure` (the emission's noise-
//! corrected envelope fourth moment, ± σ) as evidence that could only split. T-594 found it had
//! no producer: over real replays it was on 0 stored fingerprints and so took part in 0
//! comparisons (`hk-pipeline`'s `fingerprint_field_census`). It was removed rather than wired
//! because every producer that holds IQ also writes an exact `family`, and every family those
//! producers write is constant-envelope (`wfm`, `nfm`, the FSK chain, the four-level trunking
//! control channel), for which the statistic reads 1.00 by construction — so a wired comparison
//! could split only on what moves a constant envelope, i.e. multipath fading and noise-correction
//! error, which measure the look and not the emitter. A dead comparison kept "in case" implies a
//! discrimination that is not happening. Stored fingerprints written with the key still parse
//! (the key is ignored). The statistic itself remains in `hk_classify::structure`.
//!
...
impl Fingerprint {
    ...
    fn compare_inner(
        ...
        // relaxation safe, built one (`hk_classify::structure`'s `envelope_shape`) and measured it. It
        // cannot:
        //
        ...
        //
        // So the gate stays exact and ADR-0016 §1's change is **withdrawn**, not deferred. (T-233
        // also landed that statistic as split-only evidence on the fingerprint; T-594 removed it
        // for having no producer — see the module docs.) See
...
mod tests {
    ...
    /// and measured (`hk_classify::structure`'s `envelope_shape`) did not make the ADR's relaxation safe, so
    ...
    /// T-218): blind detection quality outranks contract tidiness.
    #[test]
    ...
    /// T-594: a fingerprint stored while T-233's `structure` field existed still parses, with the
    /// key ignored, so removing the dead comparison orphans no stored row. And a fingerprint that
    /// carried it compares exactly as one that did not: the key is read by nothing.
    #[test]
    fn t594_a_stored_fingerprint_with_the_removed_structure_key_still_parses() {
        let mut v = serde_json::to_value(Fingerprint::new(446.1e6, 16e3)).unwrap();
        v.as_object_mut().unwrap().insert(
            "structure".into(),
            serde_json::json!({"envelope_shape": 1.38, "envelope_shape_sigma": 0.01}),
        );
        let fp = Fingerprint::from_value(&v).expect("an old row parses");
        assert_eq!(fp, Fingerprint::new(446.1e6, 16e3));
        assert!(
