...
//! pyramid's margin). `fbo` is the coverage-weighted tile occupancy. T-118 replaces this provider
//! with its `OccupancyStat` series behind [`OccupancyProvider`]; the report does not change.
...
/// Time rows a report grid may have (the finest level that fits is used).
pub const REPORT_MAX_TIME_CELLS: usize = 20_000;
/// Frequency columns a report grid may have.
pub const REPORT_MAX_FREQ_CELLS: usize = 512;
...
impl HistoryTiles {
    /// Queries `p` over the box at the finest level within `max_t × max_f` cells.
    pub fn query(
        p: &Pyramid,
        region: FreqRange,
        span: TimeRange,
        ...
        let grid = p
            .query(&RegionQuery {
                freq: region,
                time: span,
                resolution: Resolution::MaxCells { t: max_t, f: max_f },
            })
            .map_err(|e| ReportError::Provider(format!("history query failed: {e}")))?;
        Ok(Self::from_grid(
            grid,
            p.geometry().levels[0].f_cell_hz,
    ...
    fn stat(
        ...
        let gaps: Vec<f64> = s
            ...
            .collect();
        let fco = (s.n_revisits > 0).then(|| s.n_occupied as f64 / s.n_revisits as f64);
        OccupancyStat {
            ...
            fbo: s.fbo,
            sro: None,
            n_revisits: s.n_revisits,
            n_occupied: s.n_occupied,
            n_suspect: 0,
        ...
        let mut rows = OccupancyRows {
            ...
            warnings: vec![format!(
                "occupancy from history-tile occupancy (floor + {:.1} dB) over blind inventory \
                 extents at {:.0} s × {:.0} Hz cells; interim until the occupancy engine (T-118)",
                self.margin_db,
