...
pub struct TrackerConfig {
    ...
    /// In-band fragments (T-101): a track that closes with its **band overlapping** a continuous
    /// track's (≥ 90 % on air, live or recently closed) detected extent (T-102: the wider of its
    /// OBW and its detections' threshold-crossing boxes, plus the frequency tolerance; T-390:
    /// overlap, because that extent is the host's *mean* crossing and skirt flicker is the
    /// excursions past it), its whole observed life inside that track's, that extent at least this
    /// many times wider and ≥ 6 dB stronger (mean detection SNR), is edge/modulation flicker of
    /// that emission (a WFM station's 6 ms, 5 dB skirt boxes). It is still tracked (and may attach
    /// a chain) but its summary is marked `TrackSummary::inband_fragment` and it gets no inventory
    /// entry. **This width ratio is the guard**: it is what keeps two adjacent broadcast stations,
    /// whose occupied bands genuinely overlap, two separate rows (T-233). Before T-084's
    /// channel-SNR gate these fragments hid inside chance hop sets (4; 0 disables).
