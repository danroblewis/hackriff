...
`done` is refused over a failing test, so the `tests` entries must be real exit codes from the commands you ran. A deliberate **red proof** — your new test run on the old code, or with the defect re-injected — is listed with its real non-zero exit **and `"expect": "red"`**; it counts only beside a green run of the same test with your fix. `cancel` is a **proposal** with evidence — a reviewer confirms it, you do not close the ticket. If you cannot write the file, print one line `HANDBACK: <the same JSON>` as your last output. Then stop — you don't merge, you don't spawn subagents.
