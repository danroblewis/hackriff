...
- **Dashboard changes are previewed at hand-back** (user, 2026-09-24: he does not wait behind a merge batch to SEE one). A branch that changes `ops/monitor.py` or a page module (`ops/worklog.py`, `ops/metricspage.py`, …): `bash ops/preview-dashboard.sh <branch> <changed paths>` copies its committed `ops/` + `py/` into `$HACKRIFF_OPS/preview` and runs it on **:8902** (never a worktree, never main); it must print `OK` for each changed page with data, and the `http://127.0.0.1:8902<path>` link goes in your work log. The real :8901 still restarts from main when the branch lands. Such branches (class `py+ops` / `ui`) are gated first and alone by the runner's CHEAP FIRST rule.
