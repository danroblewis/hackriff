...
test("T-630: a named page carries `client` on every tile request, and an unnamed one carries none", () => {
  // Unnamed is the default and, `planes` aside (T-700 asks for the packed plane on every tile
  // request), is byte-identical to the pre-T-630 request: an undeclared caller shares the route's
  // anonymous bucket, which behaves as the route always did.
  setTileClientId("");
  ...
  assert.equal(tileUrl(ADDR), "/api/tiles?level_f=1&level_t=2&f_index=3&t_index=4&cells=2&planes=f16&client=tab-two");
  assert.equal(tileUrl(probeAddr()), "/api/tiles?level_f=0&level_t=0&f_index=0&t_index=0&cells=8&planes=f16&client=tab-two");
