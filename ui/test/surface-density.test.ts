...
//  4. **The honesty gate**: a cell draws ONLY when it would already be under the generalization
//     threshold in both axes — never a numbered bubble, never a wash over a fine-zoom box.
//  5. **Stroke-only** (hatch pattern): the data pass is byte-identical with the layer on and off.
//  6. **Registry**: an overlay layer, on by default, below `detections`.
//  7. **Thin client**: no fetch/client/device-route in the geometry module; the host fetches on
//     the poll, never in the frame, and reaches no device route.
import { test } from "node:test";
import assert from "node:assert/strict";
import { readFileSync } from "node:fs";
import {
  DENSITY_MARK, DENSITY_MAX_TILES_PER_PANE, densityAddrs, densityQuads, densityUrl, parseDensityTile,
  type DensityTile,
} from "../src/surface/density";
import { GENERALIZE_BELOW_CSS_PX } from "../src/surface/marks";
...
// ---------------------------------------------------------------------------
// 4. The honesty gate: only where a box would already generalize
// ---------------------------------------------------------------------------

const PANE = { f0Hz: 99e6, f1Hz: 101e6, t0Ns: T0 - 20 * S, t1Ns: T0 };
/** A viewport wide enough that a coarse tile's cells are genuinely sub-pixel — the actual shape of
 * "coarse zoom", not merely "one cell the size of the pane" (which would be huge on screen, the
 * opposite of what triggers generalization). */
const WIDE_PANE = WIDE;

function fineTile(): DensityTile {
  // Cells 1 MHz x 1 s: over a 2 MHz / 1000 px pane and a 20 s / 500 px pane, each cell is ~500 x 25
  // px — nowhere near the generalization threshold, so this tile must draw nothing.
  return {
    addr: ADDR, fLoHz: 100e6, fCellHz: 1e6, t0Ns: T0 - 20 * S, tCellNs: 1 * S, nt: 2, nf: 2,
    counts: [1, 2, 0, 4],
  };
}

function coarseTile(): DensityTile {
  // Cells 100 kHz x 1 h against the 6 GHz / 30-day WIDE_PANE at RECT's 1000x500 px: each cell is a
  // small fraction of a screen pixel in both axes — the sub-pixel-burst case docs/23 names.
  return {
...
test("MAP density: a fine tile (cells already box-sized) draws NOTHING — boxes win the pixel, never doubled", () => {
  const q = densityQuads([fineTile()], PANE, RECT, { dpr: 1, generalizeBelowPx: GENERALIZE_BELOW_CSS_PX });
  assert.deepEqual(q, []);
});

test("MAP density: a coarse tile (cells under the generalization threshold) draws a hatch-filled cell per non-empty count", () => {
  const q = densityQuads([coarseTile()], WIDE_PANE, RECT, { dpr: 1, generalizeBelowPx: GENERALIZE_BELOW_CSS_PX });
...
test("MAP density: a zero-count cell draws nothing, even under the threshold", () => {
  const zero: DensityTile = { ...coarseTile(), counts: [0] };
  assert.deepEqual(densityQuads([zero], WIDE_PANE, RECT), []);
});
...
test("MAP density: alpha rises with count, monotonically, and saturates at capCount — never a numbered bubble", () => {
  const alphaOf = (count: number) =>
    densityQuads([{ ...coarseTile(), counts: [count] }], WIDE_PANE, RECT, { capCount: 8 })[0].rgba[3];
  ...
  const q = densityQuads([{ ...coarseTile(), counts: [12] }], WIDE_PANE, RECT);
...
test("MAP density: off the pane draws nothing; only the visible part of a partly-on tile", () => {
  ...
  assert.deepEqual(densityQuads([coarseTile()], away, RECT), []);
  const half = { ...WIDE_PANE, f0Hz: 1.00005e6, f1Hz: 5e9 };
  const q = densityQuads([coarseTile()], half, RECT);
  for (const r of q) for (const c of r.clip) assert.ok(c >= -1 && c <= 1, "clipped to the pane");
});

// ---------------------------------------------------------------------------
// 5. Stroke-only, placed through the tiles' own mapping
// ---------------------------------------------------------------------------

test("MAP density: every cell is placed by the tiles' own toClip", () => {
  const q = densityQuads([coarseTile()], WIDE_PANE, RECT)[0];
  const [x0, y0, x1, y1] = toClip(
    { f0Hz: coarseTile().fLoHz, f1Hz: coarseTile().fLoHz + coarseTile().fCellHz, t0Ns: coarseTile().t0Ns, t1Ns: coarseTile().t0Ns + coarseTile().tCellNs },
    WIDE_PANE,
  );
...
test("MAP density: an overlay layer, on by default, below detections, toggleable per pane", () => {
  ...
  const fns = { density: () => densityQuads([coarseTile()], WIDE_PANE, RECT) };
