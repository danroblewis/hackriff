...
test("fade reaches the Active-outputs strip — and the pills, by being in the cluster — never what §10.2 exempts", () => {
  ...
  // T-996: the surface's statements are the one status line, each pane's chip and the two
  // conditional notes. T-1003 put the per-pane Retune ON that chip — a device command with a stated
  // destination — so `.sf-scale` now carries what `.map-retune` used to. None may fade.
  const NEVER = [".sheet", ".research", ".map-offer", ".map-mode", ".map-layers", ".map-pane-menu",
    ".sf-status", ".sf-scale", ".sf-pane-retune", ".sf-fog", ".sf-readout", ".side"];
