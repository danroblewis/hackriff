...
function mount(el: HTMLElement, ctx: AppContext) {
  ...
  const stage = h("div", { class: "sf-stage" }, canvas, activeEl, liveEl, pinsEl, hudEl, priorsLabelEl, annoEl, tipEl, captureEl);
  ...
  const boxesFor = (pane: PaneView): MarkBox[] => {
    ...
    return paneMarkBoxes(Object.values(s.inventory.rows), focusId, s.selections.list, selId, pane.box, pendingRegion, false, measurements, null, pendingMeasureRegion, !ownsSelection(pane.id));
  };
