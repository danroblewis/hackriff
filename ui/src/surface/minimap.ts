...
export interface OverlayQuad {
  ...
  /** `signal-box` and `selection-box` are T-445's in-pane marks (`./marks.ts`); `trace-slice` and
   * `trace-hold` are T-457's spectrum trace (`./trace.ts`), drawn in the strip above a pane; the
   * remaining two are the map's own. All six are strokes, and `overlay.ts` can draw nothing else. */
  readonly kind: "pane-outline" | "live-segment" | "signal-box" | "selection-box" | "trace-slice" | "trace-hold";
