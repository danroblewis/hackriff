...
export interface OverlayQuad {
   ...
   * (`./hud.ts`) along a pane's bottom and left edges; `annotation` is T-820's human-authored note
   * (`./annotations.ts`, dashed); the remaining two are the map's own.
   * All are strokes, and `overlay.ts` can draw nothing else. */
  readonly kind: "pane-outline" | "live-segment" | "signal-box" | "selection-box" | "measurement-box" | "annotation" | "pending-region" | "trace-slice" | "trace-hold" | "time-rule" | "hud-tick";
