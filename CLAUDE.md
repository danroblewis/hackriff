...
**Phase: building M0 (first vertical slice), started 2026-09-13.** Product code exists: Cargo workspace, data model + SQLite, ring/source/replay, spectral estimation, noise floor, channelizer/DDC, stream contract + plugin host, spectrum history, band-plan priors, synthetic generator + e2e harness, HackRF-derived SigMF fixtures. Spikes S1/S3/S4/S5 are done (reports in `spikes/`). Live task state is `docs/tasks.yaml`; decisions, review findings and user blockers are in the `docs/planning-log.md` build log (B0.x). Research is in `docs/01`–`05`; planning in `docs/06`–`12` + `docs/adr/` (ADRs PROVISIONAL; ADR-0001 records the S1 owned-dataflow outcome). Needs the user: Jetson purchase (S2/S6, T-026), FM notch + S4b re-run, 1090 MHz antenna, 50 Ω terminator capture, open questions in the planning log.
