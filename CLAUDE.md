...
- **Build/test/run:** `just build`, `just test` (T1 unit + T2 component + T3 replay + T4 synthetic; no hardware; `gpu` off), `just replay <fixture.sigmf-meta>` (run a SigMF fixture through the pipeline), `just deploy-jetson` (rsync + on-device build). CI runs `just test` + Python tooling tests with no hardware and must stay green. HIL (T5) runs on a bench rig nightly/manually; field (T6) is logged, never gates CI.
  ...
  - **UI-only merge gate (user, 2026-09-16):** a merge that changes **only** files under `ui/` (`ui/src`, `ui/test`, `ui/package*`, …) and touches **neither `crates/` nor `docs/api.md`** is gated on the UI path alone — **`just lint` + `just test-ui`** (typecheck, vitest, ui-build, lint) — and **skips `just test` and `just acceptance`**. The UI is a thin client and the backend is contract-tested (T-079), and no acceptance test drives the browser (the e2e suites run through the **device interface**), so a ui-only change cannot move the Rust signal path; running those suites against one proves nothing and costs minutes per iteration. **Any change touching `crates/` or `docs/api.md` is not ui-only** and keeps the full gate, because the API contract or the signal path may have moved. `just test` + `just acceptance` remain the periodic/milestone check and the gate for every backend or contract change.
    - The one thing this gate does not cover — and neither did the full one: a client that asks the backend for **the wrong thing**. Contract tests assert the *server* serves a route correctly, not that the client calls it correctly. T-367's time navigator requested `/api/timeline` with no band and drew an empty canvas while every suite stayed green. The guard for that class lives in `ui/test`: **assert the request the client builds**, not only the response it renders.
