...
/// in `dir`: the pipeline replays it blind while the test keeps the truth privately.
...
/// [`replay_config`] over a blind copy of `meta` ([`blind_meta`]), kept in the returned
/// [`TempDir`] (outside the run's data directory). The replay entry point of the T-037b edits.
