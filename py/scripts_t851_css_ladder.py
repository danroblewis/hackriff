...
def adc(x, gain_db):
    ...
    def q(v):
        return np.clip(np.round(v * g), -128, 127)
...
def build(kind, sf, nsym, W, snr_db):
    ...
    if kind == "noise":
        return noise
    if kind == "wrong_sf":  # a true SF7 chirp read at the SF9 hypothesis
        c, _ = chirp_syms(7, BW, int(np.ceil(L / int(round(FS * 128 / BW)))) + 1)
    elif kind == "wrong_bw":
...
def run(sf, nsym, W):
    ...
    raw = {
        k: build(k, sf, nsym, W if k == "noise" else W // 4, s)
    ...
    for gname in ["float"] + GAINS:
        ...
        clips = {
            k: (float(np.mean(np.abs(adc(v, g).real) >= 127)) if g is not None else 0.0)
                ...
                for gname, sg, clip, st in rows:
                    ...
                    rec.append(
                        dict(
                            ...
                            delta=per,
                            distinct=int(len(np.unique(fl["noise"][mi]))),
                        )
                    )
        res[nsym] = rec
    os.makedirs("/tmp/t851", exist_ok=True)
    json.dump(res, open("/tmp/t851/out.json", "w"))
    BUCKETS = {
        "none": (0, 1.1),
        "nominal s>=.5 clip<=30": (0.5, 0.30),
        "tight s>=1 clip<=10": (1.0, 0.10),
        "s>=2 clip<=10": (2.0, 0.10),
    }
        ...
        for m in ("peak_ratio", "bin_run"):
            for b in (4, 6, 8):
                for bn, (smin, cmax) in BUCKETS.items():
                    ...
                    if ds:
                        print(
                            f"  {m:10s} b={b} {bn:24s} worst d {max(ds):+.2f}  min recall {min(rcs):.2f}"
                        )
